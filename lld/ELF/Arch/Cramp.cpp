...
class Cramp final : public TargetInfo {
public:
  Cramp();
...
uint32_t Cramp::calcEFlags() const {
  ...
  for (InputFile *f : ctx->objectFiles) {
    ...
    if (eflags & EF_CRAMP_RVC)
      target |= EF_CRAMP_RVC;

    if ((eflags & EF_CRAMP_FLOAT_ABI) != (target & EF_CRAMP_FLOAT_ABI))
      error(toString(f) +
            ": cannot link object files with different floating-point ABI");

    if ((eflags & EF_CRAMP_RVE) != (target & EF_CRAMP_RVE))
      error(toString(f) +
            ": cannot link object files with different EF_CRAMP_RVE");
  }

  return target;
}

int64_t Cramp::getImplicitAddend(const uint8_t *buf, RelType type) const {
  switch (type) {
  default:
...
void Cramp::writeGotHeader(uint8_t *buf) const {
  if (config->is64)
...
void Cramp::writeGotPlt(uint8_t *buf, const Symbol &s) const {
  if (config->is64)
...
void Cramp::writeIgotPlt(uint8_t *buf, const Symbol &s) const {
...
void Cramp::writePltHeader(uint8_t *buf) const {
...
void Cramp::writePlt(uint8_t *buf, const Symbol &sym,
...
RelType Cramp::getDynRel(RelType type) const {
  return type == target->symbolicRel ? type
                                     : static_cast<RelType>(R_RISCV_NONE);
}

RelExpr Cramp::getRelExpr(const RelType type, const Symbol &s,
...
void Cramp::relocate(uint8_t *loc, const Relocation &rel, uint64_t val) const {
...
static void relaxCall(const InputSection &sec, size_t i, uint64_t loc,
  ...
  const bool rvc = config->eflags & EF_CRAMP_RVC;
...
// relaxation pass.
bool Cramp::relaxOnce(int pass) const {
...
void elf::crampFinalizeRelax(int passes) {
  llvm::TimeTraceScope timeScope("Finalize Cramp relaxation");
...
TargetInfo *elf::getCrampTargetInfo() {
  static Cramp target;
  return &target;
}
