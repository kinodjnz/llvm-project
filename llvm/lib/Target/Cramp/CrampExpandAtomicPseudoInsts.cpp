//===-- CrampExpandAtomicPseudoInsts.cpp - Expand atomic pseudo instrs. ---===//
//
...
#define Cramp_EXPAND_ATOMIC_PSEUDO_NAME                                        \
  "Cramp atomic pseudo instruction expansion pass"

namespace {

class CrampExpandAtomicPseudo : public MachineFunctionPass {
public:
  const CrampInstrInfo *TII;
  static char ID;

  CrampExpandAtomicPseudo() : MachineFunctionPass(ID) {
    initializeCrampExpandAtomicPseudoPass(*PassRegistry::getPassRegistry());
  }

  bool runOnMachineFunction(MachineFunction &MF) override;

  StringRef getPassName() const override {
    return Cramp_EXPAND_ATOMIC_PSEUDO_NAME;
  }
...
char CrampExpandAtomicPseudo::ID = 0;

bool CrampExpandAtomicPseudo::runOnMachineFunction(MachineFunction &MF) {
...
bool CrampExpandAtomicPseudo::expandMBB(MachineBasicBlock &MBB) {
  bool Modified = false;
...
bool CrampExpandAtomicPseudo::expandMI(MachineBasicBlock &MBB,
                                       MachineBasicBlock::iterator MBBI,
                                       MachineBasicBlock::iterator &NextMBBI) {
...
static unsigned getLRForRMW32(AtomicOrdering Ordering) {
  ...
  case AtomicOrdering::Acquire:
    return Cramp::LR_W_AQ;
  case AtomicOrdering::Release:
    return Cramp::LR_W;
  case AtomicOrdering::AcquireRelease:
    return Cramp::LR_W_AQ;
  case AtomicOrdering::SequentiallyConsistent:
    return Cramp::LR_W_AQ_RL;
  }
}
...
static unsigned getSCForRMW32(AtomicOrdering Ordering) {
  ...
  case AtomicOrdering::Acquire:
    return Cramp::SC_W;
  case AtomicOrdering::Release:
    return Cramp::SC_W_RL;
  case AtomicOrdering::AcquireRelease:
    return Cramp::SC_W_RL;
  case AtomicOrdering::SequentiallyConsistent:
    return Cramp::SC_W_AQ_RL;
  }
}
...
static unsigned getLRForRMW64(AtomicOrdering Ordering) {
  ...
  case AtomicOrdering::Monotonic:
    return Cramp::LR_D;
  case AtomicOrdering::Acquire:
    return Cramp::LR_D_AQ;
  case AtomicOrdering::Release:
    return Cramp::LR_D;
  case AtomicOrdering::AcquireRelease:
    return Cramp::LR_D_AQ;
  case AtomicOrdering::SequentiallyConsistent:
    return Cramp::LR_D_AQ_RL;
  }
}
...
static unsigned getSCForRMW64(AtomicOrdering Ordering) {
  ...
  case AtomicOrdering::Acquire:
    return Cramp::SC_D;
  case AtomicOrdering::Release:
    return Cramp::SC_D_RL;
  case AtomicOrdering::AcquireRelease:
    return Cramp::SC_D_RL;
  case AtomicOrdering::SequentiallyConsistent:
    return Cramp::SC_D_AQ_RL;
  }
}
...
static void doAtomicBinOpExpansion(const CrampInstrInfo *TII, MachineInstr &MI,
  ...
  case AtomicRMWInst::Nand:
    BuildMI(LoopMBB, DL, TII->get(Cramp::AND), ScratchReg)
        .addReg(DestReg)
        .addReg(IncrReg);
    BuildMI(LoopMBB, DL, TII->get(Cramp::XORI), ScratchReg)
        .addReg(ScratchReg)
        .addImm(-1);
    break;
  }
...
static void insertMaskedMerge(const CrampInstrInfo *TII, DebugLoc DL,
  ...
  BuildMI(MBB, DL, TII->get(Cramp::XOR), ScratchReg)
      .addReg(OldValReg)
      .addReg(NewValReg);
  BuildMI(MBB, DL, TII->get(Cramp::AND), ScratchReg)
      .addReg(ScratchReg)
      .addReg(MaskReg);
  BuildMI(MBB, DL, TII->get(Cramp::XOR), DestReg)
      .addReg(OldValReg)
      .addReg(ScratchReg);
}

static void doMaskedAtomicBinOpExpansion(
    const CrampInstrInfo *TII, MachineInstr &MI, DebugLoc DL,
  ...
  case AtomicRMWInst::Xchg:
    BuildMI(LoopMBB, DL, TII->get(Cramp::ADDI), ScratchReg)
        .addReg(IncrReg)
        .addImm(0);
    break;
  case AtomicRMWInst::Add:
    BuildMI(LoopMBB, DL, TII->get(Cramp::ADD), ScratchReg)
        .addReg(DestReg)
        .addReg(IncrReg);
    break;
  case AtomicRMWInst::Sub:
    BuildMI(LoopMBB, DL, TII->get(Cramp::SUB), ScratchReg)
        .addReg(DestReg)
        .addReg(IncrReg);
    break;
  case AtomicRMWInst::Nand:
...
bool CrampExpandAtomicPseudo::expandAtomicBinOp(
...
static void insertSext(const CrampInstrInfo *TII, DebugLoc DL,
                       MachineBasicBlock *MBB, Register ValReg,
                       Register ShamtReg) {
  BuildMI(MBB, DL, TII->get(Cramp::SLL), ValReg)
      .addReg(ValReg)
      .addReg(ShamtReg);
  BuildMI(MBB, DL, TII->get(Cramp::SRA), ValReg)
      .addReg(ValReg)
      .addReg(ShamtReg);
}

bool CrampExpandAtomicPseudo::expandAtomicMinMaxOp(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator MBBI,
    AtomicRMWInst::BinOp BinOp, bool IsMasked, int Width,
    MachineBasicBlock::iterator &NextMBBI) {
  ...
  BuildMI(LoopHeadMBB, DL, TII->get(getLRForRMW32(Ordering)), DestReg)
      .addReg(AddrReg);
  BuildMI(LoopHeadMBB, DL, TII->get(Cramp::AND), Scratch2Reg)
      .addReg(DestReg)
      .addReg(MaskReg);
  BuildMI(LoopHeadMBB, DL, TII->get(Cramp::ADDI), Scratch1Reg)
      .addReg(DestReg)
      .addImm(0);
  ...
  case AtomicRMWInst::Max: {
    ...
    BuildMI(LoopHeadMBB, DL, TII->get(Cramp::BGE))
        .addReg(Scratch2Reg)
        .addReg(IncrReg)
        .addMBB(LoopTailMBB);
    break;
  ...
  case AtomicRMWInst::UMax:
    BuildMI(LoopHeadMBB, DL, TII->get(Cramp::BGEU))
        .addReg(Scratch2Reg)
        .addReg(IncrReg)
        .addMBB(LoopTailMBB);
    break;
  ...
  BuildMI(LoopTailMBB, DL, TII->get(getSCForRMW32(Ordering)), Scratch1Reg)
      .addReg(AddrReg)
      .addReg(Scratch1Reg);
  BuildMI(LoopTailMBB, DL, TII->get(Cramp::BNE))
      .addReg(Scratch1Reg)
      .addReg(Cramp::X0)
      .addMBB(LoopHeadMBB);
...
bool CrampExpandAtomicPseudo::expandAtomicCmpXchg(
  ...
  if (!IsMasked) {
    ...
    BuildMI(LoopHeadMBB, DL, TII->get(getLRForRMW(Ordering, Width)), DestReg)
        .addReg(AddrReg);
    BuildMI(LoopHeadMBB, DL, TII->get(Cramp::BNE))
        .addReg(DestReg)
        .addReg(CmpValReg)
        .addMBB(DoneMBB);
    // .looptail:
    ...
    BuildMI(LoopHeadMBB, DL, TII->get(getLRForRMW(Ordering, Width)), DestReg)
        .addReg(AddrReg);
    BuildMI(LoopHeadMBB, DL, TII->get(Cramp::AND), ScratchReg)
        .addReg(DestReg)
        .addReg(MaskReg);
    BuildMI(LoopHeadMBB, DL, TII->get(Cramp::BNE))
        .addReg(ScratchReg)
...
INITIALIZE_PASS(CrampExpandAtomicPseudo, "cramp-expand-atomic-pseudo",
                Cramp_EXPAND_ATOMIC_PSEUDO_NAME, false, false)

namespace llvm {

FunctionPass *createCrampExpandAtomicPseudoPass() {
  return new CrampExpandAtomicPseudo();
}
