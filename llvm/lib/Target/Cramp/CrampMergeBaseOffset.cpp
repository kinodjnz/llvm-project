//===----- CrampMergeBaseOffset.cpp - Optimise address calculations  ------===//
//
...
#define DEBUG_TYPE "cramp-merge-base-offset"
#define Cramp_MERGE_BASE_OFFSET_NAME "Cramp Merge Base Offset"
namespace {

struct CrampMergeBaseOffsetOpt : public MachineFunctionPass {
private:
  const CrampSubtarget *ST = nullptr;
...
public:
  ...
  CrampMergeBaseOffsetOpt() : MachineFunctionPass(ID) {}
  ...
  StringRef getPassName() const override {
    return Cramp_MERGE_BASE_OFFSET_NAME;
  }
...
char CrampMergeBaseOffsetOpt::ID = 0;
INITIALIZE_PASS(CrampMergeBaseOffsetOpt, DEBUG_TYPE,
                Cramp_MERGE_BASE_OFFSET_NAME, false, false)
...
bool CrampMergeBaseOffsetOpt::detectLuiAddiGlobal(MachineInstr &HiLUI,
                                                  MachineInstr *&LoADDI) {
  if (HiLUI.getOpcode() != Cramp::LUI ||
      HiLUI.getOperand(1).getTargetFlags() != CrampII::MO_HI ||
  ...
  if (LoADDI->getOpcode() != Cramp::ADDI ||
      LoADDI->getOperand(2).getTargetFlags() != CrampII::MO_LO ||
...
// output from LoADDI.
void CrampMergeBaseOffsetOpt::foldOffset(MachineInstr &HiLUI,
                                         MachineInstr &LoADDI,
...
bool CrampMergeBaseOffsetOpt::matchLargeOffset(MachineInstr &TailAdd,
                                               Register GAReg,
                                               int64_t &Offset) {
  assert((TailAdd.getOpcode() == Cramp::ADD) && "Expected ADD instruction!");
  ...
  if (OffsetTail.getOpcode() == Cramp::ADDI ||
      OffsetTail.getOpcode() == Cramp::ADDIW) {
    ...
    if (AddiImmOp.getTargetFlags() != CrampII::MO_None)
      return false;
    ...
    if (OffsetLui.getOpcode() != Cramp::LUI ||
        LuiImmOp.getTargetFlags() != CrampII::MO_None ||
    ...
    if (!ST->is64Bit() || OffsetTail.getOpcode() == Cramp::ADDIW)
    ...
    return true;
  } else if (OffsetTail.getOpcode() == Cramp::LUI) {
...
bool CrampMergeBaseOffsetOpt::matchShiftedOffset(MachineInstr &TailShXAdd,
                                                 Register GAReg,
                                                 int64_t &Offset) {
  assert((TailShXAdd.getOpcode() == Cramp::SH1ADD ||
          TailShXAdd.getOpcode() == Cramp::SH2ADD ||
          TailShXAdd.getOpcode() == Cramp::SH3ADD) &&
  ...
  if (!OffsetTail.getOperand(1).isReg() ||
      OffsetTail.getOperand(1).getReg() != Cramp::X0 ||
  ...
  case Cramp::SH1ADD: ShAmt = 1; break;
  case Cramp::SH2ADD: ShAmt = 2; break;
  case Cramp::SH3ADD: ShAmt = 3; break;
  }
...
bool CrampMergeBaseOffsetOpt::detectAndFoldOffset(MachineInstr &HiLUI,
                                                  MachineInstr &LoADDI) {
  ...
  if (MRI->hasOneUse(DestReg)) {
    ...
    case Cramp::ADDI: {
      ...
      if (MRI->hasOneUse(TailDestReg)) {
        ...
        if (TailTail.getOpcode() == Cramp::ADDI) {
...
bool CrampMergeBaseOffsetOpt::runOnMachineFunction(MachineFunction &Fn) {
  if (skipFunction(Fn.getFunction()))
    return false;

  ST = &Fn.getSubtarget<CrampSubtarget>();
...
FunctionPass *llvm::createCrampMergeBaseOffsetOptPass() {
  return new CrampMergeBaseOffsetOpt();
}
