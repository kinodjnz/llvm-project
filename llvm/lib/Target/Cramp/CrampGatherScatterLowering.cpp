//===- CrampGatherScatterLowering.cpp - Gather/Scatter lowering -----------===//
//
...
#define DEBUG_TYPE "cramp-gather-scatter-lowering"

namespace {

class CrampGatherScatterLowering : public FunctionPass {
  const CrampSubtarget *ST = nullptr;
  const CrampTargetLowering *TLI = nullptr;
  LoopInfo *LI = nullptr;
...
public:
  ...
  CrampGatherScatterLowering() : FunctionPass(ID) {}
  ...
  StringRef getPassName() const override {
    return "Cramp gather/scatter lowering";
  }
...
char CrampGatherScatterLowering::ID = 0;

INITIALIZE_PASS(CrampGatherScatterLowering, DEBUG_TYPE,
                "Cramp gather/scatter lowering pass", false, false)

FunctionPass *llvm::createCrampGatherScatterLoweringPass() {
  return new CrampGatherScatterLowering();
}

bool CrampGatherScatterLowering::isLegalTypeAndAlignment(Type *DataType,
                                                         Value *AlignOp) {
...
bool CrampGatherScatterLowering::matchStridedRecurrence(Value *Index, Loop *L,
                                                        Value *&Stride,
                                                        PHINode *&BasePtr,
                                                        BinaryOperator *&Inc,
                                                        IRBuilder<> &Builder) {
...
CrampGatherScatterLowering::determineBaseAndStride(GetElementPtrInst *GEP,
                                                   IRBuilder<> &Builder) {
...
bool CrampGatherScatterLowering::tryCreateStridedLoadStore(IntrinsicInst *II,
                                                           Type *DataType,
                                                           Value *Ptr,
                                                           Value *AlignOp) {
...
bool CrampGatherScatterLowering::runOnFunction(Function &F) {
  if (skipFunction(F))
    return false;

  auto &TPC = getAnalysis<TargetPassConfig>();
  auto &TM = TPC.getTM<CrampTargetMachine>();
  ST = &TM.getSubtarget<CrampSubtarget>(F);
