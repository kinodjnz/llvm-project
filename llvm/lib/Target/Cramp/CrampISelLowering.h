//===-- CrampISelLowering.h - Cramp DAG Lowering Interface ------*- C++ -*-===//
//
...
#ifndef LLVM_LIB_TARGET_Cramp_CrampISELLOWERING_H
#define LLVM_LIB_TARGET_Cramp_CrampISELLOWERING_H
...
enum NodeType : unsigned {
  ...
  // is passed as a TargetConstant operand using the CrampFPRndMode enum.
  FCVT_X,
  FCVT_XU,
...
class CrampTargetLowering : public TargetLowering {
  const CrampSubtarget &Subtarget;

public:
  explicit CrampTargetLowering(const TargetMachine &TM,
                               const CrampSubtarget &STI);

  const CrampSubtarget &getSubtarget() const { return Subtarget; }
...
private:
  /// CrampCCAssignFn - This target-specific function extends the default
  /// CCValAssign with additional information used to lower RISC-V calling
  /// conventions.
  typedef bool CrampCCAssignFn(const DataLayout &DL, CrampABI::ABI,
  ...
  SDValue lowerVPOp(SDValue Op, SelectionDAG &DAG, unsigned CrampISDOpc) const;
  ...
  /// Cramp doesn't have flags so it's better to perform the and/or in a GPR.
...
struct CrampVIntrinsicInfo {
  unsigned IntrinsicID;
  uint8_t ScalarOperand;
  uint8_t VLOperand;
  bool hasScalarOperand() const {
    // 0xF is not valid. See NoScalarOperand in IntrinsicsCramp.td.
    return ScalarOperand != 0xF;
  }
  bool hasVLOperand() const {
    // 0x1F is not valid. See NoVLOperand in IntrinsicsCramp.td.
...
#define GET_CrampVIntrinsicsTable_DECL
#include "CrampGenSearchableTables.inc"

} // end namespace CrampVIntrinsicsTable
