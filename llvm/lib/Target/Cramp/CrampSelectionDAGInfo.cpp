//===-- CrampSelectionDAGInfo.cpp - Cramp SelectionDAG Info -------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the CrampSelectionDAGInfo class.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "cramp-selectiondag-info"

// Emit, if possible, a specialized version of the given Libcall. Typically this
// means selecting the appropriately aligned version, but we also convert memset
// of 0 into memclr.
// SDValue CrampSelectionDAGInfo::EmitSpecializedLibcall(
...
SDValue CrampSelectionDAGInfo::EmitSpecializedMemclr(
  ...
  CLI.setDebugLoc(dl)
      .setChain(Chain)
      .setLibCallee(
          TLI->getLibcallCallingConv(RTLIB::BZERO), Type::getVoidTy(*DAG.getContext()),
          DAG.getExternalSymbol("__aligned_bzero4",
                                TLI->getPointerTy(DAG.getDataLayout())),
          std::move(Args))
      .setDiscardResult();
  std::pair<SDValue,SDValue> CallResult = TLI->LowerCallTo(CLI);
