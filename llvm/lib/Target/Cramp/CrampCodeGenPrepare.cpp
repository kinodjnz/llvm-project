...
//===----------------------------------------------------------------------===//
//
// This is a Cramp specific version of CodeGenPrepare.
...
#define DEBUG_TYPE "cramp-codegenprepare"
#define PASS_NAME "Cramp CodeGenPrepare"

STATISTIC(NumZExtToSExt, "Number of SExt instructions converted to ZExt");

namespace {

class CrampCodeGenPrepare : public FunctionPass {
  const DataLayout *DL;
...
public:
  ...
  CrampCodeGenPrepare() : FunctionPass(ID) {}
...
bool CrampCodeGenPrepare::optimizeZExt(ZExtInst *ZExt) {
  if (!ST->is64Bit())
    return false;
...
bool CrampCodeGenPrepare::optimizeAndExt(BinaryOperator *BO) {
  if (!ST->is64Bit())
    return false;
...
bool CrampCodeGenPrepare::runOnFunction(Function &F) {
  if (skipFunction(F))
    return false;
...
INITIALIZE_PASS_BEGIN(CrampCodeGenPrepare, DEBUG_TYPE, PASS_NAME, false, false)
INITIALIZE_PASS_DEPENDENCY(TargetPassConfig)
INITIALIZE_PASS_END(CrampCodeGenPrepare, DEBUG_TYPE, PASS_NAME, false, false)

char CrampCodeGenPrepare::ID = 0;

FunctionPass *llvm::createCrampCodeGenPreparePass() {
  return new CrampCodeGenPrepare();
}
