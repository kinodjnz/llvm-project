//=- CrampRedundantCopyElimination.cpp - Remove useless copy for Cramp ------=//
//
...
#define DEBUG_TYPE "cramp-copyelim"

STATISTIC(NumCopiesRemoved, "Number of copies removed.");

namespace {
class CrampRedundantCopyElimination : public MachineFunctionPass {
  const MachineRegisterInfo *MRI;
  const TargetRegisterInfo *TRI;

public:
  static char ID;
  CrampRedundantCopyElimination() : MachineFunctionPass(ID) {
    initializeCrampRedundantCopyEliminationPass(
  ...
  StringRef getPassName() const override {
    return "Cramp Redundant Copy Elimination";
  }
...
char CrampRedundantCopyElimination::ID = 0;

INITIALIZE_PASS(CrampRedundantCopyElimination, "cramp-copyelim",
                "Cramp redundant copy elimination pass", false, false)

static bool guaranteesZeroRegInBlock(const MachineInstr &MI,
                                     const MachineBasicBlock &MBB) {
  unsigned Opc = MI.getOpcode();
  if (Opc == Cramp::BEQ && MI.getOperand(1).getReg() == Cramp::X0 &&
      &MBB == MI.getOperand(2).getMBB())
    return true;
  if (Opc == Cramp::BNE && MI.getOperand(1).getReg() == Cramp::X0 &&
      &MBB != MI.getOperand(2).getMBB())
    return true;
...
bool CrampRedundantCopyElimination::optimizeBlock(MachineBasicBlock &MBB) {
  ...
  for (MachineBasicBlock::iterator I = MBB.begin(), E = MBB.end(); I != E;) {
    ...
    if (MI->isCopy() && MI->getOperand(0).isReg() &&
      ...
      if (SrcReg == Cramp::X0 && !MRI->isReserved(DefReg) &&
          TargetReg == DefReg) {
...
bool CrampRedundantCopyElimination::runOnMachineFunction(MachineFunction &MF) {
...
FunctionPass *llvm::createCrampRedundantCopyEliminationPass() {
  return new CrampRedundantCopyElimination();
}
