//===-- CrampBaseInfo.h - Top level definitions for Cramp MC ----*- C++ -*-===//
//
...
//
//===----------------------------------------------------------------------===//
#ifndef LLVM_LIB_TARGET_Cramp_MCTARGETDESC_CrampBASEINFO_H
#define LLVM_LIB_TARGET_Cramp_MCTARGETDESC_CrampBASEINFO_H
...
// CrampII - This namespace holds all of the target specific flags that
// instruction info tracks. All definitions must match CrampInstrFormats.td.
namespace CrampII {
enum {
  InstFormatPseudo = 0,
  InstFormatR = 1,
...
// Match with the definitions in CrampInstrFormats.td
...
enum OperandType : unsigned {
  OPERAND_FIRST_Cramp_IMM = MCOI::OPERAND_FIRST_TARGET,
  OPERAND_UIMM2 = OPERAND_FIRST_Cramp_IMM,
  OPERAND_UIMM3,
  OPERAND_UIMM4,
  OPERAND_UIMM5,
  OPERAND_UIMM7,
  ...
  OPERAND_RVKRNUM,
  OPERAND_LAST_Cramp_IMM = OPERAND_RVKRNUM,
...
inline static StringRef roundingModeToString(RoundingMode RndMode) {
  ...
  case CrampFPRndMode::RNE:
    return "rne";
  case CrampFPRndMode::RTZ:
    return "rtz";
  case CrampFPRndMode::RDN:
    return "rdn";
  case CrampFPRndMode::RUP:
    return "rup";
  case CrampFPRndMode::RMM:
    return "rmm";
  case CrampFPRndMode::DYN:
    return "dyn";
  }
}
...
inline static RoundingMode stringToRoundingMode(StringRef Str) {
  return StringSwitch<RoundingMode>(Str)
      .Case("rne", CrampFPRndMode::RNE)
      .Case("rtz", CrampFPRndMode::RTZ)
      .Case("rdn", CrampFPRndMode::RDN)
      .Case("rup", CrampFPRndMode::RUP)
      .Case("rmm", CrampFPRndMode::RMM)
      .Case("dyn", CrampFPRndMode::DYN)
      .Default(CrampFPRndMode::Invalid);
}
...
struct SysReg {
  ...
  bool haveRequiredFeatures(const FeatureBitset &ActiveFeatures) const {
    // Not in 32-bit mode.
    if (isRV32Only && ActiveFeatures[Cramp::Feature64Bit])
      return false;
...
#define GET_CrampOpcodesList_DECL
#include "CrampGenSearchableTables.inc"
} // end namespace CrampInsnOpcode
...
unsigned encodeVTYPE(CrampII::VLMUL VLMUL, unsigned SEW, bool TailAgnostic,
                     bool MaskAgnostic);

inline static CrampII::VLMUL getVLMUL(unsigned VType) {
  unsigned VLMUL = VType & 0x7;
  return static_cast<CrampII::VLMUL>(VLMUL);
}

// Decode VLMUL into 1,2,4,8 and fractional indicator.
std::pair<unsigned, bool> decodeVLMUL(CrampII::VLMUL VLMUL);

inline static CrampII::VLMUL encodeLMUL(unsigned LMUL, bool Fractional) {
  assert(isValidLMUL(LMUL, Fractional) && "Unsupported LMUL");
  unsigned LmulLog2 = Log2_32(LMUL);
  return static_cast<CrampII::VLMUL>(Fractional ? 8 - LmulLog2 : LmulLog2);
}
