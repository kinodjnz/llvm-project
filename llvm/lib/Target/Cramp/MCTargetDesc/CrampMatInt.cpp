//===- CrampMatInt.cpp - Immediate materialisation -------------*- C++ -*--===//
//
...
#include "CrampMatInt.h"
#include "MCTargetDesc/CrampMCTargetDesc.h"
#include "llvm/ADT/APInt.h"
#include "llvm/Support/MathExtras.h"
using namespace llvm;

static int getInstSeqCost(CrampMatInt::InstSeq &Res, bool HasRVC) {
  if (!HasRVC)
    return Res.size();
...
static void generateInstSeqImpl(int64_t Val,
                                ...
                                CrampMatInt::InstSeq &Res) {
  bool IsRV64 = ActiveFeatures[Cramp::Feature64Bit];
  ...
  if (isInt<32>(Val)) {
    ...
    if (Hi20)
      Res.push_back(CrampMatInt::Inst(Cramp::LUI, Hi20));

    if (Lo12 || Hi20 == 0) {
      unsigned AddiOpc = (IsRV64 && Hi20) ? Cramp::ADDIW : Cramp::ADDI;
      Res.push_back(CrampMatInt::Inst(AddiOpc, Lo12));
    }
    return;
  }
  ...
  if (ActiveFeatures[Cramp::FeatureStdExtZbs] && isPowerOf2_64(Val)) {
    Res.push_back(CrampMatInt::Inst(Cramp::BSETI, Log2_64(Val)));
    return;
  }
    ...
    if (ShiftAmount > 12 && !isInt<12>(Val)) {
      if (isInt<32>((uint64_t)Val << 12)) {
        ...
        Val = (uint64_t)Val << 12;
                 ...
                 ActiveFeatures[Cramp::FeatureStdExtZba]) {
  ...
  if (ShiftAmount) {
    if (Unsigned)
      Res.push_back(CrampMatInt::Inst(Cramp::SLLI_UW, ShiftAmount));
    else
      Res.push_back(CrampMatInt::Inst(Cramp::SLLI, ShiftAmount));
  }

  if (Lo12)
    Res.push_back(CrampMatInt::Inst(Cramp::ADDI, Lo12));
}
...
InstSeq generateInstSeq(int64_t Val, const FeatureBitset &ActiveFeatures) {
  ...
  if ((Val & 1) == 0 && Res.size() > 2) {
    ...
    TmpSeq.push_back(CrampMatInt::Inst(Cramp::SLLI, TrailingZeros));
  ...
  if (Val > 0 && Res.size() > 2) {
    assert(ActiveFeatures[Cramp::Feature64Bit] &&
    ...
    CrampMatInt::InstSeq TmpSeq;
    generateInstSeqImpl(ShiftedVal, ActiveFeatures, TmpSeq);
    TmpSeq.push_back(CrampMatInt::Inst(Cramp::SRLI, LeadingZeros));
    ...
    if (LeadingZeros == 32 && ActiveFeatures[Cramp::FeatureStdExtZba]) {
      ...
      TmpSeq.push_back(CrampMatInt::Inst(Cramp::ADD_UW, 0));
  ...
  if (Res.size() > 2 && ActiveFeatures[Cramp::FeatureStdExtZbs]) {
    assert(ActiveFeatures[Cramp::Feature64Bit] &&
           "Expected RV32 to only need 2 instructions");
    ...
    if (isInt<32>(NewVal)) {
      ...
      TmpSeq.push_back(CrampMatInt::Inst(Opc, 31));
      if (TmpSeq.size() < Res.size())
        Res = TmpSeq;
    }
    ...
    if (Opc > 0) {
      while (Hi != 0) {
        ...
        TmpSeq.push_back(CrampMatInt::Inst(Opc, Bit + 32));
        Hi &= ~(1 << Bit);
      }
      if (TmpSeq.size() < Res.size())
        Res = TmpSeq;
  ...
  if (Res.size() > 2 && ActiveFeatures[Cramp::FeatureStdExtZba]) {
    assert(ActiveFeatures[Cramp::Feature64Bit] &&
           "Expected RV32 to only need 2 instructions");
    int64_t Div = 0;
    unsigned Opc = 0;
    ...
    if (Div > 0) {
      ...
      TmpSeq.push_back(CrampMatInt::Inst(Opc, 0));
      if (TmpSeq.size() < Res.size())
        Res = TmpSeq;
    } else {
      ...
      if (Div > 0) {
        ...
        TmpSeq.push_back(CrampMatInt::Inst(Opc, 0));
        TmpSeq.push_back(CrampMatInt::Inst(Cramp::ADDI, Lo12));
        if (TmpSeq.size() < Res.size())
          Res = TmpSeq;
      }
    }
...
int getIntMatCost(const APInt &Val, unsigned Size,
  ...
  bool IsRV64 = ActiveFeatures[Cramp::Feature64Bit];
  bool HasRVC = CompressionCost && ActiveFeatures[Cramp::FeatureStdExtC];
...
OpndKind Inst::getOpndKind() const {
  ...
  case Cramp::LUI:
    return CrampMatInt::Imm;
  case Cramp::ADD_UW:
    return CrampMatInt::RegX0;
  case Cramp::SH1ADD:
  case Cramp::SH2ADD:
  case Cramp::SH3ADD:
    return CrampMatInt::RegReg;
  case Cramp::ADDI:
  case Cramp::ADDIW:
  case Cramp::SLLI:
  case Cramp::SRLI:
