//===-- CrampAsmBackend.cpp - Cramp Assembler Backend ---------------------===//
//
...
Optional<MCFixupKind> CrampAsmBackend::getFixupKind(StringRef Name) const {
...
CrampAsmBackend::getFixupKindInfo(MCFixupKind Kind) const {
  const static MCFixupKindInfo Infos[] = {
      // This table *must* be in the order that the fixup_* kinds are defined in
      // CrampFixupKinds.h.
      //
      // name                      offset bits  flags
      {"fixup_cramp_hi20", 12, 20, 0},
      {"fixup_cramp_lo12_i", 20, 12, 0},
      {"fixup_cramp_lo12_s", 0, 32, 0},
      {"fixup_cramp_pcrel_hi20", 12, 20,
       MCFixupKindInfo::FKF_IsPCRel | MCFixupKindInfo::FKF_IsTarget},
      {"fixup_cramp_pcrel_lo12_i", 20, 12,
       MCFixupKindInfo::FKF_IsPCRel | MCFixupKindInfo::FKF_IsTarget},
      {"fixup_cramp_pcrel_lo12_s", 0, 32,
       MCFixupKindInfo::FKF_IsPCRel | MCFixupKindInfo::FKF_IsTarget},
      {"fixup_cramp_got_hi20", 12, 20, MCFixupKindInfo::FKF_IsPCRel},
      {"fixup_cramp_tprel_hi20", 12, 20, 0},
      {"fixup_cramp_tprel_lo12_i", 20, 12, 0},
      {"fixup_cramp_tprel_lo12_s", 0, 32, 0},
      {"fixup_cramp_tprel_add", 0, 0, 0},
      {"fixup_cramp_tls_got_hi20", 12, 20, MCFixupKindInfo::FKF_IsPCRel},
      {"fixup_cramp_tls_gd_hi20", 12, 20, MCFixupKindInfo::FKF_IsPCRel},
      {"fixup_cramp_jal", 12, 20, MCFixupKindInfo::FKF_IsPCRel},
      {"fixup_cramp_branch", 0, 32, MCFixupKindInfo::FKF_IsPCRel},
      {"fixup_cramp_rvc_jump", 2, 11, MCFixupKindInfo::FKF_IsPCRel},
      {"fixup_cramp_rvc_branch", 0, 16, MCFixupKindInfo::FKF_IsPCRel},
      {"fixup_cramp_call", 0, 64, MCFixupKindInfo::FKF_IsPCRel},
      {"fixup_cramp_call_plt", 0, 64, MCFixupKindInfo::FKF_IsPCRel},
      {"fixup_cramp_relax", 0, 0, 0},
      {"fixup_cramp_align", 0, 0, 0},

      {"fixup_cramp_set_8", 0, 8, 0},
      {"fixup_cramp_add_8", 0, 8, 0},
      {"fixup_cramp_sub_8", 0, 8, 0},

      {"fixup_cramp_set_16", 0, 16, 0},
      {"fixup_cramp_add_16", 0, 16, 0},
      {"fixup_cramp_sub_16", 0, 16, 0},

      {"fixup_cramp_set_32", 0, 32, 0},
      {"fixup_cramp_add_32", 0, 32, 0},
      {"fixup_cramp_sub_32", 0, 32, 0},

      {"fixup_cramp_add_64", 0, 64, 0},
      {"fixup_cramp_sub_64", 0, 64, 0},

      {"fixup_cramp_set_6b", 2, 6, 0},
      {"fixup_cramp_sub_6b", 2, 6, 0},
  };
...
bool CrampAsmBackend::shouldForceRelocation(const MCAssembler &Asm,
                                            const MCFixup &Fixup,
  ...
  return STI.getFeatureBits()[Cramp::FeatureRelax] || ForceRelocs;
}

bool CrampAsmBackend::fixupNeedsRelaxationAdvanced(const MCFixup &Fixup,
                                                   bool Resolved,
                                                   uint64_t Value,
...
void CrampAsmBackend::relaxInstruction(MCInst &Inst,
  ...
  case Cramp::C_BEQZ:
    ...
    Res.setOpcode(Cramp::BEQ);
    Res.addOperand(Inst.getOperand(0));
    Res.addOperand(MCOperand::createReg(Cramp::X0));
    Res.addOperand(Inst.getOperand(1));
    break;
  case Cramp::C_BNEZ:
    // c.bnez $rs1, $imm -> bne $rs1, X0, $imm.
    Res.setOpcode(Cramp::BNE);
    Res.addOperand(Inst.getOperand(0));
    Res.addOperand(MCOperand::createReg(Cramp::X0));
    Res.addOperand(Inst.getOperand(1));
    break;
  ...
  case Cramp::C_JAL:
    ...
    Res.setOpcode(Cramp::JAL);
    Res.addOperand(MCOperand::createReg(Cramp::X1));
    Res.addOperand(Inst.getOperand(0));
    break;
  }
  Inst = std::move(Res);
...
bool CrampAsmBackend::relaxDwarfLineAddr(MCDwarfLineAddrFragment &DF,
                                         MCAsmLayout &Layout,
  ...
  if (Value > 60000) {
    ...
    Offset = OS.tell();
    Fixup = PtrSize == 4 ? std::make_pair(Cramp::fixup_cramp_add_32,
                                          Cramp::fixup_cramp_sub_32)
                         : std::make_pair(Cramp::fixup_cramp_add_64,
                                          Cramp::fixup_cramp_sub_64);
    OS.write_zeros(PtrSize);
  } else {
    OS << uint8_t(dwarf::DW_LNS_fixed_advance_pc);
    Offset = OS.tell();
    Fixup = {Cramp::fixup_cramp_add_16, Cramp::fixup_cramp_sub_16};
...
bool CrampAsmBackend::relaxDwarfCFA(MCDwarfCallFrameFragment &DF,
                                    MCAsmLayout &Layout,
                                    bool &WasRelaxed) const {
  ...
  if (isUIntN(6, Value)) {
    ...
    AddFixups(0, {Cramp::fixup_cramp_set_6b, Cramp::fixup_cramp_sub_6b});
  } else if (isUInt<8>(Value)) {
    OS << uint8_t(dwarf::DW_CFA_advance_loc1);
    support::endian::write<uint8_t>(OS, 0, support::little);
    AddFixups(1, {Cramp::fixup_cramp_set_8, Cramp::fixup_cramp_sub_8});
  } else if (isUInt<16>(Value)) {
    OS << uint8_t(dwarf::DW_CFA_advance_loc2);
    support::endian::write<uint16_t>(OS, 0, support::little);
    AddFixups(1, {Cramp::fixup_cramp_set_16, Cramp::fixup_cramp_sub_16});
  } else if (isUInt<32>(Value)) {
    OS << uint8_t(dwarf::DW_CFA_advance_loc4);
    support::endian::write<uint32_t>(OS, 0, support::little);
    AddFixups(1, {Cramp::fixup_cramp_set_32, Cramp::fixup_cramp_sub_32});
  } else {
...
unsigned CrampAsmBackend::getRelaxedOpcode(unsigned Op) const {
  switch (Op) {
  default:
    return Op;
  case Cramp::C_BEQZ:
  ...
  case Cramp::C_BNEZ:
    return Cramp::BNE;
  case Cramp::C_J:
  case Cramp::C_JAL: // fall through.
    return Cramp::JAL;
  }
}

bool CrampAsmBackend::mayNeedRelaxation(const MCInst &Inst,
                                        const MCSubtargetInfo &STI) const {
  return getRelaxedOpcode(Inst.getOpcode()) != Inst.getOpcode();
}

bool CrampAsmBackend::writeNopData(raw_ostream &OS, uint64_t Count,
                                   const MCSubtargetInfo *STI) const {
  bool HasStdExtC = STI->getFeatureBits()[Cramp::FeatureStdExtC];
...
bool CrampAsmBackend::evaluateTargetFixup(
  ...
  case Cramp::fixup_cramp_pcrel_lo12_i:
  case Cramp::fixup_cramp_pcrel_lo12_s: {
    AUIPCFixup = cast<CrampMCExpr>(Fixup.getValue())->getPCRelHiFixup(&AUIPCDF);
    if (!AUIPCFixup) {
...
void CrampAsmBackend::applyFixup(const MCAssembler &Asm, const MCFixup &Fixup,
                                 const MCValue &Target,
...
bool CrampAsmBackend::shouldInsertExtraNopBytesForCodeAlign(
...
bool CrampAsmBackend::shouldInsertFixupForCodeAlign(MCAssembler &Asm,
  ...
  // Create fixup_cramp_align fixup.
  MCFixup Fixup =
      MCFixup::create(0, Dummy, MCFixupKind(Cramp::fixup_cramp_align), SMLoc());
...
CrampAsmBackend::createObjectTargetWriter() const {
  return createCrampELFObjectWriter(OSABI, Is64Bit);
}

MCAsmBackend *llvm::createCrampAsmBackend(const Target &T,
                                          const MCSubtargetInfo &STI,
                                          const MCRegisterInfo &MRI,
                                          const MCTargetOptions &Options) {
  const Triple &TT = STI.getTargetTriple();
  uint8_t OSABI = MCELFObjectTargetWriter::getOSABI(TT.getOS());
  return new CrampAsmBackend(STI, OSABI, TT.isArch64Bit(), Options);
}
