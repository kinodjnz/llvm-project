//===-- CrampISelDAGToDAG.cpp - A dag to dag inst selector for Cramp ------===//
//
...
namespace llvm {
namespace Cramp {
#define GET_CrampVSSEGTable_IMPL
#define GET_CrampVLSEGTable_IMPL
#define GET_CrampVLXSEGTable_IMPL
#define GET_CrampVSXSEGTable_IMPL
#define GET_CrampVLETable_IMPL
#define GET_CrampVSETable_IMPL
#define GET_CrampVLXTable_IMPL
#define GET_CrampVSXTable_IMPL
#define GET_CrampMaskedPseudosTable_IMPL
#include "CrampGenSearchableTables.inc"
} // namespace Cramp
} // namespace llvm

void CrampDAGToDAGISel::PreprocessISelDAG() {
  ...
  while (Position != CurDAG->allnodes_begin()) {
    ...
    case ISD::SPLAT_VECTOR: {
      ...
      unsigned Opc =
          VT.isInteger() ? CrampISD::VMV_V_X_VL : CrampISD::VFMV_V_F_VL;
      SDLoc DL(N);
      SDValue VL = CurDAG->getRegister(Cramp::X0, Subtarget->getXLenVT());
      Result = CurDAG->getNode(Opc, DL, VT, CurDAG->getUNDEF(VT),
                               N->getOperand(0), VL);
      break;
    }
    case CrampISD::SPLAT_VECTOR_SPLIT_I64_VL: {
      ...
      CrampMachineFunctionInfo *FuncInfo =
          MF.getInfo<CrampMachineFunctionInfo>();
      SDLoc DL(N);
      ...
      SDValue IntID =
          CurDAG->getTargetConstant(Intrinsic::cramp_vlse, DL, MVT::i64);
      SDValue Ops[] = {Chain,
                       IntID,
                       Passthru,
                       StackSlot,
                       CurDAG->getRegister(Cramp::X0, MVT::i64),
                       VL};
    ...
    if (Result) {
      LLVM_DEBUG(dbgs() << "Cramp DAG preprocessing replacing:\nOld:    ");
...
void CrampDAGToDAGISel::PostprocessISelDAG() {
...
static SDNode *selectImmSeq(SelectionDAG *CurDAG, const SDLoc &DL, const MVT VT,
  ...
  SDValue SrcReg = CurDAG->getRegister(Cramp::X0, VT);
  for (CrampMatInt::Inst &Inst : Seq) {
    ...
    case CrampMatInt::RegX0:
      Result = CurDAG->getMachineNode(Inst.Opc, DL, VT, SrcReg,
                                      CurDAG->getRegister(Cramp::X0, VT));
      break;
...
static SDNode *selectImm(SelectionDAG *CurDAG, const SDLoc &DL, const MVT VT,
                         int64_t Imm, const CrampSubtarget &Subtarget) {
  CrampMatInt::InstSeq Seq =
...
static SDValue createTuple(SelectionDAG &CurDAG, ArrayRef<SDValue> Regs,
                           unsigned NF, CrampII::VLMUL LMUL) {
  static const unsigned M1TupleRegClassIDs[] = {
      Cramp::VRN2M1RegClassID, Cramp::VRN3M1RegClassID, Cramp::VRN4M1RegClassID,
      Cramp::VRN5M1RegClassID, Cramp::VRN6M1RegClassID, Cramp::VRN7M1RegClassID,
      Cramp::VRN8M1RegClassID};
  static const unsigned M2TupleRegClassIDs[] = {Cramp::VRN2M2RegClassID,
                                                Cramp::VRN3M2RegClassID,
                                                Cramp::VRN4M2RegClassID};
...
void CrampDAGToDAGISel::addVectorLoadStoreOperands(
  ...
  if (IsMasked) {
    ...
    Chain = CurDAG->getCopyToReg(Chain, DL, Cramp::V0, Mask, SDValue());
    Glue = Chain.getValue(1);
    Operands.push_back(CurDAG->getRegister(Cramp::V0, Mask.getValueType()));
  }
  SDValue VL;
...
void CrampDAGToDAGISel::selectVLSEG(SDNode *Node, bool IsMasked,
                                    bool IsStrided) {
  SDLoc DL(Node);
  ...
  const Cramp::VLSEGPseudo *P =
      Cramp::getVLSEGPseudo(NF, IsMasked, IsTU, IsStrided, /*FF*/ false, Log2SEW,
  ...
  for (unsigned I = 0; I < NF; ++I) {
    unsigned SubRegIdx = CrampTargetLowering::getSubregIndexByMVT(VT, I);
...
void CrampDAGToDAGISel::selectVLSEGFF(SDNode *Node, bool IsMasked) {
  SDLoc DL(Node);
  ...
  const Cramp::VLSEGPseudo *P =
      Cramp::getVLSEGPseudo(NF, IsMasked, IsTU, /*Strided*/ false, /*FF*/ true,
...
void CrampDAGToDAGISel::selectVLXSEG(SDNode *Node, bool IsMasked,
                                     bool IsOrdered) {
  SDLoc DL(Node);
  unsigned NF = Node->getNumValues() - 1;
  MVT VT = Node->getSimpleValueType(0);
...
void CrampDAGToDAGISel::selectVSSEG(SDNode *Node, bool IsMasked,
                                    bool IsStrided) {
  SDLoc DL(Node);
  ...
  const Cramp::VSSEGPseudo *P = Cramp::getVSSEGPseudo(
...
void CrampDAGToDAGISel::selectVSXSEG(SDNode *Node, bool IsMasked,
                                     bool IsOrdered) {
  SDLoc DL(Node);
  ...
  const Cramp::VSXSEGPseudo *P = Cramp::getVSXSEGPseudo(
...
void CrampDAGToDAGISel::selectVSETVLI(SDNode *Node) {
  ...
  assert((IntNo == Intrinsic::cramp_vsetvli ||
          IntNo == Intrinsic::cramp_vsetvlimax ||
          IntNo == Intrinsic::cramp_vsetvli_opt ||
          IntNo == Intrinsic::cramp_vsetvlimax_opt) &&
         "Unexpected vsetvli intrinsic");

  bool VLMax = IntNo == Intrinsic::cramp_vsetvlimax ||
               IntNo == Intrinsic::cramp_vsetvlimax_opt;
  ...
  unsigned SEW =
      CrampVType::decodeVSEW(Node->getConstantOperandVal(Offset) & 0x7);
  CrampII::VLMUL VLMul = static_cast<CrampII::VLMUL>(
      Node->getConstantOperandVal(Offset + 1) & 0x7);

  unsigned VTypeI = CrampVType::encodeVTYPE(VLMul, SEW, /*TailAgnostic*/ true,
                                            /*MaskAgnostic*/ false);
  ...
  SDValue VLOperand;
  unsigned Opcode = Cramp::PseudoVSETVLI;
  if (VLMax) {
    VLOperand = CurDAG->getRegister(Cramp::X0, XLenVT);
    Opcode = Cramp::PseudoVSETVLIX0;
  } else {
    ...
    if (auto *C = dyn_cast<ConstantSDNode>(VLOperand)) {
      ...
      if (isUInt<5>(AVL)) {
        ...
        ReplaceNode(
            Node, CurDAG->getMachineNode(Cramp::PseudoVSETIVLI, DL, VTs, Ops));
        return;
      }
    }
  }
...
void CrampDAGToDAGISel::Select(SDNode *Node) {
  ...
  case ISD::Constant: {
    ...
    if (VT == XLenVT && ConstNode->isZero()) {
      SDValue New =
          CurDAG->getCopyFromReg(CurDAG->getEntryNode(), DL, Cramp::X0, XLenVT);
    ...
    if (ShAmt <= 32 && isShiftedMask_64(Mask)) {
      ...
      if (TrailingZeros > 0 && LeadingZeros == 32) {
        SDNode *SRLIW = CurDAG->getMachineNode(
            Cramp::SRLIW, DL, VT, N0->getOperand(0),
            CurDAG->getTargetConstant(TrailingZeros, DL, VT));
        SDNode *SLLI = CurDAG->getMachineNode(
            Cramp::SLLI, DL, VT, SDValue(SRLIW, 0),
  ...
  case ISD::SRL: {
    ...
    if (Subtarget->hasStdExtZbs() && ShAmt + 1 == TrailingOnes) {
      SDNode *BEXTI =
          CurDAG->getMachineNode(Cramp::BEXTI, DL, VT, N0->getOperand(0),
    ...
    SDNode *SLLI =
        CurDAG->getMachineNode(Cramp::SLLI, DL, VT, N0->getOperand(0),
                               CurDAG->getTargetConstant(LShAmt, DL, VT));
    SDNode *SRLI = CurDAG->getMachineNode(
        Cramp::SRLI, DL, VT, SDValue(SLLI, 0),
  ...
  case ISD::SRA: {
    ...
    SDNode *SRAI = CurDAG->getMachineNode(
        Cramp::SRAI, DL, VT, SDValue(SLLI, 0),
        CurDAG->getTargetConstant(LShAmt + ShAmt, DL, VT));
  ...
  case ISD::AND: {
    ...
    if (!LeftShift && isMask_64(C1)) {
      ...
      if (C2 < Leading) {
        ...
        if (C2 + 32 == Leading) {
          SDNode *SRLIW = CurDAG->getMachineNode(
              Cramp::SRLIW, DL, VT, X, CurDAG->getTargetConstant(C2, DL, VT));
        ...
        if (C2 >= 32 && (Leading - C2) == 1 && N0.hasOneUse() &&
          ...
          SDNode *SRAIW =
              CurDAG->getMachineNode(Cramp::SRAIW, DL, VT, X.getOperand(0),
                                     CurDAG->getTargetConstant(31, DL, VT));
          SDNode *SRLIW = CurDAG->getMachineNode(
              Cramp::SRLIW, DL, VT, SDValue(SRAIW, 0),
      ...
      if (C2 + Leading < XLen &&
        ...
        if ((XLen - (C2 + Leading)) == 32 && Subtarget->hasStdExtZba()) {
          SDNode *SLLI_UW = CurDAG->getMachineNode(
              Cramp::SLLI_UW, DL, VT, X, CurDAG->getTargetConstant(C2, DL, VT));
    ...
    if (!LeftShift && isShiftedMask_64(C1)) {
      ...
      if (Leading == C2 && C2 + Trailing < XLen && OneUseOrZExtW && !IsCANDI) {
        ...
        SDNode *SLLI =
            CurDAG->getMachineNode(Cramp::SLLI, DL, VT, SDValue(SRLI, 0),
      ...
      if (Leading > 32 && (Leading - 32) == C2 && C2 + Trailing < 32 &&
        ...
        SDNode *SRLIW = CurDAG->getMachineNode(
            ...
            CurDAG->getTargetConstant(C2 + Trailing, DL, VT));
        SDNode *SLLI =
            CurDAG->getMachineNode(Cramp::SLLI, DL, VT, SDValue(SRLIW, 0),
                                   CurDAG->getTargetConstant(Trailing, DL, VT));
        ReplaceNode(Node, SLLI);
        return;
      }
  ...
  case ISD::MUL: {
    ...
    SDNode *MULHU = CurDAG->getMachineNode(Cramp::MULHU, DL, VT,
  ...
  case ISD::INTRINSIC_WO_CHAIN: {
    ...
    default:
      break;
    case Intrinsic::cramp_vmsgeu:
    case Intrinsic::cramp_vmsge: {
      SDValue Src1 = Node->getOperand(1);
      SDValue Src2 = Node->getOperand(2);
      bool IsUnsigned = IntNo == Intrinsic::cramp_vmsgeu;
      ...
      switch (CrampTargetLowering::getLMUL(Src1VT)) {
      default:
        llvm_unreachable("Unexpected LMUL!");
#define CASE_VMSLT_VMNAND_VMSET_OPCODES(lmulenum, suffix, suffix_b)            \
  case CrampII::VLMUL::lmulenum:                                               \
    VMSLTOpcode = IsUnsigned ? Cramp::PseudoVMSLTU_VX_##suffix                 \
                             : Cramp::PseudoVMSLT_VX_##suffix;                 \
    VMNANDOpcode = Cramp::PseudoVMNAND_MM_##suffix;                            \
    VMSetOpcode = Cramp::PseudoVMSET_M_##suffix_b;                             \
    break;
...
#undef CASE_VMSLT_VMNAND_VMSET_OPCODES
    ...
    case Intrinsic::cramp_vmsgeu_mask:
    case Intrinsic::cramp_vmsge_mask: {
      SDValue Src1 = Node->getOperand(2);
      SDValue Src2 = Node->getOperand(3);
      bool IsUnsigned = IntNo == Intrinsic::cramp_vmsgeu_mask;
      bool IsCmpUnsignedZero = false;
      // Only custom select scalar second operand.
      if (Src2.getValueType() != XLenVT)
        break;
...
#define CASE_VMSLT_OPCODES(lmulenum, suffix, suffix_b)                         \
  case CrampII::VLMUL::lmulenum:                                               \
    VMSLTOpcode = IsUnsigned ? Cramp::PseudoVMSLTU_VX_##suffix                 \
                             : Cramp::PseudoVMSLT_VX_##suffix;                 \
    VMSLTMaskOpcode = IsUnsigned ? Cramp::PseudoVMSLTU_VX_##suffix##_MASK      \
                                 : Cramp::PseudoVMSLT_VX_##suffix##_MASK;      \
    break;
...
#undef CASE_VMSLT_OPCODES
      ...
      switch (CrampTargetLowering::getLMUL(VT)) {
      default:
        llvm_unreachable("Unexpected LMUL!");
#define CASE_VMXOR_VMANDN_VMOR_OPCODES(lmulenum, suffix)                       \
  case CrampII::VLMUL::lmulenum:                                               \
    VMXOROpcode = Cramp::PseudoVMXOR_MM_##suffix;                              \
    VMANDNOpcode = Cramp::PseudoVMANDN_MM_##suffix;                            \
    VMOROpcode = Cramp::PseudoVMOR_MM_##suffix;                                \
    break;
...
#undef CASE_VMXOR_VMANDN_VMOR_OPCODES
      ...
      SDValue Chain = CurDAG->getCopyToReg(CurDAG->getEntryNode(), DL,
                                           Cramp::V0, Mask, SDValue());
      SDValue Glue = Chain.getValue(1);
      SDValue V0 = CurDAG->getRegister(Cramp::V0, VT);
  ...
  case ISD::INTRINSIC_W_CHAIN: {
    ...
    case Intrinsic::cramp_vsetvlimax:
      return selectVSETVLI(Node);
    case Intrinsic::cramp_vlseg2:
    case Intrinsic::cramp_vlseg3:
    case Intrinsic::cramp_vlseg4:
    case Intrinsic::cramp_vlseg5:
    case Intrinsic::cramp_vlseg6:
    case Intrinsic::cramp_vlseg7:
    case Intrinsic::cramp_vlseg8: {
      selectVLSEG(Node, /*IsMasked*/ false, /*IsStrided*/ false);
      return;
    }
    case Intrinsic::cramp_vlseg2_mask:
    case Intrinsic::cramp_vlseg3_mask:
    case Intrinsic::cramp_vlseg4_mask:
    case Intrinsic::cramp_vlseg5_mask:
    case Intrinsic::cramp_vlseg6_mask:
    case Intrinsic::cramp_vlseg7_mask:
    case Intrinsic::cramp_vlseg8_mask: {
      selectVLSEG(Node, /*IsMasked*/ true, /*IsStrided*/ false);
      return;
    }
    case Intrinsic::cramp_vlsseg2:
    case Intrinsic::cramp_vlsseg3:
    case Intrinsic::cramp_vlsseg4:
    case Intrinsic::cramp_vlsseg5:
    case Intrinsic::cramp_vlsseg6:
    case Intrinsic::cramp_vlsseg7:
    case Intrinsic::cramp_vlsseg8: {
      selectVLSEG(Node, /*IsMasked*/ false, /*IsStrided*/ true);
      return;
    }
    case Intrinsic::cramp_vlsseg2_mask:
    case Intrinsic::cramp_vlsseg3_mask:
    case Intrinsic::cramp_vlsseg4_mask:
    case Intrinsic::cramp_vlsseg5_mask:
    case Intrinsic::cramp_vlsseg6_mask:
    case Intrinsic::cramp_vlsseg7_mask:
    case Intrinsic::cramp_vlsseg8_mask: {
      selectVLSEG(Node, /*IsMasked*/ true, /*IsStrided*/ true);
      return;
    }
    case Intrinsic::cramp_vloxseg2:
    case Intrinsic::cramp_vloxseg3:
    case Intrinsic::cramp_vloxseg4:
    case Intrinsic::cramp_vloxseg5:
    case Intrinsic::cramp_vloxseg6:
    case Intrinsic::cramp_vloxseg7:
    case Intrinsic::cramp_vloxseg8:
      selectVLXSEG(Node, /*IsMasked*/ false, /*IsOrdered*/ true);
      return;
    case Intrinsic::cramp_vluxseg2:
    case Intrinsic::cramp_vluxseg3:
    case Intrinsic::cramp_vluxseg4:
    case Intrinsic::cramp_vluxseg5:
    case Intrinsic::cramp_vluxseg6:
    case Intrinsic::cramp_vluxseg7:
    case Intrinsic::cramp_vluxseg8:
      selectVLXSEG(Node, /*IsMasked*/ false, /*IsOrdered*/ false);
      return;
    case Intrinsic::cramp_vloxseg2_mask:
    case Intrinsic::cramp_vloxseg3_mask:
    case Intrinsic::cramp_vloxseg4_mask:
    case Intrinsic::cramp_vloxseg5_mask:
    case Intrinsic::cramp_vloxseg6_mask:
    case Intrinsic::cramp_vloxseg7_mask:
    case Intrinsic::cramp_vloxseg8_mask:
      selectVLXSEG(Node, /*IsMasked*/ true, /*IsOrdered*/ true);
      return;
    case Intrinsic::cramp_vluxseg2_mask:
    case Intrinsic::cramp_vluxseg3_mask:
    case Intrinsic::cramp_vluxseg4_mask:
    case Intrinsic::cramp_vluxseg5_mask:
    case Intrinsic::cramp_vluxseg6_mask:
    case Intrinsic::cramp_vluxseg7_mask:
    case Intrinsic::cramp_vluxseg8_mask:
      selectVLXSEG(Node, /*IsMasked*/ true, /*IsOrdered*/ false);
      return;
    case Intrinsic::cramp_vlseg8ff:
    case Intrinsic::cramp_vlseg7ff:
    case Intrinsic::cramp_vlseg6ff:
    case Intrinsic::cramp_vlseg5ff:
    case Intrinsic::cramp_vlseg4ff:
    case Intrinsic::cramp_vlseg3ff:
    case Intrinsic::cramp_vlseg2ff: {
      selectVLSEGFF(Node, /*IsMasked*/ false);
      return;
    }
    case Intrinsic::cramp_vlseg8ff_mask:
    case Intrinsic::cramp_vlseg7ff_mask:
    case Intrinsic::cramp_vlseg6ff_mask:
    case Intrinsic::cramp_vlseg5ff_mask:
    case Intrinsic::cramp_vlseg4ff_mask:
    case Intrinsic::cramp_vlseg3ff_mask:
    case Intrinsic::cramp_vlseg2ff_mask: {
      selectVLSEGFF(Node, /*IsMasked*/ true);
      return;
    }
    case Intrinsic::cramp_vloxei:
    case Intrinsic::cramp_vloxei_mask:
    case Intrinsic::cramp_vluxei:
    case Intrinsic::cramp_vluxei_mask: {
      bool IsMasked = IntNo == Intrinsic::cramp_vloxei_mask ||
                      IntNo == Intrinsic::cramp_vluxei_mask;
      bool IsOrdered = IntNo == Intrinsic::cramp_vloxei ||
                       IntNo == Intrinsic::cramp_vloxei_mask;
      ...
      const Cramp::VLX_VSXPseudo *P = Cramp::getVLXPseudo(
    ...
    case Intrinsic::cramp_vlm:
    case Intrinsic::cramp_vle:
    case Intrinsic::cramp_vle_mask:
    case Intrinsic::cramp_vlse:
    case Intrinsic::cramp_vlse_mask: {
      bool IsMasked = IntNo == Intrinsic::cramp_vle_mask ||
                      IntNo == Intrinsic::cramp_vlse_mask;
      bool IsStrided =
          IntNo == Intrinsic::cramp_vlse || IntNo == Intrinsic::cramp_vlse_mask;

      MVT VT = Node->getSimpleValueType(0);
      unsigned Log2SEW = Log2_32(VT.getScalarSizeInBits());

      unsigned CurOp = 2;
      // The cramp_vlm intrinsic are always tail agnostic and no passthru operand.
      bool HasPassthruOperand = IntNo != Intrinsic::cramp_vlm;
      // Masked intrinsic only have TU version pseduo instructions.
      ...
      CrampII::VLMUL LMUL = CrampTargetLowering::getLMUL(VT);
      const Cramp::VLEPseudo *P =
          Cramp::getVLEPseudo(IsMasked, IsTU, IsStrided, /*FF*/ false, Log2SEW,
                              static_cast<unsigned>(LMUL));
      MachineSDNode *Load =
          CurDAG->getMachineNode(P->Pseudo, DL, Node->getVTList(), Operands);
    ...
    case Intrinsic::cramp_vleff:
    case Intrinsic::cramp_vleff_mask: {
      bool IsMasked = IntNo == Intrinsic::cramp_vleff_mask;
      ...
      CrampII::VLMUL LMUL = CrampTargetLowering::getLMUL(VT);
      const Cramp::VLEPseudo *P =
          Cramp::getVLEPseudo(IsMasked, IsTU, /*Strided*/ false, /*FF*/ true,
                              Log2SEW, static_cast<unsigned>(LMUL));
  ...
  case ISD::INTRINSIC_VOID: {
    unsigned IntNo = cast<ConstantSDNode>(Node->getOperand(1))->getZExtValue();
    switch (IntNo) {
    case Intrinsic::cramp_vsseg2:
    case Intrinsic::cramp_vsseg3:
    case Intrinsic::cramp_vsseg4:
    case Intrinsic::cramp_vsseg5:
    case Intrinsic::cramp_vsseg6:
    case Intrinsic::cramp_vsseg7:
    case Intrinsic::cramp_vsseg8: {
      selectVSSEG(Node, /*IsMasked*/ false, /*IsStrided*/ false);
      return;
    }
    case Intrinsic::cramp_vsseg2_mask:
    case Intrinsic::cramp_vsseg3_mask:
    case Intrinsic::cramp_vsseg4_mask:
    case Intrinsic::cramp_vsseg5_mask:
    case Intrinsic::cramp_vsseg6_mask:
    case Intrinsic::cramp_vsseg7_mask:
    case Intrinsic::cramp_vsseg8_mask: {
      selectVSSEG(Node, /*IsMasked*/ true, /*IsStrided*/ false);
      return;
    }
    case Intrinsic::cramp_vssseg2:
    case Intrinsic::cramp_vssseg3:
    case Intrinsic::cramp_vssseg4:
    case Intrinsic::cramp_vssseg5:
    case Intrinsic::cramp_vssseg6:
    case Intrinsic::cramp_vssseg7:
    case Intrinsic::cramp_vssseg8: {
      selectVSSEG(Node, /*IsMasked*/ false, /*IsStrided*/ true);
      return;
    }
    case Intrinsic::cramp_vssseg2_mask:
    case Intrinsic::cramp_vssseg3_mask:
    case Intrinsic::cramp_vssseg4_mask:
    case Intrinsic::cramp_vssseg5_mask:
    case Intrinsic::cramp_vssseg6_mask:
    case Intrinsic::cramp_vssseg7_mask:
    case Intrinsic::cramp_vssseg8_mask: {
      selectVSSEG(Node, /*IsMasked*/ true, /*IsStrided*/ true);
      return;
    }
    case Intrinsic::cramp_vsoxseg2:
    case Intrinsic::cramp_vsoxseg3:
    case Intrinsic::cramp_vsoxseg4:
    case Intrinsic::cramp_vsoxseg5:
    case Intrinsic::cramp_vsoxseg6:
    case Intrinsic::cramp_vsoxseg7:
    case Intrinsic::cramp_vsoxseg8:
      selectVSXSEG(Node, /*IsMasked*/ false, /*IsOrdered*/ true);
      return;
    case Intrinsic::cramp_vsuxseg2:
    case Intrinsic::cramp_vsuxseg3:
    case Intrinsic::cramp_vsuxseg4:
    case Intrinsic::cramp_vsuxseg5:
    case Intrinsic::cramp_vsuxseg6:
    case Intrinsic::cramp_vsuxseg7:
    case Intrinsic::cramp_vsuxseg8:
      selectVSXSEG(Node, /*IsMasked*/ false, /*IsOrdered*/ false);
      return;
    case Intrinsic::cramp_vsoxseg2_mask:
    case Intrinsic::cramp_vsoxseg3_mask:
    case Intrinsic::cramp_vsoxseg4_mask:
    case Intrinsic::cramp_vsoxseg5_mask:
    case Intrinsic::cramp_vsoxseg6_mask:
    case Intrinsic::cramp_vsoxseg7_mask:
    case Intrinsic::cramp_vsoxseg8_mask:
      selectVSXSEG(Node, /*IsMasked*/ true, /*IsOrdered*/ true);
      return;
    case Intrinsic::cramp_vsuxseg2_mask:
    case Intrinsic::cramp_vsuxseg3_mask:
    case Intrinsic::cramp_vsuxseg4_mask:
    case Intrinsic::cramp_vsuxseg5_mask:
    case Intrinsic::cramp_vsuxseg6_mask:
    case Intrinsic::cramp_vsuxseg7_mask:
    case Intrinsic::cramp_vsuxseg8_mask:
      selectVSXSEG(Node, /*IsMasked*/ true, /*IsOrdered*/ false);
      return;
    case Intrinsic::cramp_vsoxei:
    case Intrinsic::cramp_vsoxei_mask:
    case Intrinsic::cramp_vsuxei:
    case Intrinsic::cramp_vsuxei_mask: {
      bool IsMasked = IntNo == Intrinsic::cramp_vsoxei_mask ||
                      IntNo == Intrinsic::cramp_vsuxei_mask;
      bool IsOrdered = IntNo == Intrinsic::cramp_vsoxei ||
                       IntNo == Intrinsic::cramp_vsoxei_mask;
      ...
      const Cramp::VLX_VSXPseudo *P = Cramp::getVSXPseudo(
    ...
    case Intrinsic::cramp_vsm:
    case Intrinsic::cramp_vse:
    case Intrinsic::cramp_vse_mask:
    case Intrinsic::cramp_vsse:
    case Intrinsic::cramp_vsse_mask: {
      bool IsMasked = IntNo == Intrinsic::cramp_vse_mask ||
                      IntNo == Intrinsic::cramp_vsse_mask;
      bool IsStrided =
          IntNo == Intrinsic::cramp_vsse || IntNo == Intrinsic::cramp_vsse_mask;
      ...
      CrampII::VLMUL LMUL = CrampTargetLowering::getLMUL(VT);
      const Cramp::VSEPseudo *P = Cramp::getVSEPseudo(
  ...
  case ISD::INSERT_SUBVECTOR: {
    ...
    const CrampTargetLowering &TLI = *Subtarget->getTargetLowering();
    ...
    CrampII::VLMUL SubVecLMUL = CrampTargetLowering::getLMUL(SubVecContainerVT);
    ...
    if (SubRegIdx == Cramp::NoSubRegister) {
      unsigned InRegClassID = CrampTargetLowering::getRegClassIDForVecVT(VT);
      assert(CrampTargetLowering::getRegClassIDForVecVT(SubVecContainerVT) ==
                 InRegClassID &&
  ...
  case ISD::EXTRACT_SUBVECTOR: {
    ...
    if (SubRegIdx == Cramp::NoSubRegister) {
      unsigned InRegClassID = CrampTargetLowering::getRegClassIDForVecVT(InVT);
      assert(CrampTargetLowering::getRegClassIDForVecVT(SubVecContainerVT) ==
                 InRegClassID &&
             "Unexpected subvector extraction");
      SDValue RC = CurDAG->getTargetConstant(InRegClassID, DL, XLenVT);
  ...
  case ISD::SPLAT_VECTOR:
  case CrampISD::VMV_S_X_VL:
  case CrampISD::VFMV_S_F_VL:
  case CrampISD::VMV_V_X_VL:
  case CrampISD::VFMV_V_F_VL: {
    // Try to match splat of a scalar load to a strided load with stride of x0.
    bool IsScalarMove = Node->getOpcode() == CrampISD::VMV_S_X_VL ||
                        Node->getOpcode() == CrampISD::VFMV_S_F_VL;
    ...
    if (Node->getOpcode() == ISD::SPLAT_VECTOR)
      VL = CurDAG->getTargetConstant(Cramp::VLMaxSentinel, DL, XLenVT);
    ...
    SDValue Operands[] = {Ld->getBasePtr(),
                          CurDAG->getRegister(Cramp::X0, XLenVT), VL, SEW,
                          Ld->getChain()};

    CrampII::VLMUL LMUL = CrampTargetLowering::getLMUL(VT);
    const Cramp::VLEPseudo *P = Cramp::getVLEPseudo(
...
bool CrampDAGToDAGISel::SelectInlineAsmMemoryOperand(
...
bool CrampDAGToDAGISel::SelectAddrFrameIndex(SDValue Addr, SDValue &Base,
                                             SDValue &Offset) {
...
bool CrampDAGToDAGISel::SelectFrameAddrRegImm(SDValue Addr, SDValue &Base,
                                              SDValue &Offset) {
...
static bool selectConstantAddr(SelectionDAG *CurDAG, const SDLoc &DL,
                               const MVT VT, const CrampSubtarget *Subtarget,
  ...
  if (!Subtarget->is64Bit() || isInt<32>(Hi)) {
    if (Hi) {
      ...
      Base = SDValue(
          CurDAG->getMachineNode(Cramp::LUI, DL, VT,
                                 CurDAG->getTargetConstant(Hi20, DL, VT)),
          0);
    } else {
      Base = CurDAG->getRegister(Cramp::X0, VT);
    }
...
bool CrampDAGToDAGISel::SelectAddrRegImm(SDValue Addr, SDValue &Base,
                                         SDValue &Offset) {
  if (SelectAddrFrameIndex(Addr, Base, Offset))
    return true;
  ...
  if (Addr.getOpcode() == CrampISD::ADD_LO) {
  ...
  if (CurDAG->isBaseWithConstantOffset(Addr)) {
    ...
    if (isInt<12>(CVal)) {
      Base = Addr.getOperand(0);
      if (Base.getOpcode() == CrampISD::ADD_LO) {
  ...
  if (Addr.getOpcode() == ISD::ADD && isa<ConstantSDNode>(Addr.getOperand(1))) {
    ...
    // This mirrors the AddiPair PatFrag in CrampInstrInfo.td.
    if (isInt<12>(CVal / 2) && isInt<12>(CVal - CVal / 2)) {
      int64_t Adj = CVal < 0 ? -2048 : 2047;
      Base = SDValue(
          CurDAG->getMachineNode(Cramp::ADDI, DL, VT, Addr.getOperand(0),
    ...
    if (isWorthFoldingAdd(Addr) &&
      ...
      Base = SDValue(
          CurDAG->getMachineNode(Cramp::ADD, DL, VT, Addr.getOperand(0), Base),
          0);
      return true;
    }
  }
...
bool CrampDAGToDAGISel::selectShiftMask(SDValue N, unsigned ShiftWidth,
                                        SDValue &ShAmt) {
  // Shift instructions on Cramp only read the lower 5 or 6 bits of the shift
  ...
  if (N.getOpcode() == ISD::AND && isa<ConstantSDNode>(N.getOperand(1))) {
    ...
    if (Imm != 0 && Imm % ShiftWidth == 0) {
      ...
      SDValue Zero = CurDAG->getRegister(Cramp::X0, VT);
      unsigned NegOpc = VT == MVT::i64 ? Cramp::SUBW : Cramp::SUB;
...
bool CrampDAGToDAGISel::selectSExti32(SDValue N, SDValue &Val) {
...
bool CrampDAGToDAGISel::selectZExti32(SDValue N, SDValue &Val) {
...
bool CrampDAGToDAGISel::selectSHXADDOp(SDValue N, unsigned ShAmt,
                                       SDValue &Val) {
  if (N.getOpcode() == ISD::AND && isa<ConstantSDNode>(N.getOperand(1))) {
      ...
      if (isShiftedMask_64(Mask)) {
        ...
        if (LeftShift && Leading == 0 && C2 < Trailing && Trailing == ShAmt) {
          ...
          Val = SDValue(CurDAG->getMachineNode(
                            Cramp::SRLI, DL, VT, N0.getOperand(0),
                            CurDAG->getTargetConstant(Trailing - C2, DL, VT)),
                        0);
          return true;
        }
...
// opportunities.
bool CrampDAGToDAGISel::hasAllNBitUsers(SDNode *Node, unsigned Bits) const {
  assert((Node->getOpcode() == ISD::ADD || Node->getOpcode() == ISD::SUB ||
          Node->getOpcode() == ISD::MUL || Node->getOpcode() == ISD::SHL ||
          Node->getOpcode() == ISD::SRL ||
          Node->getOpcode() == ISD::SIGN_EXTEND_INREG ||
          Node->getOpcode() == CrampISD::GREV ||
          Node->getOpcode() == CrampISD::GORC ||
...
bool CrampDAGToDAGISel::selectVLOp(SDValue N, SDValue &VL) {
  ...
  if (C && isUInt<5>(C->getZExtValue())) {
    ...
    VL = CurDAG->getTargetConstant(Cramp::VLMaxSentinel, SDLoc(N),
                                   N->getValueType(0));
  } else if (isa<RegisterSDNode>(N) &&
             cast<RegisterSDNode>(N)->getReg() == Cramp::X0) {
...
bool CrampDAGToDAGISel::selectVSplat(SDValue N, SDValue &SplatVal) {
  if (N.getOpcode() != CrampISD::VMV_V_X_VL || !N.getOperand(0).isUndef())
    return false;
...
static bool selectVSplatSimmHelper(SDValue N, SDValue &SplatVal,
  ...
  // The semantics of CrampISD::VMV_V_X_VL is that when the operand
...
bool CrampDAGToDAGISel::selectVSplatSimm5(SDValue N, SDValue &SplatVal) {
  return selectVSplatSimmHelper(N, SplatVal, *CurDAG, *Subtarget,
                                [](int64_t Imm) { return isInt<5>(Imm); });
}

bool CrampDAGToDAGISel::selectVSplatSimm5Plus1(SDValue N, SDValue &SplatVal) {
  return selectVSplatSimmHelper(
      N, SplatVal, *CurDAG, *Subtarget,
      [](int64_t Imm) { return (isInt<5>(Imm) && Imm != -16) || Imm == 16; });
}

bool CrampDAGToDAGISel::selectVSplatSimm5Plus1NonZero(SDValue N,
                                                      SDValue &SplatVal) {
  return selectVSplatSimmHelper(
...
bool CrampDAGToDAGISel::selectVSplatUimm5(SDValue N, SDValue &SplatVal) {
  if (N.getOpcode() != CrampISD::VMV_V_X_VL || !N.getOperand(0).isUndef() ||
      !isa<ConstantSDNode>(N.getOperand(1)))
    return false;
...
bool CrampDAGToDAGISel::selectRVVSimm5(SDValue N, unsigned Width,
                                       SDValue &Imm) {
...
bool CrampDAGToDAGISel::doPeepholeSExtW(SDNode *N) {
  // Look for the sext.w pattern, addiw rd, rs1, 0.
  if (N->getMachineOpcode() != Cramp::ADDIW ||
  ...
  case Cramp::SLLI: {
    ...
    case Cramp::ADD:  Opc = Cramp::ADDW;  break;
    case Cramp::ADDI: Opc = Cramp::ADDIW; break;
    case Cramp::SUB:  Opc = Cramp::SUBW;  break;
    case Cramp::MUL:  Opc = Cramp::MULW;  break;
    case Cramp::SLLI: Opc = Cramp::SLLIW; break;
    }
    ...
    if (N0.getMachineOpcode() == Cramp::SLLI &&
...
bool CrampDAGToDAGISel::doPeepholeMaskedRVV(SDNode *N) {
  const Cramp::CrampMaskedPseudoInfo *I =
      Cramp::getMaskedPseudoInfo(N->getMachineOpcode());
  if (!I)
    return false;
  ...
  if (!isa<RegisterSDNode>(N->getOperand(MaskOpIdx)) ||
      cast<RegisterSDNode>(N->getOperand(MaskOpIdx))->getReg() != Cramp::V0)
    return false;
  ...
  if (!isa<RegisterSDNode>(Glued->getOperand(1)) ||
      cast<RegisterSDNode>(Glued->getOperand(1))->getReg() != Cramp::V0)
    return false;
  ...
  const auto IsVMSet = [](unsigned Opc) {
    return Opc == Cramp::PseudoVMSET_M_B1 || Opc == Cramp::PseudoVMSET_M_B16 ||
           Opc == Cramp::PseudoVMSET_M_B2 || Opc == Cramp::PseudoVMSET_M_B32 ||
           Opc == Cramp::PseudoVMSET_M_B4 || Opc == Cramp::PseudoVMSET_M_B64 ||
           Opc == Cramp::PseudoVMSET_M_B8;
  };
  ...
  bool IsTA = true;
  if (CrampII::hasVecPolicyOp(MaskedMCID.TSFlags)) {
  ...
  assert((IsTA != CrampII::hasMergeOp(TSFlags)) &&
         CrampII::hasDummyMaskOp(TSFlags) &&
         !CrampII::hasVecPolicyOp(TSFlags) &&
...
// This pass converts a legalized DAG into a Cramp-specific DAG, ready
// for instruction scheduling.
FunctionPass *llvm::createCrampISelDag(CrampTargetMachine &TM,
                                       CodeGenOpt::Level OptLevel) {
  return new CrampDAGToDAGISel(TM, OptLevel);
}
