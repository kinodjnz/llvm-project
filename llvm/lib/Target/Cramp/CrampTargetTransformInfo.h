//===- CrampTargetTransformInfo.h - RISC-V specific TTI ---------*- C++ -*-===//
//
...
#ifndef LLVM_LIB_TARGET_Cramp_CrampTARGETTRANSFORMINFO_H
#define LLVM_LIB_TARGET_Cramp_CrampTARGETTRANSFORMINFO_H
...
class CrampTTIImpl : public BasicTTIImplBase<CrampTTIImpl> {
  using BaseT = BasicTTIImplBase<CrampTTIImpl>;
  using TTI = TargetTransformInfo;

  friend BaseT;

  const CrampSubtarget *ST;
  const CrampTargetLowering *TLI;

  const CrampSubtarget *getST() const { return ST; }
  const CrampTargetLowering *getTLI() const { return TLI; }

  unsigned getMaxVLFor(VectorType *Ty);
public:
  explicit CrampTTIImpl(const CrampTargetMachine *TM, const Function &F)
  ...
  enum CrampRegisterClass { GPRRC, FPRRC, VRRC };
  unsigned getNumberOfRegisters(unsigned ClassID) const {
    switch (ClassID) {
    case CrampRegisterClass::GPRRC:
      // 31 = 32 GPR - x0 (zero register)
      // FIXME: Should we exclude fixed registers like SP, TP or GP?
      return 31;
    case CrampRegisterClass::FPRRC:
      if (ST->hasStdExtF())
        return 32;
      return 0;
    case CrampRegisterClass::VRRC:
...
#endif // LLVM_LIB_TARGET_Cramp_CrampTARGETTRANSFORMINFO_H
