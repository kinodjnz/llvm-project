//===-- CrampTargetTransformInfo.cpp - RISC-V specific TTI ----------------===//
//
...
#define DEBUG_TYPE "cramptti"

static cl::opt<unsigned> RVVRegisterWidthLMUL(
    "cramp-v-register-bit-width-lmul",
    cl::desc(
...
InstructionCost CrampTTIImpl::getIntImmCost(const APInt &Imm, Type *Ty,
  ...
  return CrampMatInt::getIntMatCost(Imm, DL.getTypeSizeInBits(Ty),
                                    getST()->getFeatureBits());
}

InstructionCost CrampTTIImpl::getIntImmCostInst(unsigned Opcode, unsigned Idx,
...
InstructionCost
CrampTTIImpl::getIntImmCostIntrin(Intrinsic::ID IID, unsigned Idx,
                                  const APInt &Imm, Type *Ty,
                                  TTI::TargetCostKind CostKind) {
...
CrampTTIImpl::getPopcntSupport(unsigned TyWidth) {
  assert(isPowerOf2_32(TyWidth) && "Ty width must be power of 2");
  return ST->hasStdExtZbb() ? TTI::PSK_FastHardware : TTI::PSK_Software;
}

bool CrampTTIImpl::shouldExpandReduction(const IntrinsicInst *II) const {
...
Optional<unsigned> CrampTTIImpl::getMaxVScale() const {
  if (ST->hasVInstructions())
    return ST->getRealMaxVLen() / Cramp::RVVBitsPerBlock;
  return BaseT::getMaxVScale();
}

Optional<unsigned> CrampTTIImpl::getVScaleForTuning() const {
  if (ST->hasVInstructions())
    return ST->getRealMinVLen() / Cramp::RVVBitsPerBlock;
  return BaseT::getVScaleForTuning();
}

TypeSize
CrampTTIImpl::getRegisterBitWidth(TargetTransformInfo::RegisterKind K) const {
  ...
  case TargetTransformInfo::RGK_ScalableVector:
    return TypeSize::getScalable(
        ST->hasVInstructions() ? LMUL * Cramp::RVVBitsPerBlock : 0);
  }

  llvm_unreachable("Unsupported register kind");
}

InstructionCost CrampTTIImpl::getSpliceCost(VectorType *Tp, int Index) {
...
InstructionCost CrampTTIImpl::getShuffleCost(TTI::ShuffleKind Kind,
...
InstructionCost
CrampTTIImpl::getMaskedMemoryOpCost(unsigned Opcode, Type *Src, Align Alignment,
...
InstructionCost CrampTTIImpl::getGatherScatterOpCost(
...
InstructionCost
CrampTTIImpl::getIntrinsicInstrCost(const IntrinsicCostAttributes &ICA,
                                    TTI::TargetCostKind CostKind) {
...
InstructionCost CrampTTIImpl::getCastInstrCost(unsigned Opcode, Type *Dst,
                                               Type *Src,
...
unsigned CrampTTIImpl::getMaxVLFor(VectorType *Ty) {
  if (isa<ScalableVectorType>(Ty)) {
    const unsigned EltSize = DL.getTypeSizeInBits(Ty->getElementType());
    const unsigned MinSize = DL.getTypeSizeInBits(Ty).getKnownMinValue();
    const unsigned VectorBitsMax = ST->getRealMaxVLen();
    return CrampTargetLowering::computeVLMAX(VectorBitsMax, EltSize, MinSize);
  }
  return cast<FixedVectorType>(Ty)->getNumElements();
}

InstructionCost
CrampTTIImpl::getMinMaxReductionCost(VectorType *Ty, VectorType *CondTy,
                                     bool IsUnsigned,
                                     TTI::TargetCostKind CostKind) {
...
InstructionCost
CrampTTIImpl::getArithmeticReductionCost(unsigned Opcode, VectorType *Ty,
...
void CrampTTIImpl::getUnrollingPreferences(Loop *L, ScalarEvolution &SE,
...
void CrampTTIImpl::getPeelingPreferences(Loop *L, ScalarEvolution &SE,
                                         TTI::PeelingPreferences &PP) {
  BaseT::getPeelingPreferences(L, SE, PP);
}

unsigned CrampTTIImpl::getRegUsageForType(Type *Ty) {
  TypeSize Size = Ty->getPrimitiveSizeInBits();
  if (Ty->isVectorTy()) {
    if (Size.isScalable() && ST->hasVInstructions())
      return divideCeil(Size.getKnownMinValue(), Cramp::RVVBitsPerBlock);
