//===-- CrampMakeCompressible.cpp - Make more instructions compressible ---===//
//
...
#define DEBUG_TYPE "cramp-make-compressible"
#define Cramp_COMPRESS_INSTRS_NAME "Cramp Make Compressible"

namespace {

struct CrampMakeCompressibleOpt : public MachineFunctionPass {
  static char ID;

  bool runOnMachineFunction(MachineFunction &Fn) override;

  CrampMakeCompressibleOpt() : MachineFunctionPass(ID) {
    initializeCrampMakeCompressibleOptPass(*PassRegistry::getPassRegistry());
  }

  StringRef getPassName() const override { return Cramp_COMPRESS_INSTRS_NAME; }
};
} // namespace

char CrampMakeCompressibleOpt::ID = 0;
INITIALIZE_PASS(CrampMakeCompressibleOpt, "cramp-make-compressible",
                Cramp_COMPRESS_INSTRS_NAME, false, false)
...
static bool isCompressedReg(Register Reg) {
  return Cramp::GPRCRegClass.contains(Reg) ||
         Cramp::FPR32CRegClass.contains(Reg) ||
         Cramp::FPR64CRegClass.contains(Reg);
}
...
static bool isCompressibleLoad(const MachineInstr &MI) {
  const CrampSubtarget &STI = MI.getMF()->getSubtarget<CrampSubtarget>();
  const unsigned Opcode = MI.getOpcode();

  return Opcode == Cramp::LW || (!STI.is64Bit() && Opcode == Cramp::FLW) ||
         Opcode == Cramp::LD || Opcode == Cramp::FLD;
}
...
static bool isCompressibleStore(const MachineInstr &MI) {
  ...
  return Opcode == Cramp::SW || (!STI.is64Bit() && Opcode == Cramp::FSW) ||
         Opcode == Cramp::SD || Opcode == Cramp::FSD;
}
...
//   {Cramp::NoRegister, 0} - No suitable optimization found for this
//   instruction.
static RegImmPair getRegImmPairPreventingCompression(const MachineInstr &MI) {
  ...
  if (isCompressibleLoad(MI) || isCompressibleStore(MI)) {
    ...
    if (!MOImm.isImm())
      return RegImmPair(Cramp::NoRegister, 0);
    ...
    if (Cramp::SPRegClass.contains(Base)) {
...
static Register analyzeCompressibleUses(MachineInstr &FirstMI,
  ...
  if (Cramp::GPRRegClass.contains(RegImm.Reg))
    RCToScavenge = &Cramp::GPRCRegClass;
  else if (Cramp::FPR32RegClass.contains(RegImm.Reg))
    RCToScavenge = &Cramp::FPR32CRegClass;
  else if (Cramp::FPR64RegClass.contains(RegImm.Reg))
    RCToScavenge = &Cramp::FPR64CRegClass;
  else
    return Cramp::NoRegister;
...
bool CrampMakeCompressibleOpt::runOnMachineFunction(MachineFunction &Fn) {
  // This is a size optimization.
  if (skipFunction(Fn.getFunction()) || !Fn.getFunction().hasMinSize())
    return false;

  const CrampSubtarget &STI = Fn.getSubtarget<CrampSubtarget>();
  ...
  for (MachineBasicBlock &MBB : Fn) {
    ...
    for (MachineInstr &MI : MBB) {
      ...
      if (Cramp::GPRRegClass.contains(RegImm.Reg)) {
        ...
        BuildMI(MBB, MI, MI.getDebugLoc(), TII.get(Cramp::ADDI), NewReg)
            .addReg(RegImm.Reg)
            .addImm(RegImm.Imm);
      } else {
        ...
        // not the FPR operand.
        assert(RegImm.Imm == 0);
        unsigned Opcode = Cramp::FPR32RegClass.contains(RegImm.Reg)
                              ? Cramp::FSGNJ_S
                              : Cramp::FSGNJ_D;
...
FunctionPass *llvm::createCrampMakeCompressibleOptPass() {
  return new CrampMakeCompressibleOpt();
}
