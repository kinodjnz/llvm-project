//===-- CrampISelLowering.cpp - Cramp DAG Lowering Implementation  --------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines the interfaces that Cramp uses to lower LLVM code into a
// selection DAG.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "cramp-lower"

STATISTIC(NumTailCalls, "Number of tail calls");

CrampTargetLowering::CrampTargetLowering(const TargetMachine &TM,
  ...
  CrampABI::ABI ABI = Subtarget.getTargetABI();
  assert(ABI != CrampABI::ABI_Unknown && "Improperly initialised target ABI");

  if ((ABI == CrampABI::ABI_ILP32F || ABI == CrampABI::ABI_LP64F) &&
      !Subtarget.hasStdExtF()) {
    errs() << "Hard-float 'f' ABI can't be used for a target that "
                "doesn't support the F instruction set extension (ignoring "
                          "target-abi)\n";
    ABI = Subtarget.is64Bit() ? CrampABI::ABI_LP64 : CrampABI::ABI_ILP32;
  } else if ((ABI == CrampABI::ABI_ILP32D || ABI == CrampABI::ABI_LP64D) &&
  ...
  addRegisterClass(XLenVT, &Cramp::GPRRegClass);

  if (Subtarget.hasStdExtZfh())
    addRegisterClass(MVT::f16, &Cramp::FPR16RegClass);
  if (Subtarget.hasStdExtF())
    addRegisterClass(MVT::f32, &Cramp::FPR32RegClass);
  if (Subtarget.hasStdExtD())
    addRegisterClass(MVT::f64, &Cramp::FPR64RegClass);
  ...
  if (Subtarget.hasVInstructions()) {
    auto addRegClassForRVV = [this](MVT VT) {
      ...
      // RVVBitsPerBlock.
      unsigned MinElts = Cramp::RVVBitsPerBlock / Subtarget.getELEN();
      if (VT.getVectorMinNumElements() < MinElts)
        return;

      unsigned Size = VT.getSizeInBits().getKnownMinValue();
      const TargetRegisterClass *RC;
      if (Size <= Cramp::RVVBitsPerBlock)
        RC = &Cramp::VRRegClass;
      else if (Size == 2 * Cramp::RVVBitsPerBlock)
        RC = &Cramp::VRM2RegClass;
      else if (Size == 4 * Cramp::RVVBitsPerBlock)
        RC = &Cramp::VRM4RegClass;
      else if (Size == 8 * Cramp::RVVBitsPerBlock)
        RC = &Cramp::VRM8RegClass;
      else
    ...
    if (Subtarget.useRVVForFixedLengthVectors()) {
      auto addRegClassForFixedVectors = [this](MVT VT) {
        ...
        const CrampRegisterInfo &TRI = *Subtarget.getRegisterInfo();
  ...
  setStackPointerRegisterToSaveRestore(Cramp::X2);
  ...
  if (Subtarget.hasVInstructions()) {
    ...
    for (MVT VT : IntVecVTs) {
      ...
      // Integer VTs are lowered as a series of "CrampISD::TRUNCATE_VECTOR_VL"
...
EVT CrampTargetLowering::getSetCCResultType(const DataLayout &DL,
                                            LLVMContext &Context,
                                            EVT VT) const {
  if (!VT.isVector())
...
MVT CrampTargetLowering::getVPExplicitVectorLengthTy() const {
  return Subtarget.getXLenVT();
}

bool CrampTargetLowering::getTgtMemIntrinsic(IntrinsicInfo &Info,
                                             const CallInst &I,
                                             MachineFunction &MF,
  ...
  default:
    return false;
  case Intrinsic::cramp_masked_atomicrmw_xchg_i32:
  case Intrinsic::cramp_masked_atomicrmw_add_i32:
  case Intrinsic::cramp_masked_atomicrmw_sub_i32:
  case Intrinsic::cramp_masked_atomicrmw_nand_i32:
  case Intrinsic::cramp_masked_atomicrmw_max_i32:
  case Intrinsic::cramp_masked_atomicrmw_min_i32:
  case Intrinsic::cramp_masked_atomicrmw_umax_i32:
  case Intrinsic::cramp_masked_atomicrmw_umin_i32:
  case Intrinsic::cramp_masked_cmpxchg_i32:
    ...
    return true;
  case Intrinsic::cramp_masked_strided_load:
    Info.opc = ISD::INTRINSIC_W_CHAIN;
    ...
    return true;
  case Intrinsic::cramp_masked_strided_store:
    ...
    return true;
  case Intrinsic::cramp_seg2_load:
  case Intrinsic::cramp_seg3_load:
  case Intrinsic::cramp_seg4_load:
  case Intrinsic::cramp_seg5_load:
  case Intrinsic::cramp_seg6_load:
  case Intrinsic::cramp_seg7_load:
  case Intrinsic::cramp_seg8_load:
    Info.opc = ISD::INTRINSIC_W_CHAIN;
    Info.ptrVal = I.getArgOperand(0);
    Info.memVT =
...
bool CrampTargetLowering::isLegalAddressingMode(const DataLayout &DL,
...
bool CrampTargetLowering::isLegalICmpImmediate(int64_t Imm) const {
  return isInt<12>(Imm);
}

bool CrampTargetLowering::isLegalAddImmediate(int64_t Imm) const {
  return isInt<12>(Imm);
}
...
// just be used.
bool CrampTargetLowering::isTruncateFree(Type *SrcTy, Type *DstTy) const {
...
bool CrampTargetLowering::isTruncateFree(EVT SrcVT, EVT DstVT) const {
...
bool CrampTargetLowering::isZExtFree(SDValue Val, EVT VT2) const {
...
bool CrampTargetLowering::isSExtCheaperThanZExt(EVT SrcVT, EVT DstVT) const {
  return Subtarget.is64Bit() && SrcVT == MVT::i32 && DstVT == MVT::i64;
}

bool CrampTargetLowering::signExtendConstant(const ConstantInt *CI) const {
  return Subtarget.is64Bit() && CI->getType()->isIntegerTy(32);
}

bool CrampTargetLowering::isCheapToSpeculateCttz() const {
  return Subtarget.hasStdExtZbb();
}

bool CrampTargetLowering::isCheapToSpeculateCtlz() const {
  return Subtarget.hasStdExtZbb();
}

bool CrampTargetLowering::hasAndNotCompare(SDValue Y) const {
...
bool CrampTargetLowering::hasBitTest(SDValue X, SDValue Y) const {
...
bool CrampTargetLowering::shouldConvertConstantLoadToIntImm(const APInt &Imm,
                                                            Type *Ty) const {
  ...
  CrampMatInt::InstSeq Seq =
      CrampMatInt::generateInstSeq(Val, Subtarget.getFeatureBits());
  return Seq.size() <= Subtarget.getMaxBuildIntsCost();
}

bool CrampTargetLowering::
...
bool CrampTargetLowering::shouldSinkOperands(
...
bool CrampTargetLowering::shouldScalarizeBinop(SDValue VecOp) const {
...
bool CrampTargetLowering::isOffsetFoldingLegal(
...
bool CrampTargetLowering::isFPImmLegal(const APFloat &Imm, EVT VT,
...
bool CrampTargetLowering::hasBitPreservingFPLogic(EVT VT) const {
  return (VT == MVT::f16 && Subtarget.hasStdExtZfh()) ||
         (VT == MVT::f32 && Subtarget.hasStdExtF()) ||
         (VT == MVT::f64 && Subtarget.hasStdExtD());
}

MVT CrampTargetLowering::getRegisterTypeForCallingConv(LLVMContext &Context,
                                                      CallingConv::ID CC,
                                                      EVT VT) const {
...
unsigned CrampTargetLowering::getNumRegistersForCallingConv(LLVMContext &Context,
                                                           CallingConv::ID CC,
                                                           EVT VT) const {
  // Use f32 to pass f16 if it is legal and Zfh is not enabled.
  // We might still end up using a GPR but that will be decided based on ABI.
...
CrampII::VLMUL CrampTargetLowering::getLMUL(MVT VT) {
  ...
  case 8:
    return CrampII::VLMUL::LMUL_F8;
  case 16:
    return CrampII::VLMUL::LMUL_F4;
  case 32:
    return CrampII::VLMUL::LMUL_F2;
  case 64:
    return CrampII::VLMUL::LMUL_1;
  case 128:
    return CrampII::VLMUL::LMUL_2;
  case 256:
    return CrampII::VLMUL::LMUL_4;
  case 512:
    return CrampII::VLMUL::LMUL_8;
  }
}

unsigned CrampTargetLowering::getRegClassIDForLMUL(CrampII::VLMUL LMul) {
  switch (LMul) {
  default:
    llvm_unreachable("Invalid LMUL.");
  ...
  case CrampII::VLMUL::LMUL_1:
    return Cramp::VRRegClassID;
  case CrampII::VLMUL::LMUL_2:
    return Cramp::VRM2RegClassID;
  case CrampII::VLMUL::LMUL_4:
    return Cramp::VRM4RegClassID;
  case CrampII::VLMUL::LMUL_8:
    return Cramp::VRM8RegClassID;
  }
}

unsigned CrampTargetLowering::getSubregIndexByMVT(MVT VT, unsigned Index) {
  CrampII::VLMUL LMUL = getLMUL(VT);
  if (LMUL == CrampII::VLMUL::LMUL_F8 ||
      LMUL == CrampII::VLMUL::LMUL_F4 ||
      LMUL == CrampII::VLMUL::LMUL_F2 ||
      LMUL == CrampII::VLMUL::LMUL_1) {
    static_assert(Cramp::sub_vrm1_7 == Cramp::sub_vrm1_0 + 7,
                  "Unexpected subreg numbering");
    return Cramp::sub_vrm1_0 + Index;
  }
  if (LMUL == CrampII::VLMUL::LMUL_2) {
    static_assert(Cramp::sub_vrm2_3 == Cramp::sub_vrm2_0 + 3,
                  "Unexpected subreg numbering");
    return Cramp::sub_vrm2_0 + Index;
  }
  if (LMUL == CrampII::VLMUL::LMUL_4) {
    static_assert(Cramp::sub_vrm4_1 == Cramp::sub_vrm4_0 + 1,
                  "Unexpected subreg numbering");
    return Cramp::sub_vrm4_0 + Index;
  }
  llvm_unreachable("Invalid vector type.");
}

unsigned CrampTargetLowering::getRegClassIDForVecVT(MVT VT) {
  if (VT.getVectorElementType() == MVT::i1)
    return Cramp::VRRegClassID;
...
CrampTargetLowering::decomposeSubvectorInsertExtractToSubRegs(
    MVT VecVT, MVT SubVecVT, unsigned InsertExtractIdx,
    const CrampRegisterInfo *TRI) {
  static_assert((Cramp::VRM8RegClassID > Cramp::VRM4RegClassID &&
                 Cramp::VRM4RegClassID > Cramp::VRM2RegClassID &&
                 Cramp::VRM2RegClassID > Cramp::VRRegClassID),
  ...
  unsigned SubRegIdx = Cramp::NoSubRegister;
  for (const unsigned RCID :
       {Cramp::VRM4RegClassID, Cramp::VRM2RegClassID, Cramp::VRRegClassID})
...
bool CrampTargetLowering::mergeStoresAfterLegalization(EVT VT) const {
  return !Subtarget.useRVVForFixedLengthVectors() ||
         (VT.isFixedLengthVector() && VT.getVectorElementType() == MVT::i1);
}

bool CrampTargetLowering::isLegalElementTypeForRVV(Type *ScalarTy) const {
...
static SDValue getVLOperand(SDValue Op) {
  ...
  const CrampVIntrinsicsTable::CrampVIntrinsicInfo *II =
      CrampVIntrinsicsTable::getCrampVIntrinsicInfo(IntNo);
  if (!II)
    return SDValue();
...
bool CrampTargetLowering::useRVVForFixedLengthVectorVT(MVT VT) const {
...
static MVT getContainerForFixedLengthVector(const TargetLowering &TLI, MVT VT,
  ...
  case MVT::f64: {
    ...
    unsigned NumElts =
        (VT.getVectorNumElements() * Cramp::RVVBitsPerBlock) / MinVLen;
    NumElts = std::max(NumElts, Cramp::RVVBitsPerBlock / MaxELen);
...
MVT CrampTargetLowering::getContainerForFixedLengthVector(MVT VT) const {
...
static SDValue getAllOnesMask(MVT VecVT, SDValue VL, SDLoc DL,
  ...
  return DAG.getNode(CrampISD::VMSET_VL, DL, MaskVT, VL);
}
...
getDefaultVLOps(MVT VecVT, MVT ContainerVT, SDLoc DL, SelectionDAG &DAG,
  ...
  SDValue VL = VecVT.isFixedLengthVector()
                   ...
                   : DAG.getRegister(Cramp::X0, XLenVT);
...
bool CrampTargetLowering::shouldExpandBuildVectorWithShuffles(
...
static SDValue lowerFP_TO_INT_SAT(SDValue Op, SelectionDAG &DAG,
                                  const CrampSubtarget &Subtarget) {
  // Cramp FP-to-int conversions saturate to the destination register size, but
  ...
  unsigned Opc;
  if (SatVT == DstVT)
    Opc = IsSigned ? CrampISD::FCVT_X : CrampISD::FCVT_XU;
  else if (DstVT == MVT::i64 && SatVT == MVT::i32)
    Opc = IsSigned ? CrampISD::FCVT_W_RV64 : CrampISD::FCVT_WU_RV64;
  else
    return SDValue();
  ...
  SDValue FpToInt = DAG.getNode(
      Opc, DL, DstVT, Src,
      DAG.getTargetConstant(CrampFPRndMode::RTZ, DL, Subtarget.getXLenVT()));
...
// This mode isn't supported in vector hardware on Cramp. But as long as we
...
static SDValue matchSplatAsGather(SDValue SplatVal, MVT VT, const SDLoc &DL,
  ...
  SDValue Gather = DAG.getNode(CrampISD::VRGATHER_VX_VL, DL, ContainerVT, Vec,
...
static SDValue lowerBUILD_VECTOR(SDValue Op, SelectionDAG &DAG,
  ...
  if (VT.getVectorElementType() == MVT::i1) {
    if (ISD::isBuildVectorAllZeros(Op.getNode())) {
      SDValue VMClr = DAG.getNode(CrampISD::VMCLR_VL, DL, ContainerVT, VL);
      return convertFromScalableVector(VT, VMClr, DAG, Subtarget);
    }

    if (ISD::isBuildVectorAllOnes(Op.getNode())) {
      SDValue VMSet = DAG.getNode(CrampISD::VMSET_VL, DL, ContainerVT, VL);
  ...
  if (SDValue Splat = cast<BuildVectorSDNode>(Op)->getSplatValue()) {
    if (auto Gather = matchSplatAsGather(Splat, VT, DL, DAG, Subtarget))
      return Gather;
    unsigned Opc = VT.isFloatingPoint() ? CrampISD::VFMV_V_F_VL
                                        : CrampISD::VMV_V_X_VL;
    Splat =
  ...
  if (auto SimpleVID = isSimpleVIDSequence(Op)) {
    ...
    if (((StepOpcode == ISD::MUL && isInt<12>(SplatStepVal)) ||
      ...
      SDValue VID = DAG.getNode(CrampISD::VID_VL, DL, ContainerVT, Mask, VL);
  ...
  if (VT.isInteger() && EltBitSize < 64 &&
    ...
    if (ViaIntVT.bitsLE(XLenVT) || isInt<32>(SplatValue)) {
      ...
      SDValue Splat =
          DAG.getNode(CrampISD::VMV_V_X_VL, DL, ViaContainerVT,
...
static SDValue splatPartsI64WithVL(const SDLoc &DL, MVT VT, SDValue Passthru,
  ...
  if (isa<ConstantSDNode>(Lo) && isa<ConstantSDNode>(Hi)) {
    ...
    if ((LoC >> 31) == HiC)
      return DAG.getNode(CrampISD::VMV_V_X_VL, DL, VT, Passthru, Lo, VL);
    ...
    if (LoC == HiC && Const && Const->isAllOnesValue()) {
      ...
      auto InterVec = DAG.getNode(
          CrampISD::VMV_V_X_VL, DL, InterVT, DAG.getUNDEF(InterVT), Lo,
                                  DAG.getRegister(Cramp::X0, MVT::i32));
      return DAG.getNode(ISD::BITCAST, DL, VT, InterVec);
    }
  }

  // Fall back to a stack store and stride x0 vector load.
  return DAG.getNode(CrampISD::SPLAT_VECTOR_SPLIT_I64_VL, DL, VT, Passthru, Lo,
                     Hi, VL);
}
...
static SDValue lowerScalarSplat(SDValue Passthru, SDValue Scalar, SDValue VL,
  ...
  if (VT.isFloatingPoint()) {
    ...
    if (isOneConstant(VL))
      return DAG.getNode(CrampISD::VFMV_S_F_VL, DL, VT, Passthru, Scalar, VL);
    return DAG.getNode(CrampISD::VFMV_V_F_VL, DL, VT, Passthru, Scalar, VL);
  }
  ...
  if (Scalar.getValueType().bitsLE(XLenVT)) {
    ...
    if (isOneConstant(VL) &&
      ...
      return DAG.getNode(CrampISD::VMV_S_X_VL, DL, VT, Passthru, Scalar, VL);
    return DAG.getNode(CrampISD::VMV_V_X_VL, DL, VT, Passthru, Scalar, VL);
  }
  ...
  if (isOneConstant(VL) && isNullConstant(Scalar))
    return DAG.getNode(CrampISD::VMV_S_X_VL, DL, VT, Passthru,
...
static SDValue lowerVECTOR_SHUFFLE(SDValue Op, SelectionDAG &DAG,
    ...
    if (Lane >= 0) {
      ...
      if (ISD::isNormalLoad(V.getNode()) && cast<LoadSDNode>(V)->isSimple()) {
        ...
        if (SVT.isInteger() && SVT.bitsGT(XLenVT)) {
          ...
          SDValue IntID =
              DAG.getTargetConstant(Intrinsic::cramp_vlse, DL, XLenVT);
        ...
        unsigned Opc =
            VT.isFloatingPoint() ? CrampISD::VFMV_V_F_VL : CrampISD::VMV_V_X_VL;
        SDValue Splat =
      ...
      SDValue Gather = DAG.getNode(CrampISD::VRGATHER_VX_VL, DL, ContainerVT,
  ...
  if (Rotation > 0) {
    ...
    if (HiV) {
      ...
      Res =
          DAG.getNode(CrampISD::VSLIDEDOWN_VL, DL, ContainerVT, Res, HiV,
                      DAG.getConstant(Rotation, DL, XLenVT), TrueMask, DownVL);
    }
    if (LoV)
      Res = DAG.getNode(CrampISD::VSLIDEUP_VL, DL, ContainerVT, Res, LoV,
  ...
  if (isInterleaveShuffle(Mask, VT, SwapSources, Subtarget)) {
    ...
    SDValue Add = DAG.getNode(CrampISD::VWADDU_VL, DL, WideIntContainerVT, V1,
                              V2, TrueMask, VL);
    // Create 2^eltbits - 1 copies of V2 by multiplying by the largest integer.
    SDValue Multiplier = DAG.getNode(CrampISD::VMV_V_X_VL, DL, IntHalfVT,
                                     DAG.getUNDEF(IntHalfVT),
                                     DAG.getAllOnesConstant(DL, XLenVT));
    SDValue WidenMul = DAG.getNode(CrampISD::VWMULU_VL, DL, WideIntContainerVT,
    ...
    Add = DAG.getNode(CrampISD::ADD_VL, DL, WideIntContainerVT, Add, WidenMul,
                      TrueMask, VL);
  ...
  unsigned GatherVXOpc = CrampISD::VRGATHER_VX_VL;
  unsigned GatherVVOpc = CrampISD::VRGATHER_VV_VL;
  ...
  if (IndexVT.getScalarType().bitsGT(XLenVT)) {
    GatherVVOpc = CrampISD::VRGATHEREI16_VV_VL;
...
bool CrampTargetLowering::isShuffleMaskLegal(ArrayRef<int> M, EVT VT) const {
...
static SDValue lowerCTLZ_CTTZ_ZERO_UNDEF(SDValue Op, SelectionDAG &DAG) {
  ...
  // Legal types should have been checked in the CrampTargetLowering
  // constructor.
...
SDValue CrampTargetLowering::expandUnalignedRVVLoad(SDValue Op,
                                                    SelectionDAG &DAG) const {
...
SDValue CrampTargetLowering::expandUnalignedRVVStore(SDValue Op,
                                                     SelectionDAG &DAG) const {
...
static SDValue lowerConstant(SDValue Op, SelectionDAG &DAG,
  ...
  CrampMatInt::InstSeq Seq =
      CrampMatInt::generateInstSeq(Imm, Subtarget.getFeatureBits());
...
SDValue CrampTargetLowering::LowerOperation(SDValue Op,
                                            SelectionDAG &DAG) const {
  switch (Op.getOpcode()) {
  default:
  ...
  case ISD::BITCAST: {
    ...
    if (VT == MVT::f16 && Op0VT == MVT::i16 && Subtarget.hasStdExtZfh()) {
      ...
      SDValue FPConv = DAG.getNode(CrampISD::FMV_H_X, DL, MVT::f16, NewOp0);
      return FPConv;
    }
    if (VT == MVT::f32 && Op0VT == MVT::i32 && Subtarget.is64Bit() &&
      ...
      SDValue FPConv =
          DAG.getNode(CrampISD::FMV_W_X_RV64, DL, MVT::f32, NewOp0);
      return FPConv;
    }
  ...
  case ISD::BITREVERSE: {
    ...
    if (Subtarget.hasStdExtZbp()) {
      ...
      if (Op.getOpcode() == ISD::BSWAP)
        Imm &= ~0x7U;
      return DAG.getNode(CrampISD::GREV, DL, VT, Op.getOperand(0),
    ...
    return DAG.getNode(CrampISD::GREV, DL, VT, BSwap,
  ...
  case ISD::ZERO_EXTEND:
    ...
    return lowerFixedLengthVectorExtendToRVV(Op, DAG, CrampISD::VZEXT_VL);
  case ISD::SIGN_EXTEND:
    if (Op.getOperand(0).getValueType().isVector() &&
        Op.getOperand(0).getValueType().getVectorElementType() == MVT::i1)
      return lowerVectorMaskExt(Op, DAG, /*ExtVal*/ -1);
    return lowerFixedLengthVectorExtendToRVV(Op, DAG, CrampISD::VSEXT_VL);
  ...
  case ISD::VSCALE: {
    MVT VT = Op.getSimpleValueType();
    SDLoc DL(Op);
    SDValue VLENB = DAG.getNode(CrampISD::READ_VLENB, DL, VT);
    // We define our scalable vector types for lmul=1 to use a 64 bit known
    // minimum size. e.g. <vscale x 2 x i32>. VLENB is in bytes so we calculate
    // vscale as VLENB / 8.
    static_assert(Cramp::RVVBitsPerBlock == 64, "Unexpected bits per block!");
    if (Subtarget.getRealMinVLen() < Cramp::RVVBitsPerBlock)
  ...
  case ISD::UINT_TO_FP: {
    ...
    case ISD::FP_TO_SINT:
      RVVOpc = CrampISD::FP_TO_SINT_VL;
      break;
    case ISD::FP_TO_UINT:
      RVVOpc = CrampISD::FP_TO_UINT_VL;
      break;
    case ISD::SINT_TO_FP:
      RVVOpc = CrampISD::SINT_TO_FP_VL;
      break;
    case ISD::UINT_TO_FP:
      RVVOpc = CrampISD::UINT_TO_FP_VL;
      break;
    }
  ...
  case ISD::ADD:
    return lowerToScalableOp(Op, DAG, CrampISD::ADD_VL);
  case ISD::SUB:
    return lowerToScalableOp(Op, DAG, CrampISD::SUB_VL);
  case ISD::MUL:
    return lowerToScalableOp(Op, DAG, CrampISD::MUL_VL);
  case ISD::MULHS:
    return lowerToScalableOp(Op, DAG, CrampISD::MULHS_VL);
  case ISD::MULHU:
    return lowerToScalableOp(Op, DAG, CrampISD::MULHU_VL);
  case ISD::AND:
    return lowerFixedLengthVectorLogicOpToRVV(Op, DAG, CrampISD::VMAND_VL,
                                              CrampISD::AND_VL);
  case ISD::OR:
    return lowerFixedLengthVectorLogicOpToRVV(Op, DAG, CrampISD::VMOR_VL,
                                              CrampISD::OR_VL);
  case ISD::XOR:
    return lowerFixedLengthVectorLogicOpToRVV(Op, DAG, CrampISD::VMXOR_VL,
                                              CrampISD::XOR_VL);
  case ISD::SDIV:
    return lowerToScalableOp(Op, DAG, CrampISD::SDIV_VL);
  case ISD::SREM:
    return lowerToScalableOp(Op, DAG, CrampISD::SREM_VL);
  case ISD::UDIV:
    return lowerToScalableOp(Op, DAG, CrampISD::UDIV_VL);
  case ISD::UREM:
    return lowerToScalableOp(Op, DAG, CrampISD::UREM_VL);
  case ISD::SHL:
  case ISD::SRA:
  case ISD::SRL:
    ...
    return SDValue();
  case ISD::SADDSAT:
    return lowerToScalableOp(Op, DAG, CrampISD::SADDSAT_VL);
  case ISD::UADDSAT:
    return lowerToScalableOp(Op, DAG, CrampISD::UADDSAT_VL);
  case ISD::SSUBSAT:
    return lowerToScalableOp(Op, DAG, CrampISD::SSUBSAT_VL);
  case ISD::USUBSAT:
    return lowerToScalableOp(Op, DAG, CrampISD::USUBSAT_VL);
  case ISD::FADD:
    return lowerToScalableOp(Op, DAG, CrampISD::FADD_VL);
  case ISD::FSUB:
    return lowerToScalableOp(Op, DAG, CrampISD::FSUB_VL);
  case ISD::FMUL:
    return lowerToScalableOp(Op, DAG, CrampISD::FMUL_VL);
  case ISD::FDIV:
    return lowerToScalableOp(Op, DAG, CrampISD::FDIV_VL);
  case ISD::FNEG:
    return lowerToScalableOp(Op, DAG, CrampISD::FNEG_VL);
  case ISD::FABS:
    return lowerToScalableOp(Op, DAG, CrampISD::FABS_VL);
  case ISD::FSQRT:
    return lowerToScalableOp(Op, DAG, CrampISD::FSQRT_VL);
  case ISD::FMA:
    return lowerToScalableOp(Op, DAG, CrampISD::VFMADD_VL);
  case ISD::SMIN:
    return lowerToScalableOp(Op, DAG, CrampISD::SMIN_VL);
  case ISD::SMAX:
    return lowerToScalableOp(Op, DAG, CrampISD::SMAX_VL);
  case ISD::UMIN:
    return lowerToScalableOp(Op, DAG, CrampISD::UMIN_VL);
  case ISD::UMAX:
    return lowerToScalableOp(Op, DAG, CrampISD::UMAX_VL);
  case ISD::FMINNUM:
    return lowerToScalableOp(Op, DAG, CrampISD::FMINNUM_VL);
  case ISD::FMAXNUM:
    return lowerToScalableOp(Op, DAG, CrampISD::FMAXNUM_VL);
  case ISD::ABS:
  ...
  case ISD::VP_SELECT:
    return lowerVPOp(Op, DAG, CrampISD::VSELECT_VL);
  case ISD::VP_MERGE:
    return lowerVPOp(Op, DAG, CrampISD::VP_MERGE_VL);
  case ISD::VP_ADD:
    return lowerVPOp(Op, DAG, CrampISD::ADD_VL);
  case ISD::VP_SUB:
    return lowerVPOp(Op, DAG, CrampISD::SUB_VL);
  case ISD::VP_MUL:
    return lowerVPOp(Op, DAG, CrampISD::MUL_VL);
  case ISD::VP_SDIV:
    return lowerVPOp(Op, DAG, CrampISD::SDIV_VL);
  case ISD::VP_UDIV:
    return lowerVPOp(Op, DAG, CrampISD::UDIV_VL);
  case ISD::VP_SREM:
    return lowerVPOp(Op, DAG, CrampISD::SREM_VL);
  case ISD::VP_UREM:
    return lowerVPOp(Op, DAG, CrampISD::UREM_VL);
  case ISD::VP_AND:
    return lowerLogicVPOp(Op, DAG, CrampISD::VMAND_VL, CrampISD::AND_VL);
  case ISD::VP_OR:
    return lowerLogicVPOp(Op, DAG, CrampISD::VMOR_VL, CrampISD::OR_VL);
  case ISD::VP_XOR:
    return lowerLogicVPOp(Op, DAG, CrampISD::VMXOR_VL, CrampISD::XOR_VL);
  case ISD::VP_ASHR:
    return lowerVPOp(Op, DAG, CrampISD::SRA_VL);
  case ISD::VP_LSHR:
    return lowerVPOp(Op, DAG, CrampISD::SRL_VL);
  case ISD::VP_SHL:
    return lowerVPOp(Op, DAG, CrampISD::SHL_VL);
  case ISD::VP_FADD:
    return lowerVPOp(Op, DAG, CrampISD::FADD_VL);
  case ISD::VP_FSUB:
    return lowerVPOp(Op, DAG, CrampISD::FSUB_VL);
  case ISD::VP_FMUL:
    return lowerVPOp(Op, DAG, CrampISD::FMUL_VL);
  case ISD::VP_FDIV:
    return lowerVPOp(Op, DAG, CrampISD::FDIV_VL);
  case ISD::VP_FNEG:
    return lowerVPOp(Op, DAG, CrampISD::FNEG_VL);
  case ISD::VP_FMA:
    return lowerVPOp(Op, DAG, CrampISD::VFMADD_VL);
  ...
  case ISD::VP_FP_ROUND:
    return lowerVectorFPExtendOrRoundLike(Op, DAG);
  case ISD::VP_FPTOSI:
    return lowerVPFPIntConvOp(Op, DAG, CrampISD::FP_TO_SINT_VL);
  case ISD::VP_FPTOUI:
    return lowerVPFPIntConvOp(Op, DAG, CrampISD::FP_TO_UINT_VL);
  case ISD::VP_SITOFP:
    return lowerVPFPIntConvOp(Op, DAG, CrampISD::SINT_TO_FP_VL);
  case ISD::VP_UITOFP:
    return lowerVPFPIntConvOp(Op, DAG, CrampISD::UINT_TO_FP_VL);
  case ISD::VP_SETCC:
    if (Op.getOperand(0).getSimpleValueType().getVectorElementType() == MVT::i1)
      return lowerVPSetCCMaskOp(Op, DAG);
    return lowerVPOp(Op, DAG, CrampISD::SETCC_VL);
  }
}
...
SDValue CrampTargetLowering::getAddr(NodeTy *N, SelectionDAG &DAG,
                                     bool IsLocal) const {
  SDLoc DL(N);
  ...
  if (isPositionIndependent()) {
    ...
    if (IsLocal)
      ...
      // %pcrel_lo(auipc)).
      return DAG.getNode(CrampISD::LLA, DL, Ty, Addr);
    ...
    SDValue Load =
        DAG.getMemIntrinsicNode(CrampISD::LA, DL, DAG.getVTList(Ty, MVT::Other),
  ...
  case CodeModel::Small: {
    ...
    SDValue AddrHi = getTargetNode(N, DL, Ty, DAG, CrampII::MO_HI);
    SDValue AddrLo = getTargetNode(N, DL, Ty, DAG, CrampII::MO_LO);
    SDValue MNHi = DAG.getNode(CrampISD::HI, DL, Ty, AddrHi);
    return DAG.getNode(CrampISD::ADD_LO, DL, Ty, MNHi, AddrLo);
  }
...
SDValue CrampTargetLowering::lowerGlobalAddress(SDValue Op,
                                                SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
SDValue CrampTargetLowering::lowerBlockAddress(SDValue Op,
                                               SelectionDAG &DAG) const {
...
SDValue CrampTargetLowering::lowerConstantPool(SDValue Op,
                                               SelectionDAG &DAG) const {
...
SDValue CrampTargetLowering::lowerJumpTable(SDValue Op,
                                            SelectionDAG &DAG) const {
...
SDValue CrampTargetLowering::getStaticTLSAddr(GlobalAddressSDNode *N,
                                              SelectionDAG &DAG,
                                              bool UseGOT) const {
  SDLoc DL(N);
  EVT Ty = getPointerTy(DAG.getDataLayout());
  ...
  if (UseGOT) {
    ...
    SDValue Load = DAG.getMemIntrinsicNode(
        CrampISD::LA_TLS_IE, DL, DAG.getVTList(Ty, MVT::Other),
        {DAG.getEntryNode(), Addr}, Ty, MemOp);

    // Add the thread pointer.
    SDValue TPReg = DAG.getRegister(Cramp::X4, XLenVT);
  ...
  SDValue AddrHi =
      DAG.getTargetGlobalAddress(GV, DL, Ty, 0, CrampII::MO_TPREL_HI);
  SDValue AddrAdd =
      DAG.getTargetGlobalAddress(GV, DL, Ty, 0, CrampII::MO_TPREL_ADD);
  SDValue AddrLo =
      DAG.getTargetGlobalAddress(GV, DL, Ty, 0, CrampII::MO_TPREL_LO);

  SDValue MNHi = DAG.getNode(CrampISD::HI, DL, Ty, AddrHi);
  SDValue TPReg = DAG.getRegister(Cramp::X4, XLenVT);
  SDValue MNAdd =
      DAG.getNode(CrampISD::ADD_TPREL, DL, Ty, MNHi, TPReg, AddrAdd);
  return DAG.getNode(CrampISD::ADD_LO, DL, Ty, MNAdd, AddrLo);
}

SDValue CrampTargetLowering::getDynamicTLSAddr(GlobalAddressSDNode *N,
                                               SelectionDAG &DAG) const {
  SDLoc DL(N);
  EVT Ty = getPointerTy(DAG.getDataLayout());
  ...
  SDValue Addr = DAG.getTargetGlobalAddress(GV, DL, Ty, 0, 0);
  SDValue Load = DAG.getNode(CrampISD::LA_TLS_GD, DL, Ty, Addr);
...
SDValue CrampTargetLowering::lowerGlobalTLSAddress(SDValue Op,
                                                   SelectionDAG &DAG) const {
  SDLoc DL(Op);
  GlobalAddressSDNode *N = cast<GlobalAddressSDNode>(Op);
  assert(N->getOffset() == 0 && "unexpected offset in global node");
...
SDValue CrampTargetLowering::lowerSELECT(SDValue Op, SelectionDAG &DAG) const {
  ...
  // lowered CrampISD::SELECT_CC to take advantage of the integer
  // compare+branch instructions. i.e.:
  // (select (setcc lhs, rhs, cc), truev, falsev)
  // -> (crampisd::select_cc lhs, rhs, cc, truev, falsev)
  if (VT == XLenVT && CondV.getOpcode() == ISD::SETCC &&
    ...
    return DAG.getNode(CrampISD::SELECT_CC, DL, Op.getValueType(), Ops);
  }

  // Otherwise:
  // (select condv, truev, falsev)
  // -> (crampisd::select_cc condv, zero, setne, truev, falsev)
...
SDValue CrampTargetLowering::lowerBRCOND(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (CondV.getOpcode() == ISD::SETCC &&
    ...
    SDValue TargetCC = DAG.getCondCode(CCVal);
    return DAG.getNode(CrampISD::BR_CC, DL, Op.getValueType(), Op.getOperand(0),
...
SDValue CrampTargetLowering::lowerVASTART(SDValue Op, SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  CrampMachineFunctionInfo *FuncInfo = MF.getInfo<CrampMachineFunctionInfo>();
...
SDValue CrampTargetLowering::lowerFRAMEADDR(SDValue Op,
                                            SelectionDAG &DAG) const {
  const CrampRegisterInfo &RI = *Subtarget.getRegisterInfo();
  MachineFunction &MF = DAG.getMachineFunction();
...
SDValue CrampTargetLowering::lowerRETURNADDR(SDValue Op,
                                             SelectionDAG &DAG) const {
  const CrampRegisterInfo &RI = *Subtarget.getRegisterInfo();
  MachineFunction &MF = DAG.getMachineFunction();
  MachineFrameInfo &MFI = MF.getFrameInfo();
...
SDValue CrampTargetLowering::lowerShiftLeftParts(SDValue Op,
                                                 SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
SDValue CrampTargetLowering::lowerShiftRightParts(SDValue Op, SelectionDAG &DAG,
                                                  bool IsSRA) const {
  SDLoc DL(Op);
  SDValue Lo = Op.getOperand(0);
  SDValue Hi = Op.getOperand(1);
...
SDValue CrampTargetLowering::lowerVectorMaskSplat(SDValue Op,
                                                  SelectionDAG &DAG) const {
  SDLoc DL(Op);
  MVT VT = Op.getSimpleValueType();
  ...
  if (ISD::isConstantSplatVectorAllOnes(Op.getNode())) {
    ...
    return DAG.getNode(CrampISD::VMSET_VL, DL, VT, VL);
  }
  if (ISD::isConstantSplatVectorAllZeros(Op.getNode())) {
    SDValue VL = getDefaultScalableVLOps(VT, DL, DAG, Subtarget).second;
    return DAG.getNode(CrampISD::VMCLR_VL, DL, VT, VL);
  }
  MVT XLenVT = Subtarget.getXLenVT();
...
// them to VMV_V_X_VL.
SDValue CrampTargetLowering::lowerSPLAT_VECTOR_PARTS(SDValue Op,
                                                     SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ...
  if (isa<ConstantSDNode>(Lo) && isa<ConstantSDNode>(Hi)) {
    ...
    // node in order to try and match RVV vector/scalar instructions.
    if ((LoC >> 31) == HiC)
      return DAG.getNode(CrampISD::VMV_V_X_VL, DL, VecVT, DAG.getUNDEF(VecVT),
                         Lo, DAG.getRegister(Cramp::X0, MVT::i32));
  }
  ...
  if (Hi.getOpcode() == ISD::SRA && Hi.getOperand(0) == Lo &&
    ...
    return DAG.getNode(CrampISD::VMV_V_X_VL, DL, VecVT, DAG.getUNDEF(VecVT), Lo,
                       DAG.getRegister(Cramp::X0, MVT::i32));

  // Fall back to use a stack store and stride x0 vector load. Use X0 as VL.
  return DAG.getNode(CrampISD::SPLAT_VECTOR_SPLIT_I64_VL, DL, VecVT,
...
SDValue CrampTargetLowering::lowerVectorMaskExt(SDValue Op, SelectionDAG &DAG,
  ...
  SplatZero = DAG.getNode(CrampISD::VMV_V_X_VL, DL, ContainerVT,
                          DAG.getUNDEF(ContainerVT), SplatZero, VL);
  SplatTrueVal = DAG.getNode(CrampISD::VMV_V_X_VL, DL, ContainerVT,
                             DAG.getUNDEF(ContainerVT), SplatTrueVal, VL);
  SDValue Select = DAG.getNode(CrampISD::VSELECT_VL, DL, ContainerVT, CC,
                               SplatTrueVal, SplatZero, VL);

  return convertFromScalableVector(VecVT, Select, DAG, Subtarget);
}

SDValue CrampTargetLowering::lowerFixedLengthVectorExtendToRVV(
...
SDValue CrampTargetLowering::lowerVectorMaskTruncLike(SDValue Op,
                                                      SelectionDAG &DAG) const {
  ...
  SplatOne = DAG.getNode(CrampISD::VMV_V_X_VL, DL, ContainerVT,
  ...
  MVT MaskContainerVT = ContainerVT.changeVectorElementType(MVT::i1);
  SDValue Trunc =
      DAG.getNode(CrampISD::AND_VL, DL, ContainerVT, Src, SplatOne, Mask, VL);
  Trunc = DAG.getNode(CrampISD::SETCC_VL, DL, MaskContainerVT, Trunc, SplatZero,
...
SDValue CrampTargetLowering::lowerVectorTruncLike(SDValue Op,
                                                  SelectionDAG &DAG) const {
  bool IsVPTrunc = Op.getOpcode() == ISD::VP_TRUNCATE;
  SDLoc DL(Op);
  ...
  // truncates as a series of "CrampISD::TRUNCATE_VECTOR_VL" nodes which
  ...
  do {
    ...
    Result = DAG.getNode(CrampISD::TRUNCATE_VECTOR_VL, DL, ResultVT, Result,
                         Mask, VL);
...
SDValue
CrampTargetLowering::lowerVectorFPExtendOrRoundLike(SDValue Op,
                                                    SelectionDAG &DAG) const {
  bool IsVP =
  ...
  unsigned ConvOpc = IsExtend ? CrampISD::FP_EXTEND_VL : CrampISD::FP_ROUND_VL;
  ...
  unsigned InterConvOpc =
      IsExtend ? CrampISD::FP_EXTEND_VL : CrampISD::VFNCVT_ROD_VL;
...
SDValue CrampTargetLowering::lowerINSERT_VECTOR_ELT(SDValue Op,
                                                    SelectionDAG &DAG) const {
  SDLoc DL(Op);
  MVT VecVT = Op.getSimpleValueType();
  ...
  if (IsLegalInsert) {
    unsigned Opc =
        VecVT.isFloatingPoint() ? CrampISD::VFMV_S_F_VL : CrampISD::VMV_S_X_VL;
    ...
    ValInVec = DAG.getNode(CrampISD::VMV_V_X_VL, DL, I32ContainerVT,
                           DAG.getUNDEF(I32ContainerVT), Zero, InsertI64VL);
    // First slide in the hi value, then the lo in underneath it.
    ValInVec = DAG.getNode(CrampISD::VSLIDE1UP_VL, DL, I32ContainerVT,
  ...
  SDValue Slideup = DAG.getNode(CrampISD::VSLIDEUP_VL, DL, ContainerVT, Vec,
...
SDValue CrampTargetLowering::lowerEXTRACT_VECTOR_ELT(SDValue Op,
                                                     SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ...
  if (!isNullConstant(Idx)) {
    ...
    SDValue Mask = getAllOnesMask(ContainerVT, VL, DL, DAG);
    Vec = DAG.getNode(CrampISD::VSLIDEDOWN_VL, DL, ContainerVT,
  ...
  SDValue Elt0 = DAG.getNode(CrampISD::VMV_X_S, DL, XLenVT, Vec);
...
static SDValue lowerVectorIntrinsicScalars(SDValue Op, SelectionDAG &DAG,
  ...
  switch (IntNo) {
  case Intrinsic::cramp_vslide1up:
  case Intrinsic::cramp_vslide1down:
  case Intrinsic::cramp_vslide1up_mask:
  case Intrinsic::cramp_vslide1down_mask: {
    ...
    if (isa<ConstantSDNode>(AVL)) {
      ...
      unsigned MaxVLMAX =
          CrampTargetLowering::computeVLMAX(VectorBitsMax, EltSize, MinSize);

      unsigned VectorBitsMin = Subtarget.getRealMinVLen();
      unsigned MinVLMAX =
          CrampTargetLowering::computeVLMAX(VectorBitsMin, EltSize, MinSize);
      ...
      if (AVLInt <= MinVLMAX) {
        ...
        CrampII::VLMUL Lmul = CrampTargetLowering::getLMUL(I32VT);
        SDValue LMUL = DAG.getConstant(Lmul, DL, XLenVT);
        unsigned Sew = CrampVType::encodeSEW(I32VT.getScalarSizeInBits());
        SDValue SEW = DAG.getConstant(Sew, DL, XLenVT);
        SDValue SETVLMAX = DAG.getTargetConstant(
            Intrinsic::cramp_vsetvlimax_opt, DL, MVT::i32);
    ...
    if (!I32VL) {
      CrampII::VLMUL Lmul = CrampTargetLowering::getLMUL(VT);
      SDValue LMUL = DAG.getConstant(Lmul, DL, XLenVT);
      unsigned Sew = CrampVType::encodeSEW(VT.getScalarSizeInBits());
      SDValue SEW = DAG.getConstant(Sew, DL, XLenVT);
      SDValue SETVL =
          DAG.getTargetConstant(Intrinsic::cramp_vsetvli_opt, DL, MVT::i32);
    ...
    if (IntNo == Intrinsic::cramp_vslide1up ||
        IntNo == Intrinsic::cramp_vslide1up_mask) {
      Vec = DAG.getNode(CrampISD::VSLIDE1UP_VL, DL, I32VT, Passthru, Vec,
                        ScalarHi, I32Mask, I32VL);
      Vec = DAG.getNode(CrampISD::VSLIDE1UP_VL, DL, I32VT, Passthru, Vec,
                        ScalarLo, I32Mask, I32VL);
    } else {
      Vec = DAG.getNode(CrampISD::VSLIDE1DOWN_VL, DL, I32VT, Passthru, Vec,
                        ScalarLo, I32Mask, I32VL);
      Vec = DAG.getNode(CrampISD::VSLIDE1DOWN_VL, DL, I32VT, Passthru, Vec,
                        ScalarHi, I32Mask, I32VL);
    }
    ...
    if (MaskedOff.isUndef())
      return Vec;
    // TAMU
    if (Policy == CrampII::TAIL_AGNOSTIC)
      return DAG.getNode(CrampISD::VSELECT_VL, DL, VT, Mask, Vec, MaskedOff,
                         AVL);
    // TUMA or TUMU: Currently we always emit tumu policy regardless of tuma.
    // It's fine because vmerge does not care mask policy.
    return DAG.getNode(CrampISD::VP_MERGE_VL, DL, VT, Mask, Vec, MaskedOff,
                       AVL);
  }
  }
...
SDValue CrampTargetLowering::LowerINTRINSIC_WO_CHAIN(SDValue Op,
                                                     SelectionDAG &DAG) const {
  ...
  case Intrinsic::thread_pointer: {
    ...
    return DAG.getRegister(Cramp::X4, PtrVT);
  }
  case Intrinsic::cramp_orc_b:
  case Intrinsic::cramp_brev8: {
    // Lower to the GORCI encoding for orc.b or the GREVI encoding for brev8.
    unsigned Opc =
        IntNo == Intrinsic::cramp_brev8 ? CrampISD::GREV : CrampISD::GORC;
    return DAG.getNode(Opc, DL, XLenVT, Op.getOperand(1),
                       DAG.getConstant(7, DL, XLenVT));
  }
  case Intrinsic::cramp_grev:
  case Intrinsic::cramp_gorc: {
    unsigned Opc =
        IntNo == Intrinsic::cramp_grev ? CrampISD::GREV : CrampISD::GORC;
    return DAG.getNode(Opc, DL, XLenVT, Op.getOperand(1), Op.getOperand(2));
  }
  case Intrinsic::cramp_zip:
  case Intrinsic::cramp_unzip: {
    ...
    unsigned Opc =
        IntNo == Intrinsic::cramp_zip ? CrampISD::SHFL : CrampISD::UNSHFL;
    unsigned BitWidth = Op.getValueSizeInBits();
    assert(isPowerOf2_32(BitWidth) && BitWidth >= 2 && "Unexpected bit width");
    return DAG.getNode(Opc, DL, XLenVT, Op.getOperand(1),
                       DAG.getConstant((BitWidth / 2) - 1, DL, XLenVT));
  }
  case Intrinsic::cramp_shfl:
  case Intrinsic::cramp_unshfl: {
    unsigned Opc =
        IntNo == Intrinsic::cramp_shfl ? CrampISD::SHFL : CrampISD::UNSHFL;
    return DAG.getNode(Opc, DL, XLenVT, Op.getOperand(1), Op.getOperand(2));
  }
  case Intrinsic::cramp_bcompress:
  case Intrinsic::cramp_bdecompress: {
    unsigned Opc = IntNo == Intrinsic::cramp_bcompress ? CrampISD::BCOMPRESS
                                                       : CrampISD::BDECOMPRESS;
    return DAG.getNode(Opc, DL, XLenVT, Op.getOperand(1), Op.getOperand(2));
  }
  case Intrinsic::cramp_bfp:
    return DAG.getNode(CrampISD::BFP, DL, XLenVT, Op.getOperand(1),
                       Op.getOperand(2));
  case Intrinsic::cramp_fsl:
    return DAG.getNode(CrampISD::FSL, DL, XLenVT, Op.getOperand(1),
                       Op.getOperand(2), Op.getOperand(3));
  case Intrinsic::cramp_fsr:
    return DAG.getNode(CrampISD::FSR, DL, XLenVT, Op.getOperand(1),
                       Op.getOperand(2), Op.getOperand(3));
  case Intrinsic::cramp_vmv_x_s:
    assert(Op.getValueType() == XLenVT && "Unexpected VT!");
    return DAG.getNode(CrampISD::VMV_X_S, DL, Op.getValueType(),
                       Op.getOperand(1));
  case Intrinsic::cramp_vmv_v_x:
    return lowerScalarSplat(Op.getOperand(1), Op.getOperand(2),
                            Op.getOperand(3), Op.getSimpleValueType(), DL, DAG,
                            Subtarget);
  case Intrinsic::cramp_vfmv_v_f:
    return DAG.getNode(CrampISD::VFMV_V_F_VL, DL, Op.getValueType(),
                       Op.getOperand(1), Op.getOperand(2), Op.getOperand(3));
  case Intrinsic::cramp_vmv_s_x: {
    SDValue Scalar = Op.getOperand(2);

    if (Scalar.getValueType().bitsLE(XLenVT)) {
      Scalar = DAG.getNode(ISD::ANY_EXTEND, DL, XLenVT, Scalar);
      return DAG.getNode(CrampISD::VMV_S_X_VL, DL, Op.getValueType(),
    ...
    if (Op.getOperand(1).isUndef())
      return SplattedVal;
    SDValue SplattedIdx =
        DAG.getNode(CrampISD::VMV_V_X_VL, DL, VT, DAG.getUNDEF(VT),
                    DAG.getConstant(0, DL, MVT::i32), VL);

    MVT MaskVT = getMaskTypeFor(VT);
    SDValue Mask = getAllOnesMask(VT, VL, DL, DAG);
    SDValue VID = DAG.getNode(CrampISD::VID_VL, DL, VT, Mask, VL);
    SDValue SelectCond =
        DAG.getNode(CrampISD::SETCC_VL, DL, MaskVT, VID, SplattedIdx,
                    DAG.getCondCode(ISD::SETEQ), Mask, VL);
    return DAG.getNode(CrampISD::VSELECT_VL, DL, VT, SelectCond, SplattedVal,
                       Vec, VL);
  }
  }
...
SDValue CrampTargetLowering::LowerINTRINSIC_W_CHAIN(SDValue Op,
                                                    SelectionDAG &DAG) const {
  ...
  case Intrinsic::cramp_masked_strided_load: {
    ...
    SDValue IntID = DAG.getTargetConstant(
        IsUnmasked ? Intrinsic::cramp_vlse : Intrinsic::cramp_vlse_mask, DL,
        XLenVT);
    ...
    Ops.push_back(VL);
    if (!IsUnmasked) {
      SDValue Policy = DAG.getTargetConstant(CrampII::TAIL_AGNOSTIC, DL, XLenVT);
  ...
  case Intrinsic::cramp_seg8_load: {
    ...
    static const Intrinsic::ID VlsegInts[7] = {
        Intrinsic::cramp_vlseg2, Intrinsic::cramp_vlseg3,
        Intrinsic::cramp_vlseg4, Intrinsic::cramp_vlseg5,
        Intrinsic::cramp_vlseg6, Intrinsic::cramp_vlseg7,
        Intrinsic::cramp_vlseg8};
...
SDValue CrampTargetLowering::LowerINTRINSIC_VOID(SDValue Op,
                                                 SelectionDAG &DAG) const {
  unsigned IntNo = Op.getConstantOperandVal(1);
  switch (IntNo) {
  default:
  ...
  case Intrinsic::cramp_masked_strided_store: {
    ...
    SDValue IntID = DAG.getTargetConstant(
        IsUnmasked ? Intrinsic::cramp_vsse : Intrinsic::cramp_vsse_mask, DL,
        XLenVT);
...
static MVT getLMUL1VT(MVT VT) {
  ...
  return MVT::getScalableVectorVT(
      ...
      Cramp::RVVBitsPerBlock / VT.getVectorElementType().getSizeInBits());
}
...
static unsigned getRVVReductionOp(unsigned ISDOpcode) {
  ...
  case ISD::VECREDUCE_ADD:
    return CrampISD::VECREDUCE_ADD_VL;
  case ISD::VECREDUCE_UMAX:
    return CrampISD::VECREDUCE_UMAX_VL;
  case ISD::VECREDUCE_SMAX:
    return CrampISD::VECREDUCE_SMAX_VL;
  case ISD::VECREDUCE_UMIN:
    return CrampISD::VECREDUCE_UMIN_VL;
  case ISD::VECREDUCE_SMIN:
    return CrampISD::VECREDUCE_SMIN_VL;
  case ISD::VECREDUCE_AND:
    return CrampISD::VECREDUCE_AND_VL;
  case ISD::VECREDUCE_OR:
    return CrampISD::VECREDUCE_OR_VL;
  case ISD::VECREDUCE_XOR:
    return CrampISD::VECREDUCE_XOR_VL;
  }
}

SDValue CrampTargetLowering::lowerVectorMaskVecReduction(SDValue Op,
                                                         SelectionDAG &DAG,
                                                         bool IsVP) const {
  SDLoc DL(Op);
  ...
  case ISD::VP_REDUCE_AND: {
    // vcpop ~x == 0
    SDValue TrueMask = DAG.getNode(CrampISD::VMSET_VL, DL, ContainerVT, VL);
    Vec = DAG.getNode(CrampISD::VMXOR_VL, DL, ContainerVT, Vec, TrueMask, VL);
    Vec = DAG.getNode(CrampISD::VCPOP_VL, DL, XLenVT, Vec, Mask, VL);
    CC = ISD::SETEQ;
    BaseOpc = ISD::AND;
    break;
  }
...
SDValue CrampTargetLowering::lowerVECREDUCE(SDValue Op,
                                            SelectionDAG &DAG) const {
  SDLoc DL(Op);
  SDValue Vec = Op.getOperand(0);
...
getRVVFPReductionOpAndOperands(SDValue Op, SelectionDAG &DAG, EVT EltVT) {
  ...
  case ISD::VECREDUCE_FADD: {
    ...
    return std::make_tuple(CrampISD::VECREDUCE_FADD_VL, Op.getOperand(0), Zero);
  }
  case ISD::VECREDUCE_SEQ_FADD:
    return std::make_tuple(CrampISD::VECREDUCE_SEQ_FADD_VL, Op.getOperand(1),
                           Op.getOperand(0));
  case ISD::VECREDUCE_FMIN:
    return std::make_tuple(CrampISD::VECREDUCE_FMIN_VL, Op.getOperand(0),
                           DAG.getNeutralElement(BaseOpcode, DL, EltVT, Flags));
  case ISD::VECREDUCE_FMAX:
    return std::make_tuple(CrampISD::VECREDUCE_FMAX_VL, Op.getOperand(0),
                           DAG.getNeutralElement(BaseOpcode, DL, EltVT, Flags));
  }
}

SDValue CrampTargetLowering::lowerFPVECREDUCE(SDValue Op,
                                              SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
static unsigned getRVVVPReductionOp(unsigned ISDOpcode) {
  ...
  case ISD::VP_REDUCE_XOR:
    return CrampISD::VECREDUCE_XOR_VL;
  case ISD::VP_REDUCE_FADD:
    return CrampISD::VECREDUCE_FADD_VL;
  case ISD::VP_REDUCE_SEQ_FADD:
    return CrampISD::VECREDUCE_SEQ_FADD_VL;
  case ISD::VP_REDUCE_FMAX:
    return CrampISD::VECREDUCE_FMAX_VL;
  case ISD::VP_REDUCE_FMIN:
    return CrampISD::VECREDUCE_FMIN_VL;
  }
}

SDValue CrampTargetLowering::lowerVPREDUCE(SDValue Op,
                                           SelectionDAG &DAG) const {
  SDLoc DL(Op);
  SDValue Vec = Op.getOperand(1);
  EVT VecEVT = Vec.getValueType();
...
SDValue CrampTargetLowering::lowerINSERT_SUBVECTOR(SDValue Op,
                                                   SelectionDAG &DAG) const {
  SDValue Vec = Op.getOperand(0);
  ...
  CrampII::VLMUL SubVecLMUL = CrampTargetLowering::getLMUL(SubVecVT);
  bool IsSubVecPartReg = SubVecLMUL == CrampII::VLMUL::LMUL_F2 ||
                         SubVecLMUL == CrampII::VLMUL::LMUL_F4 ||
                         SubVecLMUL == CrampII::VLMUL::LMUL_F8;
  ...
  SDValue Slideup = DAG.getNode(CrampISD::VSLIDEUP_VL, DL, InterSubVT,
...
SDValue CrampTargetLowering::lowerEXTRACT_SUBVECTOR(SDValue Op,
                                                    SelectionDAG &DAG) const {
  SDValue Vec = Op.getOperand(0);
  ...
  if (SubVecVT.isFixedLengthVector()) {
    ...
    SDValue Slidedown =
        DAG.getNode(CrampISD::VSLIDEDOWN_VL, DL, ContainerVT,
  ...
  SDValue Slidedown =
      DAG.getNode(CrampISD::VSLIDEDOWN_VL, DL, InterSubVT,
...
SDValue CrampTargetLowering::lowerSTEP_VECTOR(SDValue Op,
                                              SelectionDAG &DAG) const {
  SDLoc DL(Op);
  MVT VT = Op.getSimpleValueType();
  MVT XLenVT = Subtarget.getXLenVT();
  ...
  SDValue StepVec = DAG.getNode(CrampISD::VID_VL, DL, VT, Mask, VL);
...
SDValue CrampTargetLowering::lowerVECTOR_REVERSE(SDValue Op,
                                                 SelectionDAG &DAG) const {
  SDLoc DL(Op);
  MVT VecVT = Op.getSimpleValueType();
  if (VecVT.getVectorElementType() == MVT::i1) {
  ...
  unsigned GatherOpc = CrampISD::VRGATHER_VV_VL;
  ...
  if (MaxVLMAX > 256 && EltSize == 8) {
    ...
    if (MinSize == (8 * Cramp::RVVBitsPerBlock)) {
      SDValue Lo, Hi;
    ...
    GatherOpc = CrampISD::VRGATHEREI16_VV_VL;
  }
  ...
  if (!IsRV32E64)
    ...
    SplatVL = DAG.getNode(CrampISD::VMV_V_X_VL, DL, IntVT, DAG.getUNDEF(IntVT),
                          VLMinus1, DAG.getRegister(Cramp::X0, XLenVT));

  SDValue VID = DAG.getNode(CrampISD::VID_VL, DL, IntVT, Mask, VL);
  SDValue Indices =
      DAG.getNode(CrampISD::SUB_VL, DL, IntVT, SplatVL, VID, Mask, VL);

  return DAG.getNode(GatherOpc, DL, VecVT, Op.getOperand(0), Indices, Mask,
                     DAG.getUNDEF(VecVT), VL);
}

SDValue CrampTargetLowering::lowerVECTOR_SPLICE(SDValue Op,
                                                SelectionDAG &DAG) const {
  SDLoc DL(Op);
  SDValue V1 = Op.getOperand(0);
  SDValue V2 = Op.getOperand(1);
  ...
  SDValue SlideDown =
      DAG.getNode(CrampISD::VSLIDEDOWN_VL, DL, VecVT, DAG.getUNDEF(VecVT), V1,
                  DownOffset, TrueMask, UpOffset);
  return DAG.getNode(CrampISD::VSLIDEUP_VL, DL, VecVT, SlideDown, V2, UpOffset,
                     TrueMask, DAG.getRegister(Cramp::X0, XLenVT));
}

SDValue
CrampTargetLowering::lowerFixedLengthVectorLoadToRVV(SDValue Op,
                                                     SelectionDAG &DAG) const {
  SDLoc DL(Op);
  auto *Load = cast<LoadSDNode>(Op);
  ...
  SDValue IntID = DAG.getTargetConstant(
      IsMaskOp ? Intrinsic::cramp_vlm : Intrinsic::cramp_vle, DL, XLenVT);
...
SDValue
CrampTargetLowering::lowerFixedLengthVectorStoreToRVV(SDValue Op,
                                                      SelectionDAG &DAG) const {
  SDLoc DL(Op);
  auto *Store = cast<StoreSDNode>(Op);
  ...
  bool IsMaskOp = VT.getVectorElementType() == MVT::i1;
  SDValue IntID = DAG.getTargetConstant(
      IsMaskOp ? Intrinsic::cramp_vsm : Intrinsic::cramp_vse, DL, XLenVT);
...
SDValue CrampTargetLowering::lowerMaskedLoad(SDValue Op,
                                             SelectionDAG &DAG) const {
  SDLoc DL(Op);
  MVT VT = Op.getSimpleValueType();
  ...
  unsigned IntID =
      IsUnmasked ? Intrinsic::cramp_vle : Intrinsic::cramp_vle_mask;
  ...
  Ops.push_back(VL);
  if (!IsUnmasked)
    Ops.push_back(DAG.getTargetConstant(CrampII::TAIL_AGNOSTIC, DL, XLenVT));
...
SDValue CrampTargetLowering::lowerMaskedStore(SDValue Op,
                                              SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ...
  unsigned IntID =
      IsUnmasked ? Intrinsic::cramp_vse : Intrinsic::cramp_vse_mask;
  SmallVector<SDValue, 8> Ops{Chain, DAG.getTargetConstant(IntID, DL, XLenVT)};
  Ops.push_back(Val);
  Ops.push_back(BasePtr);
  if (!IsUnmasked)
...
SDValue
CrampTargetLowering::lowerFixedLengthVectorSetccToRVV(SDValue Op,
                                                      SelectionDAG &DAG) const {
  ...
  SDValue Cmp = DAG.getNode(CrampISD::SETCC_VL, DL, MaskVT, Op1, Op2,
                            Op.getOperand(2), Mask, VL);

  return convertFromScalableVector(VT, Cmp, DAG, Subtarget);
}

SDValue CrampTargetLowering::lowerFixedLengthVectorLogicOpToRVV(
...
SDValue
CrampTargetLowering::lowerFixedLengthVectorShiftToRVV(SDValue Op,
                                                      SelectionDAG &DAG) const {
  unsigned Opc;
  switch (Op.getOpcode()) {
  default: llvm_unreachable("Unexpected opcode!");
  case ISD::SHL: Opc = CrampISD::SHL_VL; break;
  case ISD::SRA: Opc = CrampISD::SRA_VL; break;
  case ISD::SRL: Opc = CrampISD::SRL_VL; break;
  }
...
SDValue CrampTargetLowering::lowerABS(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
  MVT VT = Op.getSimpleValueType();
  ...
  SDValue SplatZero = DAG.getNode(
      CrampISD::VMV_V_X_VL, DL, ContainerVT, DAG.getUNDEF(ContainerVT),
      DAG.getConstant(0, DL, Subtarget.getXLenVT()));
  SDValue NegX =
      DAG.getNode(CrampISD::SUB_VL, DL, ContainerVT, SplatZero, X, Mask, VL);
  SDValue Max =
      DAG.getNode(CrampISD::SMAX_VL, DL, ContainerVT, X, NegX, Mask, VL);

  return convertFromScalableVector(VT, Max, DAG, Subtarget);
}

SDValue CrampTargetLowering::lowerFixedLengthVectorFCOPYSIGNToRVV(
  ...
  SDValue CopySign =
      DAG.getNode(CrampISD::FCOPYSIGN_VL, DL, ContainerVT, Mag, Sign, Mask, VL);

  return convertFromScalableVector(VT, CopySign, DAG, Subtarget);
}

SDValue CrampTargetLowering::lowerFixedLengthVectorSelectToRVV(
    SDValue Op, SelectionDAG &DAG) const {
  MVT VT = Op.getSimpleValueType();
  MVT ContainerVT = getContainerForFixedLengthVector(VT);
  ...
  SDValue Select =
      DAG.getNode(CrampISD::VSELECT_VL, DL, ContainerVT, CC, Op1, Op2, VL);

  return convertFromScalableVector(VT, Select, DAG, Subtarget);
}

SDValue CrampTargetLowering::lowerToScalableOp(SDValue Op, SelectionDAG &DAG,
                                               unsigned NewOpc,
                                               bool HasMask) const {
  MVT VT = Op.getSimpleValueType();
  MVT ContainerVT = getContainerForFixedLengthVector(VT);
...
// Lower a VP_* ISD node to the corresponding CrampISD::*_VL node:
...
//   types.
SDValue CrampTargetLowering::lowerVPOp(SDValue Op, SelectionDAG &DAG,
                                       unsigned CrampISDOpc) const {
  SDLoc DL(Op);
  MVT VT = Op.getSimpleValueType();
  ...
  if (!VT.isFixedLengthVector())
    return DAG.getNode(CrampISDOpc, DL, VT, Ops, Op->getFlags());

  MVT ContainerVT = getContainerForFixedLengthVector(VT);

  SDValue VPOp = DAG.getNode(CrampISDOpc, DL, ContainerVT, Ops, Op->getFlags());

  return convertFromScalableVector(VT, VPOp, DAG, Subtarget);
}

SDValue CrampTargetLowering::lowerVPExtMaskOp(SDValue Op,
                                              SelectionDAG &DAG) const {
  SDLoc DL(Op);
  MVT VT = Op.getSimpleValueType();
  ...
  MVT XLenVT = Subtarget.getXLenVT();
  SDValue Zero = DAG.getConstant(0, DL, XLenVT);
  SDValue ZeroSplat = DAG.getNode(CrampISD::VMV_V_X_VL, DL, ContainerVT,
                                  DAG.getUNDEF(ContainerVT), Zero, VL);

  SDValue SplatValue = DAG.getConstant(
      Op.getOpcode() == ISD::VP_ZERO_EXTEND ? 1 : -1, DL, XLenVT);
  SDValue Splat = DAG.getNode(CrampISD::VMV_V_X_VL, DL, ContainerVT,
                              DAG.getUNDEF(ContainerVT), SplatValue, VL);

  SDValue Result = DAG.getNode(CrampISD::VSELECT_VL, DL, ContainerVT, Src,
                               Splat, ZeroSplat, VL);
  if (!VT.isFixedLengthVector())
    return Result;
  return convertFromScalableVector(VT, Result, DAG, Subtarget);
...
SDValue CrampTargetLowering::lowerVPSetCCMaskOp(SDValue Op,
                                                SelectionDAG &DAG) const {
  SDLoc DL(Op);
  MVT VT = Op.getSimpleValueType();
  ...
  SDValue Result;
  SDValue AllOneMask = DAG.getNode(CrampISD::VMSET_VL, DL, ContainerVT, VL);
  ...
  // X != Y  --> (X^Y)
  case ISD::SETNE:
    Result = DAG.getNode(CrampISD::VMXOR_VL, DL, ContainerVT, Op1, Op2, VL);
    break;
  // X == Y  --> ~(X^Y)
  case ISD::SETEQ: {
    SDValue Temp =
        DAG.getNode(CrampISD::VMXOR_VL, DL, ContainerVT, Op1, Op2, VL);
    Result =
        DAG.getNode(CrampISD::VMXOR_VL, DL, ContainerVT, Temp, AllOneMask, VL);
    break;
  }
  // X >s Y   -->  X == 0 & Y == 1  -->  ~X & Y
  // X <u Y   -->  X == 0 & Y == 1  -->  ~X & Y
  ...
  case ISD::SETULT: {
    SDValue Temp =
        DAG.getNode(CrampISD::VMXOR_VL, DL, ContainerVT, Op1, AllOneMask, VL);
    Result = DAG.getNode(CrampISD::VMAND_VL, DL, ContainerVT, Temp, Op2, VL);
    break;
  }
  // X <s Y   --> X == 1 & Y == 0  -->  ~Y & X
  // X >u Y   --> X == 1 & Y == 0  -->  ~Y & X
  ...
  case ISD::SETUGT: {
    SDValue Temp =
        DAG.getNode(CrampISD::VMXOR_VL, DL, ContainerVT, Op2, AllOneMask, VL);
    Result = DAG.getNode(CrampISD::VMAND_VL, DL, ContainerVT, Op1, Temp, VL);
    break;
  }
  // X >=s Y  --> X == 0 | Y == 1  -->  ~X | Y
  // X <=u Y  --> X == 0 | Y == 1  -->  ~X | Y
  ...
  case ISD::SETULE: {
    SDValue Temp =
        DAG.getNode(CrampISD::VMXOR_VL, DL, ContainerVT, Op1, AllOneMask, VL);
    Result = DAG.getNode(CrampISD::VMXOR_VL, DL, ContainerVT, Temp, Op2, VL);
    break;
  }
  // X <=s Y  --> X == 1 | Y == 0  -->  ~Y | X
  // X >=u Y  --> X == 1 | Y == 0  -->  ~Y | X
  ...
  case ISD::SETUGE: {
    SDValue Temp =
        DAG.getNode(CrampISD::VMXOR_VL, DL, ContainerVT, Op2, AllOneMask, VL);
    Result = DAG.getNode(CrampISD::VMXOR_VL, DL, ContainerVT, Temp, Op1, VL);
    break;
  }
  }
...
SDValue CrampTargetLowering::lowerVPFPIntConvOp(SDValue Op, SelectionDAG &DAG,
                                                unsigned CrampISDOpc) const {
  SDLoc DL(Op);
  ...
  unsigned CrampISDExtOpc = (CrampISDOpc == CrampISD::SINT_TO_FP_VL ||
                             CrampISDOpc == CrampISD::FP_TO_SINT_VL)
                                ? CrampISD::VSEXT_VL
                                : CrampISD::VZEXT_VL;
  ...
  if (DstEltSize >= SrcEltSize) { // Single-width and widening conversion.
    if (SrcVT.isInteger()) {
      ...
      if (SrcEltSize == 1) {
        ...
        MVT XLenVT = Subtarget.getXLenVT();
        SDValue Zero = DAG.getConstant(0, DL, XLenVT);
        SDValue ZeroSplat = DAG.getNode(CrampISD::VMV_V_X_VL, DL, IntVT,
                                        DAG.getUNDEF(IntVT), Zero, VL);
        SDValue One = DAG.getConstant(
            CrampISDExtOpc == CrampISD::VZEXT_VL ? 1 : -1, DL, XLenVT);
        SDValue OneSplat = DAG.getNode(CrampISD::VMV_V_X_VL, DL, IntVT,
                                       DAG.getUNDEF(IntVT), One, VL);
        Src = DAG.getNode(CrampISD::VSELECT_VL, DL, IntVT, Src, OneSplat,
                          ZeroSplat, VL);
      } else if (DstEltSize > (2 * SrcEltSize)) {
        // Widen before converting.
        MVT IntVT = MVT::getVectorVT(MVT::getIntegerVT(DstEltSize / 2),
                                     DstVT.getVectorElementCount());
        Src = DAG.getNode(CrampISDExtOpc, DL, IntVT, Src, Mask, VL);
      }

      Result = DAG.getNode(CrampISDOpc, DL, DstVT, Src, Mask, VL);
    } else {
      ...
      if (DstEltSize > (2 * SrcEltSize)) {
        ...
        Src =
            DAG.getNode(CrampISD::FP_EXTEND_VL, DL, InterimFVT, Src, Mask, VL);
      }
    ...
    if (SrcVT.isInteger()) {
      ...
      Result = DAG.getNode(CrampISDOpc, DL, InterimFVT, Src, Mask, VL);

      if (InterimFVT != DstVT) {
        Src = Result;
        Result = DAG.getNode(CrampISD::FP_ROUND_VL, DL, DstVT, Src, Mask, VL);
      }
    } else {
      assert(SrcVT.isFloatingPoint() && DstVT.isInteger() &&
             "Wrong input/output vector types");
      ...
      if (DstEltSize == 1) {
        ...
        MVT InterimIVT = MVT::getVectorVT(MVT::getIntegerVT(SrcEltSize),
                                          DstVT.getVectorElementCount());
        Result = DAG.getNode(CrampISDOpc, DL, InterimIVT, Src, Mask, VL);

        // Compare the integer result to 0. The integer should be 0 or 1/-1,
        // otherwise the conversion was undefined.
        MVT XLenVT = Subtarget.getXLenVT();
        SDValue SplatZero = DAG.getConstant(0, DL, XLenVT);
        SplatZero = DAG.getNode(CrampISD::VMV_V_X_VL, DL, InterimIVT,
                                DAG.getUNDEF(InterimIVT), SplatZero);
        Result = DAG.getNode(CrampISD::SETCC_VL, DL, DstVT, Result, SplatZero,
                             DAG.getCondCode(ISD::SETNE), Mask, VL);
      } else {
        ...
        while (InterimIVT != DstVT) {
          ...
          InterimIVT = MVT::getVectorVT(MVT::getIntegerVT(SrcEltSize / 2),
                                        DstVT.getVectorElementCount());
          Result = DAG.getNode(CrampISD::TRUNCATE_VECTOR_VL, DL, InterimIVT,
                               Src, Mask, VL);
        }
      }
    }
...
SDValue CrampTargetLowering::lowerLogicVPOp(SDValue Op, SelectionDAG &DAG,
                                            unsigned MaskOpc,
...
// accordingly.
SDValue CrampTargetLowering::lowerMaskedGather(SDValue Op,
                                               SelectionDAG &DAG) const {
  SDLoc DL(Op);
  MVT VT = Op.getSimpleValueType();
  ...
  if (XLenVT == MVT::i32 && IndexVT.getVectorElementType().bitsGT(XLenVT)) {
    ...
    SDValue TrueMask = DAG.getNode(CrampISD::VMSET_VL, DL, Mask.getValueType(),
                                   VL);
    Index = DAG.getNode(CrampISD::TRUNCATE_VECTOR_VL, DL, IndexVT, Index,
                        TrueMask, VL);
  }

  unsigned IntID =
      IsUnmasked ? Intrinsic::cramp_vluxei : Intrinsic::cramp_vluxei_mask;
  SmallVector<SDValue, 8> Ops{Chain, DAG.getTargetConstant(IntID, DL, XLenVT)};
  if (IsUnmasked)
    Ops.push_back(DAG.getUNDEF(ContainerVT));
  else
...
// signed or scaled indexing is extended to the XLEN value type and scaled
// accordingly.
SDValue CrampTargetLowering::lowerMaskedScatter(SDValue Op,
                                                SelectionDAG &DAG) const {
  SDLoc DL(Op);
  const auto *MemSD = cast<MemSDNode>(Op.getNode());
  EVT MemVT = MemSD->getMemoryVT();
  ...
  unsigned IntID =
      IsUnmasked ? Intrinsic::cramp_vsoxei : Intrinsic::cramp_vsoxei_mask;
  SmallVector<SDValue, 8> Ops{Chain, DAG.getTargetConstant(IntID, DL, XLenVT)};
  Ops.push_back(Val);
  Ops.push_back(BasePtr);
  Ops.push_back(Index);
...
SDValue CrampTargetLowering::lowerGET_ROUNDING(SDValue Op,
                                               SelectionDAG &DAG) const {
  const MVT XLenVT = Subtarget.getXLenVT();
  SDLoc DL(Op);
  SDValue Chain = Op->getOperand(0);
  SDValue SysRegNo = DAG.getTargetConstant(
      CrampSysReg::lookupSysRegByName("FRM")->Encoding, DL, XLenVT);
  SDVTList VTs = DAG.getVTList(XLenVT, MVT::Other);
  SDValue RM = DAG.getNode(CrampISD::READ_CSR, DL, VTs, Chain, SysRegNo);

  // Encoding used for rounding mode in Cramp differs from that used in
  // FLT_ROUNDS. To convert it the Cramp rounding mode is used as an index in a
  // table, which consists of a sequence of 4-bit fields, each representing
  // corresponding FLT_ROUNDS mode.
  static const int Table =
      (int(RoundingMode::NearestTiesToEven) << 4 * CrampFPRndMode::RNE) |
      (int(RoundingMode::TowardZero) << 4 * CrampFPRndMode::RTZ) |
      (int(RoundingMode::TowardNegative) << 4 * CrampFPRndMode::RDN) |
      (int(RoundingMode::TowardPositive) << 4 * CrampFPRndMode::RUP) |
      (int(RoundingMode::NearestTiesToAway) << 4 * CrampFPRndMode::RMM);
...
SDValue CrampTargetLowering::lowerSET_ROUNDING(SDValue Op,
                                               SelectionDAG &DAG) const {
  const MVT XLenVT = Subtarget.getXLenVT();
  SDLoc DL(Op);
  SDValue Chain = Op->getOperand(0);
  ...
  // corresponding Cramp mode.
  static const unsigned Table =
      (CrampFPRndMode::RNE << 4 * int(RoundingMode::NearestTiesToEven)) |
      (CrampFPRndMode::RTZ << 4 * int(RoundingMode::TowardZero)) |
      (CrampFPRndMode::RDN << 4 * int(RoundingMode::TowardNegative)) |
      (CrampFPRndMode::RUP << 4 * int(RoundingMode::TowardPositive)) |
      (CrampFPRndMode::RMM << 4 * int(RoundingMode::NearestTiesToAway));
  ...
  return DAG.getNode(CrampISD::WRITE_CSR, DL, MVT::Other, Chain, SysRegNo,
                     RMValue);
}

SDValue CrampTargetLowering::lowerEH_DWARF_CFA(SDValue Op,
                                               SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();

  bool isCramp64 = Subtarget.is64Bit();
  EVT PtrVT = getPointerTy(DAG.getDataLayout());

  int FI = MF.getFrameInfo().CreateFixedObject(isCramp64 ? 8 : 4, 0, false);
  return DAG.getFrameIndex(FI, PtrVT);
}

static CrampISD::NodeType getCrampWOpcodeByIntr(unsigned IntNo) {
  switch (IntNo) {
  default:
    llvm_unreachable("Unexpected Intrinsic");
  case Intrinsic::cramp_bcompress:
    return CrampISD::BCOMPRESSW;
  case Intrinsic::cramp_bdecompress:
    return CrampISD::BDECOMPRESSW;
  case Intrinsic::cramp_bfp:
    return CrampISD::BFPW;
  case Intrinsic::cramp_fsl:
    return CrampISD::FSLW;
  case Intrinsic::cramp_fsr:
    return CrampISD::FSRW;
  }
}
...
static SDValue customLegalizeToWOpByIntr(SDNode *N, SelectionDAG &DAG,
                                         unsigned IntNo) {
  SDLoc DL(N);
  CrampISD::NodeType WOpcode = getCrampWOpcodeByIntr(IntNo);
...
static CrampISD::NodeType getCrampWOpcode(unsigned Opcode) {
  switch (Opcode) {
  default:
    llvm_unreachable("Unexpected opcode");
  case ISD::SHL:
    return CrampISD::SLLW;
  case ISD::SRA:
    return CrampISD::SRAW;
  case ISD::SRL:
    return CrampISD::SRLW;
  case ISD::SDIV:
    return CrampISD::DIVW;
  case ISD::UDIV:
    return CrampISD::DIVUW;
  case ISD::UREM:
    return CrampISD::REMUW;
  case ISD::ROTL:
    return CrampISD::ROLW;
  case ISD::ROTR:
    return CrampISD::RORW;
  }
}
...
static SDValue customLegalizeToWOp(SDNode *N, SelectionDAG &DAG,
  ...
  SDLoc DL(N);
  CrampISD::NodeType WOpcode = getCrampWOpcode(N->getOpcode());
...
void CrampTargetLowering::ReplaceNodeResults(SDNode *N,
  ...
  case ISD::FP_TO_UINT: {
    ...
    if (getTypeAction(*DAG.getContext(), Op0.getValueType()) !=
      ...
      if (!isTypeLegal(Op0.getValueType()))
        return;
      if (IsStrict) {
        unsigned Opc = IsSigned ? CrampISD::STRICT_FCVT_W_RV64
                                : CrampISD::STRICT_FCVT_WU_RV64;
        SDVTList VTs = DAG.getVTList(MVT::i64, MVT::Other);
        SDValue Res = DAG.getNode(
            Opc, DL, VTs, N->getOperand(0), Op0,
            DAG.getTargetConstant(CrampFPRndMode::RTZ, DL, MVT::i64));
        Results.push_back(DAG.getNode(ISD::TRUNCATE, DL, MVT::i32, Res));
        Results.push_back(Res.getValue(1));
        return;
      }
      unsigned Opc = IsSigned ? CrampISD::FCVT_W_RV64 : CrampISD::FCVT_WU_RV64;
      SDValue Res =
  ...
  case ISD::READCYCLECOUNTER: {
    assert(!Subtarget.is64Bit() &&
           "READCYCLECOUNTER only has custom type legalization on cramp32");

    SDVTList VTs = DAG.getVTList(MVT::i32, MVT::i32, MVT::Other);
    SDValue RCW =
        DAG.getNode(CrampISD::READ_CYCLE_WIDE, DL, VTs, N->getOperand(0));
  ...
  case ISD::MUL: {
    ...
    if (Size > XLen) {
      ...
      auto MakeMULPair = [&](SDValue S, SDValue U) {
        ...
        SDValue Hi = DAG.getNode(CrampISD::MULHSU, DL, XLenVT, S, U);
  ...
  case ISD::CTLZ_ZERO_UNDEF: {
    ...
    unsigned Opc = IsCTZ ? CrampISD::CTZW : CrampISD::CLZW;
  ...
  case ISD::BITCAST: {
    ...
    if (VT == MVT::i16 && Op0VT == MVT::f16 && Subtarget.hasStdExtZfh()) {
      SDValue FPConv = DAG.getNode(CrampISD::FMV_X_ANYEXTH, DL, XLenVT, Op0);
      Results.push_back(DAG.getNode(ISD::TRUNCATE, DL, MVT::i16, FPConv));
    } else if (VT == MVT::i32 && Op0VT == MVT::f32 && Subtarget.is64Bit() &&
               Subtarget.hasStdExtF()) {
      SDValue FPConv =
          DAG.getNode(CrampISD::FMV_X_ANYEXTW_RV64, DL, MVT::i64, Op0);
  ...
  case CrampISD::SHFL: {
    ...
    assert((Subtarget.hasStdExtZbp() ||
            (Subtarget.hasStdExtZbkb() && N->getOpcode() == CrampISD::GREV &&
  ...
  case ISD::BITREVERSE: {
    ...
    if (N->getOpcode() == ISD::BSWAP)
      Imm &= ~0x7U;
    SDValue GREVI = DAG.getNode(CrampISD::GREV, DL, XLenVT, NewOp0,
  ...
  case ISD::EXTRACT_VECTOR_ELT: {
    ...
    SDValue EltLo = DAG.getNode(CrampISD::VMV_X_S, DL, XLenVT, Vec);
    ...
    SDValue ThirtyTwoV = DAG.getNode(CrampISD::VMV_V_X_VL, DL, ContainerVT,
                                     DAG.getUNDEF(ContainerVT),
                                     DAG.getConstant(32, DL, XLenVT), VL);
    SDValue LShr32 = DAG.getNode(CrampISD::SRL_VL, DL, ContainerVT, Vec,
                                 ThirtyTwoV, Mask, VL);

    SDValue EltHi = DAG.getNode(CrampISD::VMV_X_S, DL, XLenVT, LShr32);
  ...
  case ISD::INTRINSIC_WO_CHAIN: {
    ...
    case Intrinsic::cramp_gorc: {
      ...
      SDValue NewOp2 =
          DAG.getNode(ISD::ANY_EXTEND, DL, MVT::i64, N->getOperand(2));
      unsigned Opc =
          IntNo == Intrinsic::cramp_grev ? CrampISD::GREVW : CrampISD::GORCW;
      ...
      if (isa<ConstantSDNode>(NewOp2)) {
        NewOp2 = DAG.getNode(ISD::AND, DL, MVT::i64, NewOp2,
                             DAG.getConstant(0x1f, DL, MVT::i64));
        Opc = IntNo == Intrinsic::cramp_grev ? CrampISD::GREV : CrampISD::GORC;
      }
    ...
    case Intrinsic::cramp_orc_b: {
      ...
      SDValue NewOp =
          DAG.getNode(ISD::ANY_EXTEND, DL, MVT::i64, N->getOperand(1));
      SDValue Res = DAG.getNode(CrampISD::GORC, DL, MVT::i64, NewOp,
    ...
    case Intrinsic::cramp_unshfl: {
      ...
      SDValue NewOp2 =
          DAG.getNode(ISD::ANY_EXTEND, DL, MVT::i64, N->getOperand(2));
      unsigned Opc =
          IntNo == Intrinsic::cramp_shfl ? CrampISD::SHFLW : CrampISD::UNSHFLW;
    ...
    case Intrinsic::cramp_vmv_x_s: {
      ...
      if (VT.bitsLT(XLenVT)) {
        ...
        SDValue Extract = DAG.getNode(CrampISD::VMV_X_S, DL,
      ...
      SDValue ThirtyTwoV =
          DAG.getNode(CrampISD::VMV_V_X_VL, DL, VecVT, DAG.getUNDEF(VecVT),
                      DAG.getConstant(32, DL, XLenVT), VL);
      SDValue LShr32 =
          DAG.getNode(CrampISD::SRL_VL, DL, VecVT, Vec, ThirtyTwoV, Mask, VL);
      SDValue EltHi = DAG.getNode(CrampISD::VMV_X_S, DL, XLenVT, LShr32);
...
struct CrampBitmanipPat {
  SDValue Op;
  unsigned ShAmt;
  bool IsSHL;

  bool formsPairWith(const CrampBitmanipPat &Other) const {
...
// XLen is 64.
static Optional<CrampBitmanipPat>
matchCrampBitmanipPat(SDValue Op, ArrayRef<uint64_t> BitmanipMasks) {
  ...
  return CrampBitmanipPat{Src, (unsigned)ShAmt, IsSHL};
}
...
static Optional<CrampBitmanipPat> matchGREVIPat(SDValue Op) {
  ...
  return matchCrampBitmanipPat(Op, BitmanipMasks);
}
...
static SDValue combineBinOpToReduce(SDNode *N, SelectionDAG &DAG) {
  ...
  if (ScalarV.getOpcode() != CrampISD::VFMV_S_F_VL &&
      ScalarV.getOpcode() != CrampISD::VMV_S_X_VL &&
      ScalarV.getOpcode() != CrampISD::VMV_V_X_VL)
    return SDValue();
...
static SDValue combineORToGREV(SDValue Op, SelectionDAG &DAG,
  ...
  if (VT == Subtarget.getXLenVT() || (Subtarget.is64Bit() && VT == MVT::i32)) {
    ...
    if (LHS && RHS && LHS->formsPairWith(*RHS)) {
      SDLoc DL(Op);
      return DAG.getNode(CrampISD::GREV, DL, VT, LHS->Op,
...
static SDValue combineORToGORC(SDValue Op, SelectionDAG &DAG,
  ...
  if (VT == Subtarget.getXLenVT() || (Subtarget.is64Bit() && VT == MVT::i32)) {
    ...
    auto MatchOROfReverse = [&](SDValue Reverse, SDValue X) {
      if (Reverse.getOpcode() == CrampISD::GREV && Reverse.getOperand(0) == X &&
          isa<ConstantSDNode>(Reverse.getOperand(1)) &&
          isPowerOf2_32(Reverse.getConstantOperandVal(1)))
        return DAG.getNode(CrampISD::GORC, DL, VT, X, Reverse.getOperand(1));
      ...
      if ((Reverse.getOpcode() == ISD::ROTL ||
        ...
        if (RotAmt == (VT.getSizeInBits() / 2))
          return DAG.getNode(CrampISD::GORC, DL, VT, X,
    ...
    if (LHS && RHS && LHS->formsPairWith(*RHS) && LHS->Op == OrOp1) {
      return DAG.getNode(CrampISD::GORC, DL, VT, LHS->Op,
                         DAG.getConstant(LHS->ShAmt, DL, VT));
    }
  }
  return SDValue();
...
static Optional<CrampBitmanipPat> matchSHFLPat(SDValue Op) {
  // These are the unshifted masks which we use to match bit-manipulation
  // patterns. They may be shifted left in certain circumstances.
  static const uint64_t BitmanipMasks[] = {
...
static SDValue combineORToSHFL(SDValue Op, SelectionDAG &DAG,
  ...
  SDLoc DL(Op);
  return DAG.getNode(CrampISD::SHFL, DL, VT, Match1->Op,
...
static SDValue combineROTR_ROTL_RORW_ROLW(SDNode *N, SelectionDAG &DAG,
                                          const CrampSubtarget &Subtarget) {
  bool IsWInstruction =
      N->getOpcode() == CrampISD::RORW || N->getOpcode() == CrampISD::ROLW;
  ...
  if (!Subtarget.hasStdExtZbp() || Src.getOpcode() != CrampISD::GREV)
    return SDValue();
...
static SDValue combineGREVI_GORCI(SDNode *N, SelectionDAG &DAG) {
  bool IsGORC = N->getOpcode() == CrampISD::GORC;
  assert((IsGORC || N->getOpcode() == CrampISD::GREV) && "Unexpected opcode");
  SDValue Src = N->getOperand(0);
...
static SDValue combineSelectAndUse(SDNode *N, SDValue Slct, SDValue OtherOp,
  ...
  if ((Slct.getOpcode() != ISD::SELECT &&
       Slct.getOpcode() != CrampISD::SELECT_CC) ||
      !Slct.hasOneUse())
    return SDValue();
  ...
  bool SwapSelectOps;
  unsigned OpOffset = Slct.getOpcode() == CrampISD::SELECT_CC ? 2 : 0;
  ...
  if (Slct.getOpcode() == CrampISD::SELECT_CC)
    return DAG.getNode(CrampISD::SELECT_CC, SDLoc(N), VT,
...
static SDValue performANDCombine(SDNode *N, SelectionDAG &DAG,
  ...
  // type legalization, we'll create CrampISD::SRLW and we can't recover it
...
static SDValue performXORCombine(SDNode *N, SelectionDAG &DAG) {
  ...
  if (N0.getOpcode() == CrampISD::SLLW &&
      isAllOnesConstant(N1) && isOneConstant(N0.getOperand(0)) &&
      TLI.isOperationLegal(ISD::ROTL, MVT::i64)) {
    SDLoc DL(N);
    return DAG.getNode(CrampISD::ROLW, DL, MVT::i64,
...
performSIGN_EXTEND_INREGCombine(SDNode *N, SelectionDAG &DAG,
  ...
  if (Src.getOpcode() == CrampISD::FMV_X_ANYEXTH &&
      cast<VTSDNode>(N->getOperand(1))->getVT().bitsGE(MVT::i16))
    return DAG.getNode(CrampISD::FMV_X_SIGNEXTH, SDLoc(N), VT,
                       Src.getOperand(0));
...
static SDValue combineADDSUB_VLToVWADDSUB_VL(SDNode *N, SelectionDAG &DAG,
                                             bool Commute = false) {
  assert((N->getOpcode() == CrampISD::ADD_VL ||
          N->getOpcode() == CrampISD::SUB_VL) &&
         "Unexpected opcode");
  bool IsAdd = N->getOpcode() == CrampISD::ADD_VL;
  SDValue Op0 = N->getOperand(0);
  ...
  if ((Op1.getOpcode() == CrampISD::VZEXT_VL ||
       Op1.getOpcode() == CrampISD::VSEXT_VL) &&
    ...
    unsigned WOpc;
    if (ExtOpc == CrampISD::VSEXT_VL)
      WOpc = IsAdd ? CrampISD::VWADD_W_VL : CrampISD::VWSUB_W_VL;
    else
      WOpc = IsAdd ? CrampISD::VWADDU_W_VL : CrampISD::VWSUBU_W_VL;
...
static SDValue combineVWADD_W_VL_VWSUB_W_VL(SDNode *N, SelectionDAG &DAG) {
  ...
  switch (N->getOpcode()) {
  default: llvm_unreachable("Unexpected opcode");
  case CrampISD::VWADD_W_VL:  VOpc = CrampISD::VWADD_VL;  break;
  case CrampISD::VWSUB_W_VL:  VOpc = CrampISD::VWSUB_VL;  break;
  case CrampISD::VWADDU_W_VL: VOpc = CrampISD::VWADDU_VL; break;
  case CrampISD::VWSUBU_W_VL: VOpc = CrampISD::VWSUBU_VL; break;
  }

  bool IsSigned = N->getOpcode() == CrampISD::VWADD_W_VL ||
                  N->getOpcode() == CrampISD::VWSUB_W_VL;

  SDLoc DL(N);

  // If the LHS is a sext or zext, we can narrow this op to the same size as
  // the RHS.
  if (((Op0.getOpcode() == CrampISD::VZEXT_VL && !IsSigned) ||
       (Op0.getOpcode() == CrampISD::VSEXT_VL && IsSigned)) &&
  ...
  bool IsAdd = N->getOpcode() == CrampISD::VWADD_W_VL ||
               N->getOpcode() == CrampISD::VWADDU_W_VL;

  // Look for splats on the left hand side of a vwadd(u).wv. We might be able
  // to commute and use a vwadd(u).vx instead.
  if (IsAdd && Op0.getOpcode() == CrampISD::VMV_V_X_VL &&
    ...
    Op0 = DAG.getNode(CrampISD::VMV_V_X_VL, DL, NarrowVT,
...
static SDValue combineMUL_VLToVWMUL_VL(SDNode *N, SelectionDAG &DAG,
                                       bool Commute) {
  assert(N->getOpcode() == CrampISD::MUL_VL && "Unexpected opcode");
  SDValue Op0 = N->getOperand(0);
  SDValue Op1 = N->getOperand(1);
  if (Commute)
    std::swap(Op0, Op1);

  bool IsSignExt = Op0.getOpcode() == CrampISD::VSEXT_VL;
  bool IsZeroExt = Op0.getOpcode() == CrampISD::VZEXT_VL;
  bool IsVWMULSU = IsSignExt && Op1.getOpcode() == CrampISD::VZEXT_VL;
  ...
  if (IsVWMULSU || Op0.getOpcode() == Op1.getOpcode()) {
    ...
    Op1 = Op1.getOperand(0);
  } else if (Op1.getOpcode() == CrampISD::VMV_V_X_VL) {
    ...
    Op1 = DAG.getNode(CrampISD::VMV_V_X_VL, DL, NarrowVT,
  ...
  // Re-introduce narrower extends if needed.
  unsigned ExtOpc = IsSignExt ? CrampISD::VSEXT_VL : CrampISD::VZEXT_VL;
  if (Op0.getValueType() != NarrowVT)
    Op0 = DAG.getNode(ExtOpc, DL, NarrowVT, Op0, Mask, VL);
  // vwmulsu requires second operand to be zero extended.
  ExtOpc = IsVWMULSU ? CrampISD::VZEXT_VL : ExtOpc;
  if (Op1.getValueType() != NarrowVT)
    Op1 = DAG.getNode(ExtOpc, DL, NarrowVT, Op1, Mask, VL);

  unsigned WMulOpc = CrampISD::VWMULSU_VL;
  if (!IsVWMULSU)
    WMulOpc = IsSignExt ? CrampISD::VWMUL_VL : CrampISD::VWMULU_VL;
  return DAG.getNode(WMulOpc, DL, VT, Op0, Op1, Mask, VL);
}

static CrampFPRndMode::RoundingMode matchRoundingOp(SDValue Op) {
  switch (Op.getOpcode()) {
  case ISD::FROUNDEVEN: return CrampFPRndMode::RNE;
  case ISD::FTRUNC:     return CrampFPRndMode::RTZ;
  case ISD::FFLOOR:     return CrampFPRndMode::RDN;
  case ISD::FCEIL:      return CrampFPRndMode::RUP;
  case ISD::FROUND:     return CrampFPRndMode::RMM;
  }

  return CrampFPRndMode::Invalid;
}
...
static SDValue performFP_TO_INTCombine(SDNode *N,
  ...
  CrampFPRndMode::RoundingMode FRM = matchRoundingOp(Src);
  if (FRM == CrampFPRndMode::Invalid)
    return SDValue();
...
static SDValue performBITREVERSECombine(SDNode *N, SelectionDAG &DAG,
  ...
  SDLoc DL(N);
  return DAG.getNode(CrampISD::GREV, DL, VT, Src.getOperand(0),
                     DAG.getConstant(7, DL, VT));
}
...
static unsigned negateFMAOpcode(unsigned Opcode, bool NegMul, bool NegAcc) {
  ...
  if (NegMul) {
    ...
    switch (Opcode) {
    default: llvm_unreachable("Unexpected opcode");
    case CrampISD::VFMADD_VL:  Opcode = CrampISD::VFNMSUB_VL; break;
    case CrampISD::VFNMSUB_VL: Opcode = CrampISD::VFMADD_VL;  break;
    case CrampISD::VFNMADD_VL: Opcode = CrampISD::VFMSUB_VL;  break;
    case CrampISD::VFMSUB_VL:  Opcode = CrampISD::VFNMADD_VL; break;
    }
    // clang-format on
  }
  ...
  if (NegAcc) {
    ...
    switch (Opcode) {
    default: llvm_unreachable("Unexpected opcode");
    case CrampISD::VFMADD_VL:  Opcode = CrampISD::VFMSUB_VL;  break;
    case CrampISD::VFMSUB_VL:  Opcode = CrampISD::VFMADD_VL;  break;
    case CrampISD::VFNMADD_VL: Opcode = CrampISD::VFNMSUB_VL; break;
    case CrampISD::VFNMSUB_VL: Opcode = CrampISD::VFNMADD_VL; break;
    }
    // clang-format on
  }
...
static bool combine_CC(SDValue &LHS, SDValue &RHS, SDValue &CC, const SDLoc &DL,
                       SelectionDAG &DAG, const CrampSubtarget &Subtarget) {
...
SDValue CrampTargetLowering::PerformDAGCombine(SDNode *N,
  ...
  default:
    break;
  case CrampISD::SplitF64: {
    SDValue Op0 = N->getOperand(0);
    // If the input to SplitF64 is just BuildPairF64 then the operation is
    // redundant. Instead, use BuildPairF64's operands directly.
    if (Op0->getOpcode() == CrampISD::BuildPairF64)
    ...
    if (!(Op0.getOpcode() == ISD::FNEG || Op0.getOpcode() == ISD::FABS) ||
      ...
      break;
    SDValue NewSplitF64 =
        DAG.getNode(CrampISD::SplitF64, DL, DAG.getVTList(MVT::i32, MVT::i32),
                    Op0.getOperand(0));
  ...
  case CrampISD::RORW:
  case CrampISD::ROLW: {
    if (N->getOpcode() == CrampISD::RORW || N->getOpcode() == CrampISD::ROLW) {
      // Only the lower 32 bits of LHS and lower 5 bits of RHS are read.
      if (SimplifyDemandedLowBitsHelper(0, 32) ||
          SimplifyDemandedLowBitsHelper(1, 5))
        return SDValue(N, 0);
  ...
  case CrampISD::BCOMPRESSW:
  case CrampISD::BDECOMPRESSW: {
  ...
  case CrampISD::FSLW: {
    bool IsWInstruction =
        N->getOpcode() == CrampISD::FSRW || N->getOpcode() == CrampISD::FSLW;
    unsigned BitWidth =
  ...
  case CrampISD::FMV_X_ANYEXTH:
  case CrampISD::FMV_X_ANYEXTW_RV64: {
    SDLoc DL(N);
    SDValue Op0 = N->getOperand(0);
    MVT VT = N->getSimpleValueType(0);
    ...
    if ((N->getOpcode() == CrampISD::FMV_X_ANYEXTW_RV64 &&
         Op0->getOpcode() == CrampISD::FMV_W_X_RV64) ||
        (N->getOpcode() == CrampISD::FMV_X_ANYEXTH &&
         Op0->getOpcode() == CrampISD::FMV_H_X)) {
    ...
    unsigned FPBits = N->getOpcode() == CrampISD::FMV_X_ANYEXTW_RV64 ? 32 : 16;
  ...
  case CrampISD::SELECT_CC: {
    ...
    if (combine_CC(LHS, RHS, CC, DL, DAG, Subtarget))
      return DAG.getNode(CrampISD::SELECT_CC, DL, N->getValueType(0),
  ...
  case CrampISD::BR_CC: {
    ...
    if (combine_CC(LHS, RHS, CC, DL, DAG, Subtarget))
      return DAG.getNode(CrampISD::BR_CC, DL, N->getValueType(0),
  ...
  case ISD::VP_SCATTER: {
    ...
    MVT XLenVT = Subtarget.getXLenVT();
    // Cramp indexed loads only support the "unsigned unscaled" addressing
  ...
  case CrampISD::SRA_VL:
  case CrampISD::SRL_VL:
  case CrampISD::SHL_VL: {
    SDValue ShAmt = N->getOperand(1);
    if (ShAmt.getOpcode() == CrampISD::SPLAT_VECTOR_SPLIT_I64_VL) {
      // We don't need the upper 32 bits of a 64-bit element for a shift amount.
      SDLoc DL(N);
      SDValue VL = N->getOperand(3);
      EVT VT = N->getValueType(0);
      ShAmt = DAG.getNode(CrampISD::VMV_V_X_VL, DL, VT, DAG.getUNDEF(VT),
  ...
  case CrampISD::VFNMSUB_VL: {
    ...
    auto invertIfNegative = [&Mask, &VL](SDValue &V) {
      if (V.getOpcode() == CrampISD::FNEG_VL && V.getOperand(1) == Mask &&
          V.getOperand(2) == VL) {
  ...
  case ISD::STORE: {
    ...
    // FIXME: Support FP.
    if (Val.getOpcode() == CrampISD::VMV_X_S) {
  ...
  case ISD::INTRINSIC_WO_CHAIN: {
    ...
    default:
      return SDValue();
    case Intrinsic::cramp_vcpop:
    case Intrinsic::cramp_vcpop_mask:
    case Intrinsic::cramp_vfirst:
    case Intrinsic::cramp_vfirst_mask: {
      SDValue VL = N->getOperand(2);
      if (IntNo == Intrinsic::cramp_vcpop_mask ||
          IntNo == Intrinsic::cramp_vfirst_mask)
        VL = N->getOperand(3);
      ...
      SDLoc DL(N);
      EVT VT = N->getValueType(0);
      if (IntNo == Intrinsic::cramp_vfirst ||
          IntNo == Intrinsic::cramp_vfirst_mask)
...
bool CrampTargetLowering::isDesirableToCommuteWithShift(
  ...
  if (Ty.isScalarInteger() &&
    ...
    if (C1 && C2) {
      ...
      // costs.
      int C1Cost = CrampMatInt::getIntMatCost(C1Int, Ty.getSizeInBits(),
                                              Subtarget.getFeatureBits(),
                                              /*CompressionCost*/true);
      int ShiftedC1Cost = CrampMatInt::getIntMatCost(
...
bool CrampTargetLowering::targetShrinkDemandedConstant(
...
void CrampTargetLowering::computeKnownBitsForTargetNode(const SDValue Op,
                                                        KnownBits &Known,
  ...
  case CrampISD::GORC: {
    if (auto *C = dyn_cast<ConstantSDNode>(Op.getOperand(1))) {
      ...
      bool IsGORC = Op.getOpcode() == CrampISD::GORC;
  ...
  case ISD::INTRINSIC_WO_CHAIN: {
    ...
    default:
      ...
      break;
    case Intrinsic::cramp_vsetvli:
    case Intrinsic::cramp_vsetvlimax:
    case Intrinsic::cramp_vsetvli_opt:
    case Intrinsic::cramp_vsetvlimax_opt:
...
unsigned CrampTargetLowering::ComputeNumSignBitsForTargetNode(
  ...
  case CrampISD::BDECOMPRESSW:
  case CrampISD::BFPW:
  case CrampISD::FCVT_W_RV64:
  case CrampISD::FCVT_WU_RV64:
  case CrampISD::STRICT_FCVT_W_RV64:
  case CrampISD::STRICT_FCVT_WU_RV64:
...
const Constant *
CrampTargetLowering::getTargetConstantFromLoad(LoadSDNode *Ld) const {
  ...
  if (Ptr.getOpcode() != CrampISD::ADD_LO ||
      Ptr.getOperand(0).getOpcode() != CrampISD::HI)
    return nullptr;
  ...
  if (!CNodeLo || CNodeLo->getTargetFlags() != CrampII::MO_LO ||
      !CNodeHi || CNodeHi->getTargetFlags() != CrampII::MO_HI)
    return nullptr;
...
static MachineBasicBlock *emitReadCycleWidePseudo(MachineInstr &MI,
  ...
  assert(MI.getOpcode() == Cramp::ReadCycleWide && "Unexpected instruction");
  ...
  Register ReadAgainReg = RegInfo.createVirtualRegister(&Cramp::GPRRegClass);
  Register LoReg = MI.getOperand(0).getReg();
  Register HiReg = MI.getOperand(1).getReg();
  DebugLoc DL = MI.getDebugLoc();

  const TargetInstrInfo *TII = MF.getSubtarget().getInstrInfo();
  BuildMI(LoopMBB, DL, TII->get(Cramp::CSRRS), HiReg)
      .addImm(CrampSysReg::lookupSysRegByName("CYCLEH")->Encoding)
      .addReg(Cramp::X0);
  BuildMI(LoopMBB, DL, TII->get(Cramp::CSRRS), LoReg)
      .addImm(CrampSysReg::lookupSysRegByName("CYCLE")->Encoding)
      .addReg(Cramp::X0);
  BuildMI(LoopMBB, DL, TII->get(Cramp::CSRRS), ReadAgainReg)
      .addImm(CrampSysReg::lookupSysRegByName("CYCLEH")->Encoding)
      .addReg(Cramp::X0);
...
static MachineBasicBlock *emitSplitF64Pseudo(MachineInstr &MI,
                                             MachineBasicBlock *BB) {
  assert(MI.getOpcode() == Cramp::SplitF64Pseudo && "Unexpected instruction");
  ...
  const TargetRegisterClass *SrcRC = &Cramp::FPR64RegClass;
  int FI = MF.getInfo<CrampMachineFunctionInfo>()->getMoveF64FrameIndex(MF);
  ...
  BuildMI(*BB, MI, DL, TII.get(Cramp::LW), LoReg)
      .addFrameIndex(FI)
      .addImm(0)
      .addMemOperand(MMOLo);
  BuildMI(*BB, MI, DL, TII.get(Cramp::LW), HiReg)
      .addFrameIndex(FI)
      .addImm(4)
      .addMemOperand(MMOHi);
...
static MachineBasicBlock *emitBuildPairF64Pseudo(MachineInstr &MI,
                                                 MachineBasicBlock *BB) {
  assert(MI.getOpcode() == Cramp::BuildPairF64Pseudo &&
  ...
  const TargetRegisterClass *DstRC = &Cramp::FPR64RegClass;
  int FI = MF.getInfo<CrampMachineFunctionInfo>()->getMoveF64FrameIndex(MF);
  ...
  BuildMI(*BB, MI, DL, TII.get(Cramp::SW))
...
static bool isSelectPseudo(MachineInstr &MI) {
  ...
  default:
    return false;
  case Cramp::Select_GPR_Using_CC_GPR:
  case Cramp::Select_FPR16_Using_CC_GPR:
  case Cramp::Select_FPR32_Using_CC_GPR:
  case Cramp::Select_FPR64_Using_CC_GPR:
    return true;
  }
}
...
static MachineBasicBlock *emitQuietFCMP(MachineInstr &MI, MachineBasicBlock *BB,
  ...
  Register SavedFFlags = MRI.createVirtualRegister(&Cramp::GPRRegClass);
  const TargetInstrInfo &TII = *BB->getParent()->getSubtarget().getInstrInfo();

  // Save the current FFLAGS.
  BuildMI(*BB, MI, DL, TII.get(Cramp::ReadFFLAGS), SavedFFlags);
  ...
  // Restore the FFLAGS.
  BuildMI(*BB, MI, DL, TII.get(Cramp::WriteFFLAGS))
      .addReg(SavedFFlags, RegState::Kill);

  // Issue a dummy FEQ opcode to raise exception for signaling NaNs.
  auto MIB2 = BuildMI(*BB, MI, DL, TII.get(EqOpcode), Cramp::X0)
...
EmitLoweredCascadedSelect(MachineInstr &First, MachineInstr &Second,
  ...
  const CrampInstrInfo &TII = *Subtarget.getInstrInfo();
  ...
  auto FirstCC = static_cast<CrampCC::CondCode>(First.getOperand(3).getImm());
  ...
  auto SecondCC = static_cast<CrampCC::CondCode>(Second.getOperand(3).getImm());
  // Insert appropriate branch.
  ...
  BuildMI(*SinkMBB, SinkMBB->begin(), DL, TII.get(Cramp::PHI), DestReg)
      .addReg(Op2Reg4)
      .addMBB(ThisMBB)
      .addReg(Op1Reg4)
      .addMBB(FirstMBB)
...
static MachineBasicBlock *emitSelectPseudo(MachineInstr &MI,
  ...
  auto CC = static_cast<CrampCC::CondCode>(MI.getOperand(3).getImm());
  ...
  if (MI.getOpcode() != Cramp::Select_GPR_Using_CC_GPR && Next != BB->end() &&
  ...
  while (SelectMBBI != SelectEnd) {
    ...
    if (isSelectPseudo(*SelectMBBI)) {
      ...
      BuildMI(*TailMBB, InsertionPoint, SelectMBBI->getDebugLoc(),
              TII.get(Cramp::PHI), SelectMBBI->getOperand(0).getReg())
...
static MachineBasicBlock *emitAlignedBzero4Pseudo(MachineInstr &MI,
                                                  MachineBasicBlock *BB) {
  assert(MI.getOpcode() == Cramp::AlignedBzero4 && "Unexpected instruction");
...
CrampTargetLowering::EmitInstrWithCustomInserter(MachineInstr &MI,
  ...
  case Cramp::ReadCycleWide:
    assert(!Subtarget.is64Bit() &&
           "ReadCycleWrite is only to be used on cramp32");
  ...
  case Cramp::BuildPairF64Pseudo:
    return emitBuildPairF64Pseudo(MI, BB);
  case Cramp::SplitF64Pseudo:
    return emitSplitF64Pseudo(MI, BB);
  case Cramp::PseudoQuietFLE_H:
    return emitQuietFCMP(MI, BB, Cramp::FLE_H, Cramp::FEQ_H, Subtarget);
  case Cramp::PseudoQuietFLT_H:
    return emitQuietFCMP(MI, BB, Cramp::FLT_H, Cramp::FEQ_H, Subtarget);
  case Cramp::PseudoQuietFLE_S:
    return emitQuietFCMP(MI, BB, Cramp::FLE_S, Cramp::FEQ_S, Subtarget);
  case Cramp::PseudoQuietFLT_S:
    return emitQuietFCMP(MI, BB, Cramp::FLT_S, Cramp::FEQ_S, Subtarget);
  case Cramp::PseudoQuietFLE_D:
    return emitQuietFCMP(MI, BB, Cramp::FLE_D, Cramp::FEQ_D, Subtarget);
  case Cramp::PseudoQuietFLT_D:
...
void CrampTargetLowering::AdjustInstrPostInstrSelection(MachineInstr &MI,
                                                        SDNode *Node) const {
  // Add FRM dependency to any instructions with dynamic rounding mode.
  unsigned Opc = MI.getOpcode();
  auto Idx = Cramp::getNamedOperandIdx(Opc, Cramp::OpName::frm);
  if (Idx < 0)
    return;
  if (MI.getOperand(Idx).getImm() != CrampFPRndMode::DYN)
    return;
  // If the instruction already reads FRM, don't add another read.
  if (MI.readsRegister(Cramp::FRM))
    return;
  MI.addOperand(
      MachineOperand::CreateReg(Cramp::FRM, /*isDef*/ false, /*isImp*/ true));
}
...
static const MCPhysReg ArgGPRs[] = {
  Cramp::X10, Cramp::X11, Cramp::X12, Cramp::X13,
  Cramp::X14, Cramp::X15, Cramp::X16, Cramp::X17
};
static const MCPhysReg ArgFPR16s[] = {
  Cramp::F10_H, Cramp::F11_H, Cramp::F12_H, Cramp::F13_H,
  Cramp::F14_H, Cramp::F15_H, Cramp::F16_H, Cramp::F17_H
};
static const MCPhysReg ArgFPR32s[] = {
  Cramp::F10_F, Cramp::F11_F, Cramp::F12_F, Cramp::F13_F,
  Cramp::F14_F, Cramp::F15_F, Cramp::F16_F, Cramp::F17_F
};
static const MCPhysReg ArgFPR64s[] = {
  Cramp::F10_D, Cramp::F11_D, Cramp::F12_D, Cramp::F13_D,
  Cramp::F14_D, Cramp::F15_D, Cramp::F16_D, Cramp::F17_D
};
// This is an interim calling convention and it may be changed in the future.
static const MCPhysReg ArgVRs[] = {
    Cramp::V8,  Cramp::V9,  Cramp::V10, Cramp::V11, Cramp::V12, Cramp::V13,
    Cramp::V14, Cramp::V15, Cramp::V16, Cramp::V17, Cramp::V18, Cramp::V19,
    Cramp::V20, Cramp::V21, Cramp::V22, Cramp::V23};
static const MCPhysReg ArgVRM2s[] = {Cramp::V8M2,  Cramp::V10M2, Cramp::V12M2,
                                     Cramp::V14M2, Cramp::V16M2, Cramp::V18M2,
                                     Cramp::V20M2, Cramp::V22M2};
static const MCPhysReg ArgVRM4s[] = {Cramp::V8M4, Cramp::V12M4, Cramp::V16M4,
                                     Cramp::V20M4};
static const MCPhysReg ArgVRM8s[] = {Cramp::V8M8, Cramp::V16M8};

// Pass a 2*XLEN argument that has been split into two XLEN values through
// registers or the stack as necessary.
static bool CC_CrampAssign2XLen(unsigned XLen, CCState &State, CCValAssign VA1,
...
static unsigned allocateRVVReg(MVT ValVT, unsigned ValNo,
                               ...
                               CCState &State, const CrampTargetLowering &TLI) {
  const TargetRegisterClass *RC = TLI.getRegClassFor(ValVT);
  if (RC == &Cramp::VRRegClass) {
    // Assign the first mask argument to V0.
    // This is an interim calling convention and it may be changed in the
    // future.
    if (FirstMaskArgument && ValNo == *FirstMaskArgument)
      return State.AllocateReg(Cramp::V0);
    return State.AllocateReg(ArgVRs);
  }
  if (RC == &Cramp::VRM2RegClass)
    return State.AllocateReg(ArgVRM2s);
  if (RC == &Cramp::VRM4RegClass)
    return State.AllocateReg(ArgVRM4s);
  if (RC == &Cramp::VRM8RegClass)
    return State.AllocateReg(ArgVRM8s);
  llvm_unreachable("Unhandled register class for ValueType");
}

// Implements the RISC-V calling convention. Returns true upon failure.
static bool CC_Cramp(const DataLayout &DL, CrampABI::ABI ABI, unsigned ValNo,
                     MVT ValVT, MVT LocVT, CCValAssign::LocInfo LocInfo,
                     ISD::ArgFlagsTy ArgFlags, CCState &State, bool IsFixed,
                     bool IsRet, Type *OrigTy, const CrampTargetLowering &TLI,
                     Optional<unsigned> FirstMaskArgument) {
  ...
  if (ValVT.isScalarInteger() && ArgFlags.isSplitEnd() &&
    ...
    return CC_CrampAssign2XLen(XLen, State, VA, AF, ValNo, ValVT, LocVT,
                               ArgFlags);
  }
...
void CrampTargetLowering::analyzeInputArgs(
    MachineFunction &MF, CCState &CCInfo,
    const SmallVectorImpl<ISD::InputArg> &Ins, bool IsRet,
    CrampCCAssignFn Fn) const {
  ...
  for (unsigned i = 0; i != NumArgs; ++i) {
    ...
    CrampABI::ABI ABI = MF.getSubtarget<CrampSubtarget>().getTargetABI();
...
void CrampTargetLowering::analyzeOutputArgs(
    MachineFunction &MF, CCState &CCInfo,
    const SmallVectorImpl<ISD::OutputArg> &Outs, bool IsRet,
    CallLoweringInfo *CLI, CrampCCAssignFn Fn) const {
...
static SDValue convertLocVTToValVT(SelectionDAG &DAG, SDValue Val,
  ...
  case CCValAssign::BCvt:
    if (VA.getLocVT().isInteger() && VA.getValVT() == MVT::f16)
      Val = DAG.getNode(CrampISD::FMV_H_X, DL, MVT::f16, Val);
    else if (VA.getLocVT() == MVT::i64 && VA.getValVT() == MVT::f32)
      Val = DAG.getNode(CrampISD::FMV_W_X_RV64, DL, MVT::f32, Val);
    else
...
static SDValue convertValVTToLocVT(SelectionDAG &DAG, SDValue Val,
  ...
  case CCValAssign::BCvt:
    if (VA.getLocVT().isInteger() && VA.getValVT() == MVT::f16)
      Val = DAG.getNode(CrampISD::FMV_X_ANYEXTH, DL, VA.getLocVT(), Val);
    else if (VA.getLocVT() == MVT::i64 && VA.getValVT() == MVT::f32)
      Val = DAG.getNode(CrampISD::FMV_X_ANYEXTW_RV64, DL, MVT::i64, Val);
    else
...
static SDValue unpackF64OnRV32DSoftABI(SelectionDAG &DAG, SDValue Chain,
  ...
  Register LoVReg = RegInfo.createVirtualRegister(&Cramp::GPRRegClass);
  RegInfo.addLiveIn(VA.getLocReg(), LoVReg);
  SDValue Lo = DAG.getCopyFromReg(Chain, DL, LoVReg, MVT::i32);
  SDValue Hi;
  if (VA.getLocReg() == Cramp::X17) {
    ...
    Register HiVReg = RegInfo.createVirtualRegister(&Cramp::GPRRegClass);
    RegInfo.addLiveIn(VA.getLocReg() + 1, HiVReg);
    Hi = DAG.getCopyFromReg(Chain, DL, HiVReg, MVT::i32);
  }
  return DAG.getNode(CrampISD::BuildPairF64, DL, MVT::f64, Lo, Hi);
}

// FastCC has less than 1% performance improvement for some particular
// benchmark. But theoretically, it may has benenfit for some cases.
static bool CC_Cramp_FastCC(const DataLayout &DL, CrampABI::ABI ABI,
  ...
  static const MCPhysReg GPRList[] = {
      Cramp::X10, Cramp::X11, Cramp::X12, Cramp::X13, Cramp::X14,
      Cramp::X15, Cramp::X16, Cramp::X17, Cramp::X7,  Cramp::X28,
      Cramp::X29, Cramp::X30, Cramp::X31};
  ...
  if (LocVT == MVT::f16) {
    static const MCPhysReg FPR16List[] = {
        Cramp::F10_H, Cramp::F11_H, Cramp::F12_H, Cramp::F13_H, Cramp::F14_H,
        Cramp::F15_H, Cramp::F16_H, Cramp::F17_H, Cramp::F0_H,  Cramp::F1_H,
        Cramp::F2_H,  Cramp::F3_H,  Cramp::F4_H,  Cramp::F5_H,  Cramp::F6_H,
        Cramp::F7_H,  Cramp::F28_H, Cramp::F29_H, Cramp::F30_H, Cramp::F31_H};
  ...
  if (LocVT == MVT::f32) {
    static const MCPhysReg FPR32List[] = {
        Cramp::F10_F, Cramp::F11_F, Cramp::F12_F, Cramp::F13_F, Cramp::F14_F,
        Cramp::F15_F, Cramp::F16_F, Cramp::F17_F, Cramp::F0_F,  Cramp::F1_F,
        Cramp::F2_F,  Cramp::F3_F,  Cramp::F4_F,  Cramp::F5_F,  Cramp::F6_F,
        Cramp::F7_F,  Cramp::F28_F, Cramp::F29_F, Cramp::F30_F, Cramp::F31_F};
  ...
  if (LocVT == MVT::f64) {
    static const MCPhysReg FPR64List[] = {
        Cramp::F10_D, Cramp::F11_D, Cramp::F12_D, Cramp::F13_D, Cramp::F14_D,
        Cramp::F15_D, Cramp::F16_D, Cramp::F17_D, Cramp::F0_D,  Cramp::F1_D,
        Cramp::F2_D,  Cramp::F3_D,  Cramp::F4_D,  Cramp::F5_D,  Cramp::F6_D,
        Cramp::F7_D,  Cramp::F28_D, Cramp::F29_D, Cramp::F30_D, Cramp::F31_D};
...
static bool CC_Cramp_GHC(unsigned ValNo, MVT ValVT, MVT LocVT,
                         CCValAssign::LocInfo LocInfo,
                         ISD::ArgFlagsTy ArgFlags, CCState &State) {
  ...
  if (LocVT == MVT::i32 || LocVT == MVT::i64) {
    ...
    static const MCPhysReg GPRList[] = {
        Cramp::X9, Cramp::X18, Cramp::X19, Cramp::X20, Cramp::X21, Cramp::X22,
        Cramp::X23, Cramp::X24, Cramp::X25, Cramp::X26, Cramp::X27};
    if (unsigned Reg = State.AllocateReg(GPRList)) {
      State.addLoc(CCValAssign::getReg(ValNo, ValVT, Reg, LocVT, LocInfo));
      return false;
    }
  ...
  if (LocVT == MVT::f32) {
    ...
    //                        fs0 ... fs5
    static const MCPhysReg FPR32List[] = {Cramp::F8_F, Cramp::F9_F,
                                          Cramp::F18_F, Cramp::F19_F,
                                          Cramp::F20_F, Cramp::F21_F};
    if (unsigned Reg = State.AllocateReg(FPR32List)) {
      State.addLoc(CCValAssign::getReg(ValNo, ValVT, Reg, LocVT, LocInfo));
      return false;
    }
  ...
  if (LocVT == MVT::f64) {
    ...
    //                        fs6 ... fs11
    static const MCPhysReg FPR64List[] = {Cramp::F22_D, Cramp::F23_D,
                                          Cramp::F24_D, Cramp::F25_D,
                                          Cramp::F26_D, Cramp::F27_D};
    if (unsigned Reg = State.AllocateReg(FPR64List)) {
      State.addLoc(CCValAssign::getReg(ValNo, ValVT, Reg, LocVT, LocInfo));
      return false;
    }
...
SDValue CrampTargetLowering::LowerFormalArguments(
  ...
  case CallingConv::GHC:
    if (!MF.getSubtarget().getFeatureBits()[Cramp::FeatureStdExtF] ||
        !MF.getSubtarget().getFeatureBits()[Cramp::FeatureStdExtD])
      report_fatal_error(
  ...
  if (CallConv == CallingConv::GHC)
    CCInfo.AnalyzeFormalArguments(Ins, CC_Cramp_GHC);
  else
    analyzeInputArgs(MF, CCInfo, Ins, /*IsRet=*/false,
                     CallConv == CallingConv::Fast ? CC_Cramp_FastCC
                                                   : CC_Cramp);
  ...
  if (IsVarArg) {
    ...
    const TargetRegisterClass *RC = &Cramp::GPRRegClass;
    MachineFrameInfo &MFI = MF.getFrameInfo();
    MachineRegisterInfo &RegInfo = MF.getRegInfo();
    CrampMachineFunctionInfo *RVFI = MF.getInfo<CrampMachineFunctionInfo>();
...
bool CrampTargetLowering::isEligibleForTailCallOptimization(
...
SDValue CrampTargetLowering::LowerCall(CallLoweringInfo &CLI,
  ...
  if (CallConv == CallingConv::GHC)
    ArgCCInfo.AnalyzeCallOperands(Outs, CC_Cramp_GHC);
  else
  ...
  for (unsigned i = 0, j = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (IsF64OnRV32DSoftABI && VA.isRegLoc()) {
      SDValue SplitF64 = DAG.getNode(
          CrampISD::SplitF64, DL, DAG.getVTList(MVT::i32, MVT::i32), ArgValue);
      ...
      if (RegLo == Cramp::X17) {
        ...
        if (!StackPtr.getNode())
          StackPtr = DAG.getCopyFromReg(Chain, DL, Cramp::X2, PtrVT);
        // Emit the store.
        MemOpChains.push_back(
            DAG.getStore(Chain, DL, Hi, StackPtr, MachinePointerInfo()));
      } else {
        // Second half of f64 is passed in another GPR.
        assert(RegLo < Cramp::X31 && "Invalid register pair");
  ...
  if (!IsTailCall &&
      MF.getSubtarget<CrampSubtarget>().isRegisterReservedByUser(Cramp::X1))
  ...
  if (GlobalAddressSDNode *S = dyn_cast<GlobalAddressSDNode>(Callee)) {
    ...
    unsigned OpFlags = CrampII::MO_CALL;
    if (!getTargetMachine().shouldAssumeDSOLocal(*GV->getParent(), GV))
      OpFlags = CrampII::MO_PLT;
  ...
  if (IsTailCall) {
    ...
    return DAG.getNode(CrampISD::TAIL, DL, NodeTys, Ops);
  }

  Chain = DAG.getNode(CrampISD::CALL, DL, NodeTys, Ops);
  ...
  analyzeInputArgs(MF, RetCCInfo, Ins, /*IsRet=*/true, CC_Cramp);
  ...
  for (auto &VA : RVLocs) {
    ...
    if (VA.getLocVT() == MVT::i32 && VA.getValVT() == MVT::f64) {
      ...
      RetValue = DAG.getNode(CrampISD::BuildPairF64, DL, MVT::f64, RetValue,
                             RetValue2);
    }
...
bool CrampTargetLowering::CanLowerReturn(
  ...
  for (unsigned i = 0, e = Outs.size(); i != e; ++i) {
    ...
    ISD::ArgFlagsTy ArgFlags = Outs[i].Flags;
    CrampABI::ABI ABI = MF.getSubtarget<CrampSubtarget>().getTargetABI();
    if (CC_Cramp(MF.getDataLayout(), ABI, i, VT, VT, CCValAssign::Full,
...
SDValue
CrampTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                                 bool IsVarArg,
                                 const SmallVectorImpl<ISD::OutputArg> &Outs,
                                 const SmallVectorImpl<SDValue> &OutVals,
                                 const SDLoc &DL, SelectionDAG &DAG) const {
  const MachineFunction &MF = DAG.getMachineFunction();
  const CrampSubtarget &STI = MF.getSubtarget<CrampSubtarget>();
  ...
  for (unsigned i = 0, e = RVLocs.size(); i < e; ++i) {
    ...
    if (VA.getLocVT() == MVT::i32 && VA.getValVT() == MVT::f64) {
      ...
      SDValue SplitF64 = DAG.getNode(CrampISD::SplitF64, DL,
  ...
  unsigned RetOpc = CrampISD::RET_FLAG;
  ...
  if (Func.hasFnAttribute("interrupt")) {
    ...
    if (Kind == "user")
      RetOpc = CrampISD::URET_FLAG;
    else if (Kind == "supervisor")
      RetOpc = CrampISD::SRET_FLAG;
    else
      RetOpc = CrampISD::MRET_FLAG;
  }

  return DAG.getNode(RetOpc, DL, MVT::Other, RetOps);
}

void CrampTargetLowering::validateCCReservedRegs(
...
bool CrampTargetLowering::mayBeEmittedAsTailCall(const CallInst *CI) const {
  return CI->isTailCall();
}

const char *CrampTargetLowering::getTargetNodeName(unsigned Opcode) const {
#define NODE_NAME_CASE(NODE)                                                   \
  case CrampISD::NODE:                                                         \
    return "CrampISD::" #NODE;
  // clang-format off
  switch ((CrampISD::NodeType)Opcode) {
  case CrampISD::FIRST_NUMBER:
    break;
...
CrampTargetLowering::ConstraintType
CrampTargetLowering::getConstraintType(StringRef Constraint) const {
...
CrampTargetLowering::getRegForInlineAsmConstraint(const TargetRegisterInfo *TRI,
                                                  StringRef Constraint,
                                                  MVT VT) const {
  // First, see if this is a constraint that directly corresponds to a
  // Cramp register class.
  if (Constraint.size() == 1) {
    switch (Constraint[0]) {
    case 'r':
      ...
      if (VT.isVector())
        break;
      return std::make_pair(0U, &Cramp::GPRRegClass);
    case 'f':
      if (Subtarget.hasStdExtZfh() && VT == MVT::f16)
        return std::make_pair(0U, &Cramp::FPR16RegClass);
      if (Subtarget.hasStdExtF() && VT == MVT::f32)
        return std::make_pair(0U, &Cramp::FPR32RegClass);
      if (Subtarget.hasStdExtD() && VT == MVT::f64)
        return std::make_pair(0U, &Cramp::FPR64RegClass);
      break;
    default:
      break;
    }
  } else if (Constraint == "vr") {
    for (const auto *RC : {&Cramp::VRRegClass, &Cramp::VRM2RegClass,
                           &Cramp::VRM4RegClass, &Cramp::VRM8RegClass}) {
      if (TRI->isTypeLegalForClass(*RC, VT.SimpleTy))
        return std::make_pair(0U, RC);
    }
  } else if (Constraint == "vm") {
    if (TRI->isTypeLegalForClass(Cramp::VMV0RegClass, VT.SimpleTy))
      return std::make_pair(0U, &Cramp::VMV0RegClass);
  }
  ...
  unsigned XRegFromAlias = StringSwitch<unsigned>(Constraint.lower())
                               ...
                               .Case("{t5}", Cramp::X30)
                               .Case("{t6}", Cramp::X31)
                               .Default(Cramp::NoRegister);
  if (XRegFromAlias != Cramp::NoRegister)
    return std::make_pair(XRegFromAlias, &Cramp::GPRRegClass);
  ...
  if (Subtarget.hasStdExtF()) {
    unsigned FReg = StringSwitch<unsigned>(Constraint.lower())
                        .Cases("{f0}", "{ft0}", Cramp::F0_F)
                        .Cases("{f1}", "{ft1}", Cramp::F1_F)
                        .Cases("{f2}", "{ft2}", Cramp::F2_F)
                        .Cases("{f3}", "{ft3}", Cramp::F3_F)
                        .Cases("{f4}", "{ft4}", Cramp::F4_F)
                        .Cases("{f5}", "{ft5}", Cramp::F5_F)
                        .Cases("{f6}", "{ft6}", Cramp::F6_F)
                        .Cases("{f7}", "{ft7}", Cramp::F7_F)
                        .Cases("{f8}", "{fs0}", Cramp::F8_F)
                        .Cases("{f9}", "{fs1}", Cramp::F9_F)
                        .Cases("{f10}", "{fa0}", Cramp::F10_F)
                        .Cases("{f11}", "{fa1}", Cramp::F11_F)
                        .Cases("{f12}", "{fa2}", Cramp::F12_F)
                        .Cases("{f13}", "{fa3}", Cramp::F13_F)
                        .Cases("{f14}", "{fa4}", Cramp::F14_F)
                        .Cases("{f15}", "{fa5}", Cramp::F15_F)
                        .Cases("{f16}", "{fa6}", Cramp::F16_F)
                        .Cases("{f17}", "{fa7}", Cramp::F17_F)
                        .Cases("{f18}", "{fs2}", Cramp::F18_F)
                        .Cases("{f19}", "{fs3}", Cramp::F19_F)
                        .Cases("{f20}", "{fs4}", Cramp::F20_F)
                        .Cases("{f21}", "{fs5}", Cramp::F21_F)
                        .Cases("{f22}", "{fs6}", Cramp::F22_F)
                        .Cases("{f23}", "{fs7}", Cramp::F23_F)
                        .Cases("{f24}", "{fs8}", Cramp::F24_F)
                        .Cases("{f25}", "{fs9}", Cramp::F25_F)
                        .Cases("{f26}", "{fs10}", Cramp::F26_F)
                        .Cases("{f27}", "{fs11}", Cramp::F27_F)
                        .Cases("{f28}", "{ft8}", Cramp::F28_F)
                        .Cases("{f29}", "{ft9}", Cramp::F29_F)
                        .Cases("{f30}", "{ft10}", Cramp::F30_F)
                        .Cases("{f31}", "{ft11}", Cramp::F31_F)
                        .Default(Cramp::NoRegister);
    if (FReg != Cramp::NoRegister) {
      assert(Cramp::F0_F <= FReg && FReg <= Cramp::F31_F && "Unknown fp-reg");
      if (Subtarget.hasStdExtD() && (VT == MVT::f64 || VT == MVT::Other)) {
        unsigned RegNo = FReg - Cramp::F0_F;
        unsigned DReg = Cramp::F0_D + RegNo;
        return std::make_pair(DReg, &Cramp::FPR64RegClass);
      }
      if (VT == MVT::f32 || VT == MVT::Other)
        return std::make_pair(FReg, &Cramp::FPR32RegClass);
      if (Subtarget.hasStdExtZfh() && VT == MVT::f16) {
        unsigned RegNo = FReg - Cramp::F0_F;
        unsigned HReg = Cramp::F0_H + RegNo;
        return std::make_pair(HReg, &Cramp::FPR16RegClass);
      }
    }
  }
  ...
  if (Subtarget.hasVInstructions()) {
    Register VReg = StringSwitch<Register>(Constraint.lower())
                        ...
                        .Case("{v31}", Cramp::V31)
                        .Default(Cramp::NoRegister);
    if (VReg != Cramp::NoRegister) {
      if (TRI->isTypeLegalForClass(Cramp::VMRegClass, VT.SimpleTy))
        return std::make_pair(VReg, &Cramp::VMRegClass);
      if (TRI->isTypeLegalForClass(Cramp::VRRegClass, VT.SimpleTy))
        return std::make_pair(VReg, &Cramp::VRRegClass);
      for (const auto *RC :
           {&Cramp::VRM2RegClass, &Cramp::VRM4RegClass, &Cramp::VRM8RegClass}) {
        if (TRI->isTypeLegalForClass(*RC, VT.SimpleTy)) {
          VReg = TRI->getMatchingSuperReg(VReg, Cramp::sub_vrm1_0, RC);
  ...
  if (Res.second == &Cramp::GPRF16RegClass ||
      Res.second == &Cramp::GPRF32RegClass ||
      Res.second == &Cramp::GPRF64RegClass)
    return std::make_pair(Res.first, &Cramp::GPRRegClass);

  return Res;
}

unsigned
CrampTargetLowering::getInlineAsmMemConstraint(StringRef ConstraintCode) const {
...
void CrampTargetLowering::LowerAsmOperandForConstraint(
...
Instruction *CrampTargetLowering::emitLeadingFence(IRBuilderBase &Builder,
                                                   Instruction *Inst,
...
Instruction *CrampTargetLowering::emitTrailingFence(IRBuilderBase &Builder,
                                                    Instruction *Inst,
                                                    AtomicOrdering Ord) const {
...
CrampTargetLowering::shouldExpandAtomicRMWInIR(AtomicRMWInst *AI) const {
...
getIntrinsicForMaskedAtomicRMWBinOp(unsigned XLen, AtomicRMWInst::BinOp BinOp) {
  if (XLen == 32) {
    ...
    case AtomicRMWInst::Xchg:
      return Intrinsic::cramp_masked_atomicrmw_xchg_i32;
    case AtomicRMWInst::Add:
      return Intrinsic::cramp_masked_atomicrmw_add_i32;
    case AtomicRMWInst::Sub:
      return Intrinsic::cramp_masked_atomicrmw_sub_i32;
    case AtomicRMWInst::Nand:
      return Intrinsic::cramp_masked_atomicrmw_nand_i32;
    case AtomicRMWInst::Max:
      return Intrinsic::cramp_masked_atomicrmw_max_i32;
    case AtomicRMWInst::Min:
      return Intrinsic::cramp_masked_atomicrmw_min_i32;
    case AtomicRMWInst::UMax:
      return Intrinsic::cramp_masked_atomicrmw_umax_i32;
    case AtomicRMWInst::UMin:
      return Intrinsic::cramp_masked_atomicrmw_umin_i32;
    }
  }
  ...
  if (XLen == 64) {
    ...
    default:
      llvm_unreachable("Unexpected AtomicRMW BinOp");
    case AtomicRMWInst::Xchg:
      return Intrinsic::cramp_masked_atomicrmw_xchg_i64;
    case AtomicRMWInst::Add:
      return Intrinsic::cramp_masked_atomicrmw_add_i64;
    case AtomicRMWInst::Sub:
      return Intrinsic::cramp_masked_atomicrmw_sub_i64;
    case AtomicRMWInst::Nand:
      return Intrinsic::cramp_masked_atomicrmw_nand_i64;
    case AtomicRMWInst::Max:
      return Intrinsic::cramp_masked_atomicrmw_max_i64;
    case AtomicRMWInst::Min:
      return Intrinsic::cramp_masked_atomicrmw_min_i64;
    case AtomicRMWInst::UMax:
      return Intrinsic::cramp_masked_atomicrmw_umax_i64;
    case AtomicRMWInst::UMin:
      return Intrinsic::cramp_masked_atomicrmw_umin_i64;
    }
  }
...
Value *CrampTargetLowering::emitMaskedAtomicRMWIntrinsic(
...
TargetLowering::AtomicExpansionKind
CrampTargetLowering::shouldExpandAtomicCmpXchgInIR(
...
Value *CrampTargetLowering::emitMaskedAtomicCmpXchgIntrinsic(
    IRBuilderBase &Builder, AtomicCmpXchgInst *CI, Value *AlignedAddr,
    Value *CmpVal, Value *NewVal, Value *Mask, AtomicOrdering Ord) const {
  unsigned XLen = Subtarget.getXLen();
  Value *Ordering = Builder.getIntN(XLen, static_cast<uint64_t>(Ord));
  Intrinsic::ID CmpXchgIntrID = Intrinsic::cramp_masked_cmpxchg_i32;
  if (XLen == 64) {
    CmpVal = Builder.CreateSExt(CmpVal, Builder.getInt64Ty());
    NewVal = Builder.CreateSExt(NewVal, Builder.getInt64Ty());
    Mask = Builder.CreateSExt(Mask, Builder.getInt64Ty());
    CmpXchgIntrID = Intrinsic::cramp_masked_cmpxchg_i64;
  }
  Type *Tys[] = {AlignedAddr->getType()};
...
bool CrampTargetLowering::shouldRemoveExtendFromGSIndex(EVT IndexVT,
                                                        EVT DataVT) const {
  return false;
}

bool CrampTargetLowering::shouldConvertFpToSat(unsigned Op, EVT FPVT,
                                               EVT VT) const {
...
unsigned CrampTargetLowering::getJumpTableEncoding() const {
...
const MCExpr *CrampTargetLowering::LowerCustomJumpTableEntry(
...
bool CrampTargetLowering::isVScaleKnownToBeAPowerOfTwo() const {
  ...
  static_assert(Cramp::RVVBitsPerBlock == 64,
                "RVVBitsPerBlock changed, audit needed");
  return true;
}

bool CrampTargetLowering::isFMAFasterThanFMulAndFAdd(const MachineFunction &MF,
                                                     EVT VT) const {
  VT = VT.getScalarType();
...
Register CrampTargetLowering::getExceptionPointerRegister(
    const Constant *PersonalityFn) const {
  return Cramp::X10;
}

Register CrampTargetLowering::getExceptionSelectorRegister(
    const Constant *PersonalityFn) const {
  return Cramp::X11;
}

bool CrampTargetLowering::shouldExtendTypeInLibCall(EVT Type) const {
  // Return false to suppress the unnecessary extensions if the LibCall
  // arguments or return value is f32 type for LP64 ABI.
  CrampABI::ABI ABI = Subtarget.getTargetABI();
  if (ABI == CrampABI::ABI_LP64 && (Type == MVT::f32))
    return false;

  return true;
}

bool CrampTargetLowering::shouldSignExtendTypeInLibCall(EVT Type, bool IsSigned) const {
...
bool CrampTargetLowering::decomposeMulByConstant(LLVMContext &Context, EVT VT,
                                                 SDValue C) const {
...
bool CrampTargetLowering::isMulAddWithConstProfitable(SDValue AddNode,
...
bool CrampTargetLowering::allowsMisalignedMemoryAccesses(
...
bool CrampTargetLowering::splitValueIntoRegisterParts(
...
SDValue CrampTargetLowering::joinRegisterPartsIntoValue(
...
SDValue
CrampTargetLowering::BuildSDIVPow2(SDNode *N, const APInt &Divisor,
                                   SelectionDAG &DAG,
...
#include "CrampGenAsmMatcher.inc"

Register
CrampTargetLowering::getRegisterByName(const char *RegName, LLT VT,
                                       const MachineFunction &MF) const {
  Register Reg = MatchRegisterAltName(RegName);
  if (Reg == Cramp::NoRegister)
...
namespace llvm {
namespace CrampVIntrinsicsTable {

#define GET_CrampVIntrinsicsTable_IMPL
