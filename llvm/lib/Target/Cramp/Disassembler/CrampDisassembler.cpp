//===-- CrampDisassembler.cpp - Disassembler for Cramp --------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the CrampDisassembler class.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "cramp-disassembler"

typedef MCDisassembler::DecodeStatus DecodeStatus;

namespace {
class CrampDisassembler : public MCDisassembler {
  std::unique_ptr<MCInstrInfo const> const MCII;

public:
  CrampDisassembler(const MCSubtargetInfo &STI, MCContext &Ctx,
...
static MCDisassembler *createCrampDisassembler(const Target &T,
                                               const MCSubtargetInfo &STI,
                                               MCContext &Ctx) {
  return new CrampDisassembler(STI, Ctx, T.createMCInstrInfo());
}

extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializeCrampDisassembler() {
  // Register the disassembler for each target.
  TargetRegistry::RegisterMCDisassembler(getTheCramp32Target(),
                                         createCrampDisassembler);
  TargetRegistry::RegisterMCDisassembler(getTheCramp64Target(),
                                         createCrampDisassembler);
}
...
static DecodeStatus DecodeFPR16RegisterClass(MCInst &Inst, uint64_t RegNo,
  ...
  MCRegister Reg = Cramp::F0_H + RegNo;
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeFPR32RegisterClass(MCInst &Inst, uint64_t RegNo,
  ...
  MCRegister Reg = Cramp::F0_F + RegNo;
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeFPR32CRegisterClass(MCInst &Inst, uint64_t RegNo,
  ...
  MCRegister Reg = Cramp::F8_F + RegNo;
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeFPR64RegisterClass(MCInst &Inst, uint64_t RegNo,
  ...
  MCRegister Reg = Cramp::F0_D + RegNo;
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeFPR64CRegisterClass(MCInst &Inst, uint64_t RegNo,
  ...
  MCRegister Reg = Cramp::F8_D + RegNo;
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeVRM2RegisterClass(MCInst &Inst, uint64_t RegNo,
  ...
  const CrampDisassembler *Dis =
      static_cast<const CrampDisassembler *>(Decoder);
  const MCRegisterInfo *RI = Dis->getContext().getRegisterInfo();
  MCRegister Reg =
      RI->getMatchingSuperReg(Cramp::V0 + RegNo, Cramp::sub_vrm1_0,
                              &CrampMCRegisterClasses[Cramp::VRM2RegClassID]);
...
static DecodeStatus DecodeVRM4RegisterClass(MCInst &Inst, uint64_t RegNo,
  ...
  MCRegister Reg =
      RI->getMatchingSuperReg(Cramp::V0 + RegNo, Cramp::sub_vrm1_0,
                              &CrampMCRegisterClasses[Cramp::VRM4RegClassID]);
...
static DecodeStatus DecodeVRM8RegisterClass(MCInst &Inst, uint64_t RegNo,
  ...
  MCRegister Reg =
      RI->getMatchingSuperReg(Cramp::V0 + RegNo, Cramp::sub_vrm1_0,
                              &CrampMCRegisterClasses[Cramp::VRM8RegClassID]);
...
static DecodeStatus decodeVMaskReg(MCInst &Inst, uint64_t RegNo,
                                   uint64_t Address,
                                   const MCDisassembler *Decoder) {
  MCRegister Reg = Cramp::NoRegister;
  switch (RegNo) {
  default:
    return MCDisassembler::Fail;
  case 0:
...
static void addImplySP(MCInst &Inst, int64_t Address,
                       const MCDisassembler *Decoder) {
  if (Inst.getOpcode() == Cramp::C_LWSP || Inst.getOpcode() == Cramp::C_SWSP ||
      Inst.getOpcode() == Cramp::C_LDSP || Inst.getOpcode() == Cramp::C_SDSP ||
      Inst.getOpcode() == Cramp::C_FLWSP ||
      Inst.getOpcode() == Cramp::C_FSWSP ||
      Inst.getOpcode() == Cramp::C_FLDSP ||
      Inst.getOpcode() == Cramp::C_FSDSP ||
      Inst.getOpcode() == Cramp::C_ADDI4SPN) {
    DecodeGPRRegisterClass(Inst, 2, Address, Decoder);
  }
  if (Inst.getOpcode() == Cramp::C_ADDI16SP) {
    DecodeGPRRegisterClass(Inst, 2, Address, Decoder);
    DecodeGPRRegisterClass(Inst, 2, Address, Decoder);
  }
}
...
static DecodeStatus decodeFRMArg(MCInst &Inst, uint64_t Imm, int64_t Address,
  ...
  if (!llvm::CrampFPRndMode::isValidRoundingMode(Imm))
    return MCDisassembler::Fail;
...
#include "CrampGenDisassemblerTables.inc"
...
DecodeStatus CrampDisassembler::getInstruction(MCInst &MI, uint64_t &Size,
  ...
  if ((Bytes[0] & 0x3) == 0x3) {
    ...
    if (STI.getFeatureBits()[Cramp::FeatureStdExtZdinx] &&
        !STI.getFeatureBits()[Cramp::Feature64Bit]) {
    ...
    if (STI.getFeatureBits()[Cramp::FeatureStdExtZfinx]) {
    ...
    LLVM_DEBUG(dbgs() << "Trying Cramp32 table :\n");
    ...
    if (!STI.getFeatureBits()[Cramp::Feature64Bit]) {
      LLVM_DEBUG(
          dbgs() << "Trying Cramp32Only_16 table (16-bit Instruction):\n");
      // Calling the auto-generated decoder function.
      Result = decodeInstruction(DecoderTableCramp32Only_16, MI, Insn, Address,
                                 this, STI);
      if (Result != MCDisassembler::Fail) {
        Size = 2;
        return Result;
    ...
    LLVM_DEBUG(dbgs() << "Trying Cramp_C table (16-bit Instruction):\n");
    // Calling the auto-generated decoder function.
