//===-- CrampFrameLowering.cpp - Cramp Frame Information ------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains the Cramp implementation of TargetFrameLowering class.
//
//===----------------------------------------------------------------------===//
...
static void emitSCSPrologue(MachineFunction &MF, MachineBasicBlock &MBB,
  ...
  Register SCSPReg = CrampABI::getSCSPReg();
  ...
  const CrampInstrInfo *TII = STI.getInstrInfo();
  bool IsRV64 = STI.hasFeature(Cramp::Feature64Bit);
  int64_t SlotSize = STI.getXLen() / 8;
  // Store return address to shadow call stack
  // s[w|d]  ra, 0(s2)
  // addi    s2, s2, [4|8]
  BuildMI(MBB, MI, DL, TII->get(IsRV64 ? Cramp::SD : Cramp::SW))
      .addReg(RAReg)
      .addReg(SCSPReg)
      .addImm(0)
      .setMIFlag(MachineInstr::FrameSetup);
  BuildMI(MBB, MI, DL, TII->get(Cramp::ADDI))
...
static void emitSCSEpilogue(MachineFunction &MF, MachineBasicBlock &MBB,
  ...
  // l[w|d]  ra, -[4|8](s2)
  // addi    s2, s2, -[4|8]
  BuildMI(MBB, MI, DL, TII->get(IsRV64 ? Cramp::LD : Cramp::LW))
...
static int getLibCallID(const MachineFunction &MF,
  ...
  Register MaxReg = Cramp::NoRegister;
  for (auto &CS : CSI)
    // CrampRegisterInfo::hasReservedSpillSlot assigns negative frame indexes to
    // registers which can be saved by libcall.
    if (CS.getFrameIdx() < 0)
      MaxReg = std::max(MaxReg.id(), CS.getReg().id());

  if (MaxReg == Cramp::NoRegister)
    return -1;

  switch (MaxReg) {
  default:
    llvm_unreachable("Something has gone wrong!");
  case /*s11*/ Cramp::X27: return 12;
  case /*s10*/ Cramp::X26: return 11;
  case /*s9*/  Cramp::X25: return 10;
  case /*s8*/  Cramp::X24: return 9;
  case /*s7*/  Cramp::X23: return 8;
  case /*s6*/  Cramp::X22: return 7;
  case /*s5*/  Cramp::X21: return 6;
  case /*s4*/  Cramp::X20: return 5;
  case /*s3*/  Cramp::X19: return 4;
  case /*s2*/  Cramp::X18: return 3;
  case /*s1*/  Cramp::X9:  return 2;
  case /*s0*/  Cramp::X8:  return 1;
  case /*ra*/  Cramp::X1:  return 0;
  }
}
...
bool CrampFrameLowering::hasFP(const MachineFunction &MF) const {
...
bool CrampFrameLowering::hasBP(const MachineFunction &MF) const {
  const MachineFrameInfo &MFI = MF.getFrameInfo();
...
void CrampFrameLowering::determineFrameLayout(MachineFunction &MF) const {
  MachineFrameInfo &MFI = MF.getFrameInfo();
  auto *RVFI = MF.getInfo<CrampMachineFunctionInfo>();
...
uint64_t CrampFrameLowering::getStackSizeWithRVVPadding(
...
void CrampFrameLowering::adjustReg(MachineBasicBlock &MBB,
  ...
  if (isInt<12>(Val)) {
    BuildMI(MBB, MBBI, DL, TII->get(Cramp::ADDI), DestReg)
        .addReg(SrcReg)
        .addImm(Val)
        .setMIFlag(Flag);
    return;
...
static Register getFPReg(const CrampSubtarget &STI) { return Cramp::X8; }

// Returns the register used to hold the stack pointer.
static Register getSPReg(const CrampSubtarget &STI) { return Cramp::X2; }
...
void CrampFrameLowering::adjustStackForRVV(MachineFunction &MF,
...
void CrampFrameLowering::emitPrologue(MachineFunction &MF,
                                      MachineBasicBlock &MBB) const {
  MachineFrameInfo &MFI = MF.getFrameInfo();
  auto *RVFI = MF.getInfo<CrampMachineFunctionInfo>();
  const CrampRegisterInfo *RI = STI.getRegisterInfo();
  const CrampInstrInfo *TII = STI.getInstrInfo();
  MachineBasicBlock::iterator MBBI = MBB.begin();

  Register FPReg = getFPReg(STI);
  Register SPReg = getSPReg(STI);
  Register BPReg = CrampABI::getBPReg();
  ...
  if (hasFP(MF)) {
    ...
    if (RI->hasStackRealignment(MF)) {
      ...
      if (isInt<12>(-(int)MaxAlignment.value())) {
        BuildMI(MBB, MBBI, DL, TII->get(Cramp::ANDI), SPReg)
            .addReg(SPReg)
        ...
        Register VR =
            MF.getRegInfo().createVirtualRegister(&Cramp::GPRRegClass);
        BuildMI(MBB, MBBI, DL, TII->get(Cramp::SRLI), VR)
            .addReg(SPReg)
            .addImm(ShiftAmount)
            .setMIFlag(MachineInstr::FrameSetup);
        BuildMI(MBB, MBBI, DL, TII->get(Cramp::SLLI), SPReg)
            .addReg(VR)
            .addImm(ShiftAmount)
            .setMIFlag(MachineInstr::FrameSetup);
      }
      ...
      if (hasBP(MF)) {
        // move BP, SP
        BuildMI(MBB, MBBI, DL, TII->get(Cramp::ADDI), BPReg)
            .addReg(SPReg)
            .addImm(0)
            .setMIFlag(MachineInstr::FrameSetup);
      }
...
void CrampFrameLowering::emitEpilogue(MachineFunction &MF,
                                      MachineBasicBlock &MBB) const {
  const CrampRegisterInfo *RI = STI.getRegisterInfo();
  MachineFrameInfo &MFI = MF.getFrameInfo();
  auto *RVFI = MF.getInfo<CrampMachineFunctionInfo>();
...
StackOffset
CrampFrameLowering::getFrameIndexReference(const MachineFunction &MF, int FI,
  ...
  assert(FrameReg == CrampABI::getBPReg() || !MFI.hasVarSizedObjects());
...
void CrampFrameLowering::determineCalleeSaves(MachineFunction &MF,
                                              BitVector &SavedRegs,
  ...
  // pointer.
  if (hasFP(MF)) {
    SavedRegs.set(Cramp::X1);
    SavedRegs.set(Cramp::X8);
  }
  // Mark BP as used if function has dedicated base pointer.
  if (hasBP(MF))
    SavedRegs.set(CrampABI::getBPReg());
  ...
  if (MF.getFunction().hasFnAttribute("interrupt") && MFI.hasCalls()) {
    ...
    static const MCPhysReg CSRegs[] = { Cramp::X1,      /* ra */
      Cramp::X5, Cramp::X6, Cramp::X7,                  /* t0-t2 */
      Cramp::X10, Cramp::X11,                           /* a0-a1, a2-a7 */
      Cramp::X12, Cramp::X13, Cramp::X14, Cramp::X15, Cramp::X16, Cramp::X17,
      Cramp::X28, Cramp::X29, Cramp::X30, Cramp::X31, 0 /* t3-t6 */
    };

    for (unsigned i = 0; CSRegs[i]; ++i)
      SavedRegs.set(CSRegs[i]);

    if (MF.getSubtarget<CrampSubtarget>().hasStdExtF()) {

      // If interrupt is enabled, this list contains all FP registers.
      const MCPhysReg * Regs = MF.getRegInfo().getCalleeSavedRegs();

      for (unsigned i = 0; Regs[i]; ++i)
        if (Cramp::FPR16RegClass.contains(Regs[i]) ||
            Cramp::FPR32RegClass.contains(Regs[i]) ||
            Cramp::FPR64RegClass.contains(Regs[i]))
...
CrampFrameLowering::assignRVVStackObjectOffsets(MachineFunction &MF) const {
  MachineFrameInfo &MFI = MF.getFrameInfo();
  ...
  const auto &ST = MF.getSubtarget<CrampSubtarget>();
...
static unsigned getScavSlotsNumForRVV(MachineFunction &MF) {
  ...
  if (!MF.getSubtarget<CrampSubtarget>().hasVInstructions())
    return false;
        ...
        if (IsRVVSpill) {
          MaxScavSlotsNum = std::max(
              MaxScavSlotsNum, IsScalableVectorID
                                   ...
                                   : ScavSlotsNumRVVSpillNonScalableObject);
        } else if (MI.getOpcode() == Cramp::ADDI && IsScalableVectorID) {
          MaxScavSlotsNum =
...
void CrampFrameLowering::processFunctionBeforeFrameFinalized(
    MachineFunction &MF, RegScavenger *RS) const {
  const CrampRegisterInfo *RegInfo =
...
static bool hasRVVFrameObject(const MachineFunction &MF) {
  ...
  return MF.getSubtarget<CrampSubtarget>().hasVInstructions();
}
...
bool CrampFrameLowering::hasReservedCallFrame(const MachineFunction &MF) const {
  return !MF.getFrameInfo().hasVarSizedObjects() &&
         !(hasFP(MF) && hasRVVFrameObject(MF));
}

// Eliminate ADJCALLSTACKDOWN, ADJCALLSTACKUP pseudo instructions.
MachineBasicBlock::iterator CrampFrameLowering::eliminateCallFramePseudoInstr(
    MachineFunction &MF, MachineBasicBlock &MBB,
    MachineBasicBlock::iterator MI) const {
  Register SPReg = Cramp::X2;
  ...
  if (!hasReservedCallFrame(MF)) {
    ...
    if (Amount != 0) {
      ...
      if (MI->getOpcode() == Cramp::ADJCALLSTACKDOWN)
        Amount = -Amount;
...
//   add     sp,sp,-64
uint64_t
CrampFrameLowering::getFirstSPAdjustAmount(const MachineFunction &MF) const {
  const auto *RVFI = MF.getInfo<CrampMachineFunctionInfo>();
  const MachineFrameInfo &MFI = MF.getFrameInfo();
...
bool CrampFrameLowering::spillCalleeSavedRegisters(
  ...
  if (SpillLibCall) {
    ...
    BuildMI(MBB, MI, DL, TII.get(Cramp::PseudoCALLReg), Cramp::X5)
        .addExternalSymbol(SpillLibCall, CrampII::MO_CALL)
        .setMIFlag(MachineInstr::FrameSetup);
...
bool CrampFrameLowering::restoreCalleeSavedRegisters(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator MI,
  ...
  if (RestoreLibCall) {
    ...
    MachineBasicBlock::iterator NewMI =
        BuildMI(MBB, MI, DL, TII.get(Cramp::PseudoTAIL))
            .addExternalSymbol(RestoreLibCall, CrampII::MO_CALL)
            .setMIFlag(MachineInstr::FrameDestroy);

    // Remove trailing returns, since the terminator is now a tail call to the
    // restore function.
    if (MI != MBB.end() && MI->getOpcode() == Cramp::PseudoRET) {
...
bool CrampFrameLowering::enableShrinkWrapping(const MachineFunction &MF) const {
...
bool CrampFrameLowering::canUseAsPrologue(const MachineBasicBlock &MBB) const {
  ...
  // Inserting a call to a __cramp_save libcall requires the use of the register
  ...
  return !RS.isRegUsed(Cramp::X5);
}

bool CrampFrameLowering::canUseAsEpilogue(const MachineBasicBlock &MBB) const {
  const MachineFunction *MF = MBB.getParent();
  MachineBasicBlock *TmpMBB = const_cast<MachineBasicBlock *>(&MBB);
  const auto *RVFI = MF->getInfo<CrampMachineFunctionInfo>();
  ...
  // Using the __cramp_restore libcalls to restore CSRs requires a tail call.
...
bool CrampFrameLowering::isSupportedStackID(TargetStackID::Value ID) const {
  switch (ID) {
...
TargetStackID::Value CrampFrameLowering::getStackIDForScalableVectors() const {
