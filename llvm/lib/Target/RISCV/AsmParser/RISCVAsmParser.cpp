...
public:
  ...
  bool isSImm3Not0Nor4() const {
    if (!isImm())
      return false;
    RISCVMCExpr::VariantKind VK = RISCVMCExpr::VK_RISCV_None;
    int64_t Imm;
    bool IsConstantImm = evaluateConstantImm(getImm(), Imm, VK);
    return IsConstantImm && isInt<3>(Imm) && (Imm != 0) && (Imm != 4) &&
           VK == RISCVMCExpr::VK_RISCV_None;
  }
  ...
  bool isSImm7Lsb00NonZero() const {
    if (!isImm())
      return false;
    RISCVMCExpr::VariantKind VK = RISCVMCExpr::VK_RISCV_None;
    int64_t Imm;
    bool IsConstantImm = evaluateConstantImm(getImm(), Imm, VK);
    return IsConstantImm && isShiftedInt<5, 2>(Imm) && (Imm != 0) &&
           VK == RISCVMCExpr::VK_RISCV_None;
  }
  ...
  bool isUImm4Lsb0() const {
    ...
    RISCVMCExpr::VariantKind VK = RISCVMCExpr::VK_RISCV_None;
    bool IsConstantImm = evaluateConstantImm(getImm(), Imm, VK);
    return IsConstantImm && isShiftedUInt<3, 1>(Imm) &&
           VK == RISCVMCExpr::VK_RISCV_None;
  }
  ...
  bool isSImm6Lsb0() const { return isBareSimmNLsb0<6>(); }
...
bool RISCVAsmParser::MatchAndEmitInstruction(SMLoc IDLoc, unsigned &Opcode,
  ...
  case Match_InvalidUImm4Lsb0:
    return generateImmOutOfRangeError(Operands, ErrorInfo, 0, (1 << 4) - 2,
